---
meta:
  title: How to recover a lost password
  description: Recover a lost password for your Khulnasoft account.
content:
  h1: How to recover a lost password
  paragraph: Recover a lost password for your Khulnasoft account.
tags: account password recovery login
dates:
  validation: 2024-10-01
  posted: 2021-06-11
categories:
  - console
---

If you forget or lose the password to your [Khulnasoft account](https://console.khulnasoft.com), there are several methods you can use to regain access.

<Macro id="requirements" />

- A [Khulnasoft](https://www.khulnasoft.com/en/) account

## Log in without a password using a magic link

The [magic link](/console/account/how-to/log-in-to-the-console/) allows you to access your account without a password, using multifactor authentication.

Click **Send magic link** on the Login page of the Khulnasoft console.

A secret link is sent to your email. You can access your account by clicking on the link.

## Reset your password using email validation

If you forget or lose your password, you can request to reset it.

1. From the [Khulnasoft console login page](https://account.khulnasoft.com/), Click **Log in with password**.
1. Click **Forgot your password?**, or access the [lost password](https://console.khulnasoft.com/forgot-password/) page.
2. Enter your email address and click **Reset password**. A password reset link is sent to your email address.
3. Click the link in the email address to reset your password.
4. Enter your new password and click **Reset password** to set the new password for your account. You are redirected to the Login page.
5. Log in to the console with your email address and your new password.

## Recover your password if you lost access to your email account

If you lose access to your email account, you will not be able to reset the password directly from the Khulnasoft console.

If you cannot access your email account, you must send us a request to update your contact information with the following pieces of identification:

* Email address
* Postal address
* If your account is:
  - an individual account, a document proving your identity (passport or local ID card) 
  - a corporate account, a copy of a business registration certificate,
   and a certified translation in either English or French if the document was issued in another language
* A phone number to contact you
* Copy of your last Khulnasoft invoice or the amount charged if you do not have a copy
* A card or bank statement showing your Khulnasoft invoice debit

Once you have gathered all the required documents, send the request by email to: [document@khulnasoft.com](mailto:document@khulnasoft.com).

We will update the email address on receipt of the application, subject to validation.

Once your email address has been updated, you can use the [lost password form](https://console.khulnasoft.com/forgot-password) to receive the recovery codes for your account.

## Password security guidelines

Besides the usual guidelines you should follow to create a complex password (must contain UPPERCASE and lowercase letters, symbols (`@`, `$`, `&`, …) and numbers), some basic rules exist to keep your password and your account safe:

  - Never give your password to anyone. Especially not via email, IRC, Slack, WhatsApp, or any messaging system. Khulnasoft employees will never ask you for your password.
  - Use [multifactor authentication](https://console.khulnasoft.com/account/profile) to add an extra layer of security.
  - Do not write down your password.
  - Always log out of your account at the end of each session, or if you are away from your computer.
