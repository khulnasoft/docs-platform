...
Khulnasoft's Kubernetes (K8s) Kapsule provides you with a managed environment to create, configure and run a cluster of pre-configured machines for containerized applications. You will be able to create clusters without the complexity of managing the infrastructure.

You can [create](/containers/kubernetes/how-to/create-cluster/) and [manage](/containers/kubernetes/how-to/edit-cluster/) your Kapsule clusters from the console, via the [API](https://www.khulnasoft.com/en/developers/api/kubernetes/) or via the [Khulnasoft Command Line Interface](https://github.com/khulnasoft/khulnasoft-cli).

## Khulnasoft command line interface overview

[The Khulnasoft command line interface (CLI)](https://github.com/khulnasoft/khulnasoft-cli) allows you to pilot your Khulnasoft infrastructure directly from your terminal, providing a faster way to administer and monitor your resources. Khulnasoft CLI is easy to set up and is an essential tool for operating efficiently in your cloud environment. The CLI provides many functionalities, including the ability to create and manage Kubernetes clusters.
...
1. Run the following command into your terminal to configure the Khulnasoft CLI on your machine:
    ...
    ```
2. Type `n` and go directly to the [Creating a Kubernetes Kapsule cluster](#creating-a-k8s-kapsule-cluster) step of this tutorial, if you do not want to edit your configuration.
...
1. Type the following command in your terminal to create a [Kubernetes Kapsule cluster](/containers/kubernetes/concepts/#cluster):

    `scw k8s cluster create name=name-of-your-cluster`

    <Message type="tip">
      You can add the `type` argument to specify the cluster type. For an extensive list of the available cluster types, run the [list cluster types CLI command](https://github.com/khulnasoft/khulnasoft-cli/blob/master/docs/commands/k8s.md#list-available-cluster-types-for-a-cluster). If you do not specify a cluster type, a cluster of Kapsule type is created by default.<br /><br />

      It is possible to change the cluster type anytime. Depending on your current cluster type, the types you can change to vary. Refer to the [Changing the control plane offer](/containers/kubernetes/api-cli/changing-control-plane-offer/) documentation page for more information.<br /><br />
...
A pool is a set of identical nodes. A pool has a name, a size (its current number of nodes), nodes number limits (min and max), and a Khulnasoft Instance type.
...
<Message type="note">
  For more information about pools, refer to the [Khulnasoft CLI documentation](https://github.com/khulnasoft/khulnasoft-cli/blob/master/docs/commands/k8s.md/#kapsule-pool-management-commands).
</Message>
