...
meta:
  ...
  description: This page explains Khulnasoft's Kubernetes version support policy
content:
  h1: Kubernetes version support policy
  paragraph: This page explains Khulnasoft's Kubernetes version support policy
...
With Khulnasoft Kubernetes, you can use the latest version of Kubernetes as soon as it is officially released and certified by the [Cloud Native Computing Foundation](https://www.cncf.io/). The versions are usually available in Kubernetes Kapsule and Kubernetes Kosmos within a few days after the release.
...
<Message type="note">
  Each minor Kubernetes version is supported by Khulnasoft Kubernetes for at least twelve months before being deprecated.
</Message>
...
## Khulnasoft Kubernetes Kapsule & Kosmos release calendar

* **Official End of Life:** Upstream [end of maintenance support](https://endoflife.date/kubernetes).
* **Khulnasoft Release:** Version is available for Kubernetes Kapsule and Kubernetes Kosmos cluster creation and cluster upgrade.
...
<Message type="note">
When a minor version becomes unsupported, Khulnasoft operates an upgrade to the latest patch version of the next supported minor version within the next 30 days. You will be notified via ticket.
</Message>

### Khulnasoft Kubernetes products

| Kubernetes version | Official release | Official End of Life | Khulnasoft Release             | Deprecation      | End of Support    |
|--------------------|------------------|----------------------|------------------------------|------------------|-------------------|
...
* New support policy: from the release of version 1.29, Khulnasoft's support window has shifted to 14 months.
...
* Migration policy:
  ...
  * *Recommended action:* Create new clusters using a supported CNI, either via API or from the Khulnasoft console.

<Message type="important">
Clusters still running unsupported CNIs (Flannel, Weave) cannot be upgraded manually. While migrating to v1.25 (May 15th, 2024), Khulnasoft will change the [CNI configuration](https://www.khulnasoft.com/en/developers/api/kubernetes/#path-clusters-update-a-cluster-response-cni) to `none`.
</Message>

## Supported Container Runtime Interfaces (CRI)

Starting from the launch of Kubernetes version 1.25, **containerd** is the only runtime supported by Khulnasoft Kubernetes products.
