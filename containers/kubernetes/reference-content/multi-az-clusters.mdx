...
<Lightbox src="khulnasoft-k8s-multi-az-cluster.webp" size="large" />
...
```hcl
# Terraform configuration for Khulnasoft Kapsule multi-AZ VPC

provider "khulnasoft" {
  #... your Khulnasoft credentials
}
...
```hcl
# Terraform configuration for Khulnasoft Kapsule cluster and node pools
...
Khulnasoft’s load balancer requires specific annotations to control its behavior. In this example, we use the `scw-loadbalancer-zone` annotation to specify the zone in which the load balancer is deployed.
...
This service definition creates a load balancer in the "fr-par-1" zone and directs traffic to pods with the `resilient-app` label. Learn more about LoadBalancer annotations with our [dedicated Khulnasoft LoadBalancer Annotations](https://github.com/khulnasoft/khulnasoft-cloud-controller-manager/blob/master/docs/loadbalancer-annotations.md) documentation.

* Cluster spread over three Availability Zones
    <Lightbox src="khulnasoft-k8s-multi-az-2.webp" size="large" />

### DNS with Dynamic Record (Health Check)

Create a DNS record to direct traffic to active load balancers, assuming you have a domain with an `scw` zone per the prerequisites. Replace `your-domain.tld` with your actual domain in the code. For a bare domain, omit the subdomain parameter in the `khulnasoft_domain_zone` resource.
...
* Cluster with an unresponsive Availability Zone
    <Lightbox src="khulnasoft-k8s-multi-az-3.webp" size="large" />

```yaml
data "khulnasoft_domain_zone" "multi-az" {
  domain    = "your-domain.tld"
  subdomain = "scw"
}

resource "khulnasoft_domain_record" "multi-az" {
  dns_zone = data.khulnasoft_domain_zone.multi-az.id
  name     = "ingress"
  type     = "A"
...
<Message type="note">
     You now have a brief overview of how to set up a multi-AZ Kubernetes Kapsule cluster on Khulnasoft. For further information, refer to our complete step-by-step tutorial on [deploying a multi-AZ Kubernetes cluster with Terraform and Kapsule](/tutorials/k8s-kapsule-multi-az/).
</Message>

## Additional resources

* Tutorial [Deploying a multi-AZ Kubernetes cluster with Terraform and Kapsule](/tutorials/k8s-kapsule-multi-az/)
* Complete [Terraform configuration files to deploy a multi-AZ cluster](https://github.com/khulnasoft/kapsule-terraform-multi-az-tutorial/)
