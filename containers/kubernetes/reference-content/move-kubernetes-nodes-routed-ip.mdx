...
As part of our ongoing efforts to enhance infrastructure capabilities and provide better services to our customers, Khulnasoft is [introducing routed IPs for all products](https://www.khulnasoft.com/en/news/routed-ips-are-coming-to-all-khulnasoft-products/), including Kubernetes (K8s) worker nodes.

One of the standout benefits of routed IPs is their support for [IP mobility](/compute/instances/concepts/#ip-mobility), streamlining IP management and movement within the Khulnasoft ecosystem. By simplifying the process of reallocating IPs across different products, routed IPs empower users with unprecedented flexibility and control over their network infrastructure.
...
<Message type="important">
    NAT IPs will reach end-of-life on October 1st, 2024. After this date, Khulnasoft will organize migrations for all clusters to transition to routed IPs.
    Migrations are planned region by region:
    * For clusters in Warsaw (`pl-waw-1`), the migration is scheduled for October 1st to October 7th, 2024. [Learn more](https://status.khulnasoft.com/incidents/l45384bsm5w0).
    * For clusters in Amsterdam (`nl-ams-1`, `nl-ams-2`, and `nl-ams-3`), the migration is scheduled for October 7 to October 14, 2024. [Learn more](https://status.khulnasoft.com/incidents/bctz2b9fhpqc).
    * For clusters in Paris (`fr-par-1`, `fr-par-2`, and `fr-ar-3`), the migration is scheduled for October 14 to October 21, 2024. [Learn more](https://status.khulnasoft.com/incidents/1gh354wb3j27).
</Message>
...
### Using the Khulnasoft console

1. Click **Kubernetes** in the **Containers** section of the [Khulnasoft console](https://console.khulnasoft.com) side menu. The Kubernetes overview displays.
...
1. Configure the following variables for your Kubernetes cluster:
    ...
    SCW_SECRET_KEY=196dadd9-5685-42c8-aa71-a34d46861303 # Your Khulnasoft secret key
    ```
2. Run the following command to launch the migration of existing Kubernetes clusters to routed IPs:
    ```
    curl --location "https://api.khulnasoft.com/k8s/v1/regions/$REGION/clusters/$CLUSTER_ID/migrate-to-routed-ips" --header "x-auth-token: $SCW_SECRET_KEY" --header 'Content-Type: application/json' --data '{}'
    ```
    <Message type="important">
