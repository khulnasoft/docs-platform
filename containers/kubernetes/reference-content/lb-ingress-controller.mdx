...
This document will guide you through deploying a test application on a Kubernetes cluster, exposing it via an ingress object, and using a Khulnasoft Load Balancer to ensure persistent IP addressing.
...
## Exposing the ingress controller using a Khulnasoft Load Balancer
...
1. In the [Khulnasoft console](https://console.khulnasoft.com/instance/security-groups), navigate to the **Compute** > **Security groups** section and find the security group named `kubernetes <cluster-id>`.
...
2. Create the ingress object (`coffee-ingress.yaml`) using the DNS wildcard provided by Khulnasoft:
    ...
    spec:
      rules:
        - host: YOUR_KHULNASOFT_DNS_WILDCARD
          http:
            paths:
              - path: /tea
                pathType: Prefix
...
4. Test the ingress:
    ...
    ```bash
    curl http://YOUR_KHULNASOFT_DNS_WILDCARD/coffee
    ```

## Using a reserved IP with a Load Balancer

Reserve a flexible Load Balancer IP address [through the Khulnasoft API](/containers/kubernetes/reference-content/managing-load-balancer-ips/#reserve-a-load-balancer-flexible-ip-address-via-the-api). Take note of the IP address, referred to as `RESERVED_IP` from now on.
