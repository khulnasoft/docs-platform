...
curl -X POST \
  ...
  -d '{}' \
  "https://api.khulnasoft.com/lb/v1/zones/$SCW_DEFAULT_ZONE/ips"
```

### Using the console

1. From the [Load Balancer Flexible IPs](https://console.khulnasoft.com/load-balancer/ips) page of the Khulnasoft console, click **Create flexible IP**.
...
You must have an existing [Load Balancer flexible IP](https://console.khulnasoft.com/load-balancer/ips) in your account, that is not attached to any other Load Balancer.
...
- Identify the IP address that was randomly assigned to the Load Balancer, either via the [Khulnasoft console](https://console.khulnasoft.com/load-balancer/ips) or via a `kubectl get svc` command. Let's imagine that this IP address is `51.159.10.49`.
