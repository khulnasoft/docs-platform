...
meta:
  ...
  description: This page provides information about the different control plane offers available for Khulnasoft Kubernetes Kapsule and Kosmos
content:
  h1: Kubernetes control plane offers overview
  paragraph: This page provides information about the different control plane offers available for Khulnasoft Kubernetes Kapsule and Kosmos
...
On a managed Kubernetes service such as Kapsule or Kosmos, Khulnasoft is responsible for overseeing the Kubernetes control plane.
...
Khulnasoft offers a range of Kubernetes control planes to cater to a wide range of use cases and to provide you with maximum flexibility.
...
<Lightbox src="khulnasoft-dcp-detailed.webp" alt="Schema Khulnasoft Dedicated control planes for Kubernetes" size="large" />
...
Once a dedicated control plane is no longer under commitment, you can freely migrate to a lower tier using the Khulnasoft console or command-line interface (CLI).
...
## etcd size limitation on Khulnasoft managed Kubernetes products

In Khulnasoft's managed Kubernetes offers, the size of the etcd key-value store is a critical factor that users must consider.
...
You can monitor the size of your control plane’s etcd using [Cockpit](/observability/cockpit/quickstart/), Khulnasoft’s intuitive management interface. This visibility helps in proactive management and ensures you remain within the allotted limits, avoiding potential disruptions.
...
Refer to our dedicated documentation for more information regarding the [choice of control plane offers for clusters](/faq/kubernetes/#can-i-choose-the-type-of-control-plane-for-my-cluster), as well as instructions on how to manage a control plane through the [console](/containers/kubernetes/how-to/edit-cluster/) or [API](https://www.khulnasoft.com/en/developers/api/kubernetes/).