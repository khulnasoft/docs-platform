...
Khulnasoft Kubernetes Kapsule provides a managed environment to create, configure, and run a cluster of preconfigured machines for containerized applications. This allows you to create Kubernetes clusters without the complexity of managing the infrastructure.

All new Kubernetes clusters are deployed with a Khulnasoft [Private Network](/network/vpc/concepts/#private-networks) using [controlled isolation](#what-is-the-difference-between-controlled-isolation-and-full-isolation).
...
<Lightbox src="khulnasoft-kubernetes-kapsule-on-pn-v2.webp" alt="Schema Kubernetes Kapsule on Private Networks" size="large" />
...
* **Implementation of best practices** in terms of security: all Khulnasoft resources can communicate securely (Instances, Load Balancers, Managed Databases), with less surface area for attack. For further information, refer to our blog post [10 best practices to configure your VPC](https://www.khulnasoft.com/en/blog/10-best-practices-to-configure-your-vpc/).
...
<Message type="note">
  ...
  * [Dynamic NAT must be activated](/network/public-gateways/concepts/#nat) (enabled by default).
  * [Advertise DefaultRoute must be activated](/network/public-gateways/concepts/#default-route) (enabled by default).
...
Kosmos uses Kilo as a CNI, which uses WireGuard to create a VPN Mesh between nodes for communication between pods. Any node in Kosmos, either in Khulnasoft or outside, uses these VPN tunnels to communicate securely by construct.

### Are Managed Databases compatible with Kubernetes Kapsule on Private Networks?

Yes, they are. Since July 2023, the automatic allocation of IP addresses via IPAM is available for Managed Databases. These IP addresses are compatible with Khulnasoft's VPC, which is now in General Availability. For more information about product compatibility, refer to the [VPC documentation](/network/vpc/troubleshooting/vpc-limitations/).
...
```bash
curl --request POST \
  --url https://api.khulnasoft.com/rdb/v1/regions/$REGION/instances/$INSTANCE_ID/endpoints \
...
Refer to the [Managed Database for PostgreSQL and MySQL API documentation](https://www.khulnasoft.com/en/developers/api/managed-database-postgre-mysql/#path-endpoints-create-a-new-database-instance-endpoint) for further information.
...
### From the Khulnasoft console

1. Go to the [Instances section](https://console.khulnasoft.com/instance/servers) of the Khulnasoft console.
...
You can find a Terraform configuration example below:
  ```json
  data "khulnasoft_k8s_version" "latest" {
  name = "latest"
  }

  resource "khulnasoft_vpc_private_network" "kapsule" {
    name = "pn_kapsule"
    tags = ["kapsule"]
  }

  resource "khulnasoft_k8s_cluster" "kapsule" {
    name                        = "open-pn-test"
    version                     = data.khulnasoft_k8s_version.latest.name
    cni                         = "cilium"
    private_network_id          = khulnasoft_vpc_private_network.kapsule.id
    delete_additional_resources = true
    depends_on                  = [khulnasoft_vpc_private_network.kapsule]
  }

  resource "khulnasoft_instance_security_group" "kapsule" {
    name                    = "kubernetes ${split("/", khulnasoft_k8s_cluster.kapsule.id)[1]}"
    ...
    depends_on              = [khulnasoft_k8s_cluster.kapsule]
  }

  resource "khulnasoft_k8s_pool" "default" {
    cluster_id          = khulnasoft_k8s_cluster.kapsule.id
    name                = "default"
    node_type           = "DEV1-M"
    size                = 1
    autohealing         = true
    wait_for_pool_ready = true
    depends_on          = [khulnasoft_instance_security_group.kapsule]
  }
  ...
  resource "khulnasoft_rdb_instance" "main" {
    ...
    private_network {
      pn_id = khulnasoft_vpc_private_network.kapsule.id
    }
  }
  ```
