...
We show you how to containerize two simple applications with the popular containerization open-source platform Docker, starting from the Dockerfile to the basic docker commands with examples. Finally, we demonstrate how to safely push the container images we have created to the Khulnasoft Container Registry, so they are securely stored in the cloud.

Future videos will show you how to deploy your containerized application with the Khulnasoft Kubernetes Kapsule, and cover topics like load balancing and storage for your Kubernetes application.
...
In this video, we show you how to deploy a containerized application with the Khulnasoft Kubernetes Kapsule.

First, we review some key Kubernetes terminology (including pools, nodes, and pods) and then demonstrate how to create a Kubernetes Kapsule via the Khulnasoft console.
...
In this video, we explain why you should add a load balancer service to your Khulnasoft Kubernetes Kapsule, and walk you through how to do it.
...
Next, we show you how to create your LoadBalancer service with a YAML manifest and `kubectl`. The CloudController in the control plane takes care of creating your external Khulnasoft Load Balancer.

We then demonstrate how to find the external IP of your Load Balancer, either via the Khulnasoft console or by using `kubectl`, and how to check out your cluster's application using this IP address.
...
In this video, we explain when and how to add storage to your Khulnasoft Kubernetes Kapsule, and walk you through how to do it.
...
Next, we show you how to create a StatefulSet object with its own Khulnasoft Block Storage volumes, using a yaml manifest and kubectl. The CSI takes care of provisioning the storage from Khulnasoft.
...
<Iframe src="https://www.youtube.com/embed/OAR_3Fuwtw0" title="YouTube video player" />

<Message type="tip">
    A written tutorial version of this video is [also available](/tutorials/get-started-kubernetes-storage/)
</Message>
...
### The magic behind Khulnasoft's managed Kubernetes

In this video, dive into the magic behind Khulnasoft's managed Kubernetes. You can learn how we made a managed Kubernetes product and how this product works. Additionally, we take a look at the lifecycle of a cluster, and the challenges related to running Kubernetes inside Kubernetes.
