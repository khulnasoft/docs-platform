...
A cluster is a set of machines, called nodes, running containerized applications managed by [Kubernetes](#kubernetes). A cluster has several worker nodes and at least one control plane. Refer to the [control plane](#control-plane) concept for more information regarding the cluster's limitations. Khulnasoft allows you to create two configurations of clusters: [Kapsule](#kubernetes-kapsule), for clusters comprising Khulnasoft Instances, and [Kosmos](#kubernetes-kosmos), for multi-cloud clusters. Clusters can be tailored to availability requirements depending on the cluster availability types: zonal (single-zone or multi-AZ) and regional. 
...
The control plane manages the worker nodes and the pods in the cluster. In production environments, the control plane usually runs across multiple computers, and a cluster usually runs multiple nodes, providing fault-tolerance and high availability. Khulnasoft manages the control plane and associated Load Balancers. Consider the following when creating a control plane:

- A cluster belongs to one region.
- As the cluster's control plane and Load Balancer are managed by Khulnasoft, it is not possible to access them directly or configure them individually.
...
The Easy Deploy feature allows you to pull images directly from Khulnasoft Container Registry, instantly deploying containerized applications in your Kubernetes Kapsule cluster. With only the basic options to set, you can use Kubernetes Kapsule without needing to manage your `.yaml ` manifests. Check out our documentation on [creating containerized applications with the Easy Deploy feature](/containers/kubernetes/how-to/enable-easy-deploy/) for more information.
...
`kubeconfig` is a file provided by Khulnasoft when creating a Kubernetes cluster. It allows you to manage your cluster from your local computer by using kubectl.
...
The Pool resource is a group of Khulnasoft Instances, organized by type (e.g., GP1-S, GP1-M). It represents the computing power of the cluster and contains the Kubernetes nodes, on which the containers run. Consider the following when creating a Pool:
