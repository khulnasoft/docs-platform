---
meta:
  title: Configuring a SQL storage backend in Helm on Khulnasoft Kubernetes Kapsule
  description: Troubleshoot issues deploying large Helm releases on Khulnasoft Kubernetes Kapsule by configuring a SQL storage backend.
content:
  h1: Configuring a SQL storage backend in Helm on Khulnasoft Kubernetes Kapsule
  paragraph: Troubleshoot issues deploying large Helm releases on Khulnasoft Kubernetes Kapsule by configuring a SQL storage backend.
...
When deploying large Helm releases on Khulnasoft Kapsule Kubernetes, you might encounter errors like the following:
...
### Option A: Khulnasoft's managed PostgreSQL Service
1. [Create a PostgreSQL instance](/managed-databases/postgresql-and-mysql/how-to/create-a-database/) using Khulnasoft's Managed Database service to create a PostgreSQL database.
...
<Message type="important">
    **Update security group rules**<br />
    When you create a Khulnasoft Kapsule cluster, a new security group blocks incoming traffic by default. To allow external traffic (if needed):

    1. Navigate to **Compute > Security Groups** in the Khulnasoft console.
...
If you have further questions, feel free to engage with the [Khulnasoft Slack community](https://slack.khulnasoft.com) to exchange insights, share experiences, and discover practical solutions.