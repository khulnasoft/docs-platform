...
2. You should see the **Private Network** connected to the deployment resource and its **allocated IPs** listed.
    <Lightbox src="khulnasoft-inference-pn-connected.webp" alt="A screenshot of the Khulnasoft console, hightlighting the Private Network configuration of the Managed Inference deployment" size="medium" />
...
2. Download the TLS certificate from your Managed Inference deployment, available from the **Overview** tab in the **Endpoints** section.
    <Lightbox src="khulnasoft-inference-tls-dl.webp" alt="A screenshot of the Managed Interface product overview tab in the Khulnasoft console, highlighting the TLS certificate download section" size="medium" />
3. Transfer the TLS certificate to the Instance using the `scp` (secure copy) command to securely transfer the certificate from your local machine to the Khulnasoft Instance.
   - Example command:
     ```bash
   ...
   - Replace placeholders in the command above as follows:
     ...
     - `root`: Your Khulnasoft Instance username (`root` for default configuration).
     - `51.115.xxx.yyy`: Public IP address of your Khulnasoft Instance.
     - `:/root`: Destination directory on the Khulnasoft Instance.
    <Message type="note">
...
4. Click **Detach Private Network**. A pop-up displays.
    <Lightbox src="khulnasoft-inference-pn-detach.webp" alt="A screenshot of the Managed Interface product overview tab in the Khulnasoft console, highlighting the Private Network detach section" size="medium" />
