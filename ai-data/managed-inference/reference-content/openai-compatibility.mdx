...
meta:
  ...
  description: Khulnasoft Managed Inference provides compatibility with parts of the OpenAI API, to help connect existing applications.
content:
  h1: Khulnasoft Managed Inference as drop-in replacement for the OpenAI APIs
  paragraph: This page provides information on Khulnasoft's managed inference compatibility with OpenAI's REST API
...
You can use any of the OpenAI [official libraries](https://platform.openai.com/docs/libraries/), for example, the [OpenAI Python client library](https://github.com/openai/openai-python) to interact with your Khulnasoft Managed Inference deployment. 
...
To invoke Khulnasoft Managed Inference's OpenAI-compatible Chat API, simply edit your dedicated endpoints with a suffix `/v1/chat/completions`:
```
https://<Deployment UUID>.ifr.fr-par.khulnasoft.com/v1/chat/completions
```
...
client = OpenAI(
    base_url='https://<Deployment UUID>.ifr.fr-par.khulnasoft.com/v1/',
    api_key='<IAM API key>'
)
...
chat_completion = client.chat.completions.create(
    messages=[
        ...
        {
            "role": "user",
            "content": "Sing me a song about Khulnasoft"
        }
    ],
...
If you have a use case requiring one of these unsupported features, please [contact us via Slack](https://slack.khulnasoft.com/).
...
```
https://<Deployment UUID>.ifr.fr-par.khulnasoft.com/v1/embeddings
```

```
curl https://<Deployment UUID>.ifr.fr-par.khulnasoft.com/v1/embeddings \
...
In the context of a Khulnasoft Managed Inference deployment, it returns the name of the current model being served.

```
https://<Deployment UUID>.ifr.fr-par.khulnasoft.com/v1/models
```

```
curl https://<Deployment UUID>.ifr.fr-par.khulnasoft.com/v1/models \
  -H "Authorization: Bearer $SCW_API_KEY" \
  -H "Content-Type: application/json"
```
## Differences
...
Khulnasoft Managed Inference endpoints return usage stats for both streaming and non-streaming responses. 
...
If you have a use case requiring one of these unsupported APIs, please [contact us via Slack](https://slack.khulnasoft.com/).
