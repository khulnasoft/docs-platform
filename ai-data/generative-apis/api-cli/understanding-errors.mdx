...
Khulnasoft uses conventional HTTP response codes to indicate the success or failure of an API request. 
In general, codes in the 2xx range indicate success, codes in the 4xx range indicate an error caused by the information provided, and codes in the 5xx range show an error from Khulnasoft servers.
...
- 500 - **API error**: An unexpected internal error has occurred within Khulnasoft's systems. If the issue persists, please [open a support ticket](https://console.khulnasoft.com/support/tickets/create).
