...
All requests to the Khulnasoft Generative APIs must include an `Authorization` HTTP header with your API key prefixed by `Bearer`.
...
curl -X GET \
    ...
    "https://api.khulnasoft.ai/v1/models"
```
...
In contrast, when directly integrating with the Khulnasoft Generative APIs, you are responsible for manually setting the Authorization header with the API key for each request to ensure proper authentication.

## Content types

Khulnasoft Generative APIs accept JSON in request bodies and returns JSON in response bodies. 
You will want to send the `Content-Type: application/json` HTTP header in your POST requests.

```
curl --request POST \
     --url https://api.khulnasoft.ai/v1/chat/completions \
...
Permissions define the actions a user or an application can perform on Khulnasoft Generative APIs. They are managed using Khulnasoft’s [Identity and Access Management](/identity-and-access-management/iam/quickstart/) interface.
...
Querying AI models hosted by Khulnasoft Generative APIs will require any of the following [permission sets](/identity-and-access-management/iam/concepts/#permission-set):
...
1. Find your Project ID in your [Project settings](https://console.khulnasoft.com/project/settings)
2. Insert your Project ID in the Generative APIs service URL, for example:

```
https://api.khulnasoft.ai/78e655b5-feb0-417c-bb3f-8c448bd0e8da/v1
```
