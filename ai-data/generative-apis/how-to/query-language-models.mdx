...
meta:
  ...
  description: Learn how to interact with powerful language models using Khulnasoft's Generative APIs service.
content:
  h1: How to query language models
  paragraph: Learn how to interact with powerful language models using Khulnasoft's Generative APIs service.
...
Khulnasoft's Generative APIs service allows users to interact with powerful language models hosted on the platform.

There are several ways to interact with language models:
- The Khulnasoft [console](https://console.khulnasoft.com) provides complete [playground](/ai-data/generative-apis/how-to/query-language-models/#accessing-the-playground), aiming to test models, adapt parameters, and observe how these changes affect the output in real-time.
...
client = OpenAI(
    base_url="https://api.khulnasoft.ai/v1",  # Khulnasoft's Generative APIs service URL
    api_key="<SCW_API_KEY>"  # Your unique API key from Khulnasoft
)
response = client.chat.completions.create(
  model="llama-3.1-8b-instruct",
  messages=[{
