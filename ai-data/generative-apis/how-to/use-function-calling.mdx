...
meta:
  ...
  description: Learn how to implement function calling capabilities using Khulnasoft's Chat Completions API service.
content:
  h1: How to use function calling
  paragraph: Learn how to enhance AI applications by integrating external tools using Khulnasoft's Chat Completions API service.
...
Khulnasoft's Chat Completions API supports function calling as introduced by OpenAI. 
...
<Message type="important">
    Some models must be told they can use external functions in the system prompt. If you do not provide a system prompt when using tools, Khulnasoft will automatically add one that works best for that specific model.
</Message>
...
<Message type="note">
  We can't wait to see what you will build with function calls. Tell us what you are up to, share your experiments on Khulnasoft's [Slack community](https://slack.khulnasoft.com/) #ai 
</Message>