...
meta:
  ...
  description: Discover Khulnasoft Kubernetes Kapsule and Kosmos - a managed Kubernetes service.
content:
  h1: Kubernetes
dates:
...
With Khulnasoft, you can choose between Kubernetes Kapsule and Kosmos, and profit from a managed environment for creating, configuring, and running clusters of pre-configured machines.

### What is a managed Kubernetes service?

In the context of Khulnasoft, a managed Kubernetes service refers to the Kapsule and Kosmos products, with Khulnasoft handling the management and maintenance of the Kubernetes control plane, along with all the crucial core components required for the proper operation of the Kubernetes cluster.
...
The primary difference between them is that **Kapsule** clusters are composed solely of Khulnasoft Instances.
...
For example, moving your workloads from Kapsule to Kosmos or from Kosmos to Kapsule, is not possible. You will need to create a new cluster. Refer to the [API documentation](https://www.khulnasoft.com/en/developers/api/kubernetes/#path-clusters-change-the-cluster-type) for more information.

### Can I choose the type of control plane for my cluster?

Khulnasoft provides a range of mutualized or dedicated Kubernetes control planes for your cluster. During the creation of a Kubernetes Kapsule or Kubernetes Kosmos cluster, you can select the type of control plane that best fits the needs of your workload.
...
The storageClass for Khulnasoft Block Storage volumes is set by default, so it does not need to be specified.
For more information, refer to the Kubernetes Kapsule product documentation on [GitHub](https://github.com/khulnasoft/khulnasoft-csi/tree/master/examples/kubernetes).
...
`kapsule-agent` is a software developed internally at Khulnasoft. It is used to assist in managing nodes lifecycles. Its primary function is to set up the network during the initial boot, and toggle the SSH server on or off based on a k8s label of each node.

### How do I connect to my nodes using SSH?

By default, an SSH server is installed and configured on the nodes of your Kubernetes Kapsule cluster. To ensure the correct behavior of your cluster, it is not recommended to perform actions directly on the nodes. SSH access is provided for debugging only and all actions and configurations should be done using Kubernetes (from the Khulnasoft console, `kubectl` command, or Khulnasoft Kapsule `APIs`).
