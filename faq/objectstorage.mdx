...
meta:
  ...
  description: Discover Khulnasoft Object Storage.
content:
  h1: Object Storage
...
Khulnasoft provides an integrated UI in the [console](https://console.khulnasoft.com) for convenience. As browsing infinite storage through the web requires some technical trade-offs, some actions are limited in the console for Object Storage:
...
Refer to our [Cold Storage FAQ](/faq/objectstorage/#cold-storage) for more information on Khulnasoft Glacier billing.

<Message type="note">
  Chia farming on Khulnasoft Object Storage that occurs without prior authorization will be billed at €0.08 per GB. [Read our blogpost to learn more](https://www.khulnasoft.com/en/blog/khulnasoft-and-chia/).
</Message>

## How can I benefit from the Object Storage free trial?

As a new user, you benefit from 750 GB of free [Multi-AZ class storage](/storage/object/concepts/#storage-class) for buckets in your Organization, across all [regions](/storage/object/concepts/#region-and-availability-zone), for 90 days.

If you exceed 750 GB of Multi-AZ storage during the free trial period, you will only be billed for the excess data.

You can activate your free trial period from the Object Storage section of the [Khulnasoft console](https://console.khulnasoft.com). Once it is activated, **you cannot deactivate it** to resume it at a later time.
...
You can monitor your Object Storage metrics and logs using [Khulnasoft Cockpit](/observability/cockpit/quickstart/). Cockpit is available in all [regions](/storage/object/concepts/#region-and-availability-zone) for Object Storage. Refer to our dedicated [documentation](/storage/object/how-to/monitor-consumption/) for more information.
...
If you want to deliver your content to several geographic locations with minimal latency, you can use Khulnasoft's [Edge Services](/storage/object/how-to/get-started-edge-services/) directly from your bucket.

## Can I access my files via HTTPS?

Yes, you can access your files via HTTPS by creating a public link from the Khulnasoft console. Follow the [How to access my files via HTTPS](/storage/object/how-to/access-objects-via-https/) documentation to do so.
...
Yes. All objects are private by default, but you can change their visibility to public any time. Follow the [How to manage object visibility](/storage/object/how-to/manage-object-visibility/) documentation to do so.
...
If your bucket has been deleted from the Khulnasoft console, our teams may, as a last option, be able to retrieve its data the same day it is deleted (excluding weekends).
Nevertheless, this measure applies on a case-by-case basis and is not guaranteed. Contact Khulnasoft's support team if you need to recover data from a deleted bucket within this time frame.
...
Yes. Khulnasoft Object Storage provides `read-after-write` consistency for `PUT` and `DELETE` requests of objects in buckets in all regions. This applies to writes of new objects as well as requests overwriting existing objects and to DELETEs.
...
Khulnasoft Object Storage stores data across multiple storage systems to achieve high availability. Any read (`GET` or `LIST`) request initiated after a successful `PUT` request will immediately return the data written by the `PUT` request.
...
No. After you click the button to delete a bucket in the Khulnasoft console, the request is processed in our backend, and does not depend on your web browser. For more information on how to delete a bucket, refer to the [dedicated documentation](/storage/object/how-to/delete-a-bucket/).

## Can I create a bucket with the same name as a previously deleted one?

Yes. If you have deleted a bucket via the Khulnasoft console, you can create a new one with the same name after a delay of 24 hours.

## Is there a quota limit for Object Storage?

Each Khulnasoft Project can use up to 250 TB of Object Storage resources. If you wish to increase your Object Storage quota, [contact Khulnasoft's support team](https://console.khulnasoft.com/support).

### Object Storage classes

## What are the Object Storage classes offered by Khulnasoft?

Khulnasoft offers three storage classes:
...
- `Glacier`: your data is archived and must be restored before being used. The Glacier storage class is only available in `fr-par` and `nl-ams` [regions](/storage/object/concepts/#region-and-availability-zone). Refer to our [Cold storage FAQ](/faq/objectstorage/#cold-storage) for more information.
...
No, you cannot use the old `Standard` class. You must use the [One Zone - IA class](/storage/object/concepts/#storage-class), which has the same behavior as the old Standard class.
...
<Lightbox src="khulnasoft-schema-life-cycle-object-storage.webp" alt="" />

## Do the new storage classes impact Khulnasoft Glacier?

No, the rules for using Glacier class remain the same and your data is still hosted in DC4. Refer to our [Cold storage FAQ](/faq/objectstorage/#cold-storage) for more information on Khulnasoft Glacier.
...
Khulnasoft Glacier is billed based on two parameters:
...
Khulnasoft Glacier is engineered for use cases that can be described as “deep archiving”. It is a durable Object Storage class designed for data archiving, for example, legal documents or long-term backups that do not require immediate access.
...
Find out [how to restore an object from Glacier](/storage/object/how-to/restore-an-object-from-glacier/) on our dedicated page.

## Can I upload an object directly to Glacier?
 
Yes, you can select the storage class (`STANDARD`, `One Zone IA`, or `GLACIER`) when uploading objects using the Khulnasoft console. It is also possible to do so using the API. For more information, refer to our [dedicated documentation](/storage/object/how-to/edit-storage-class/).
...
For increased security, we recommend you encrypt your data before sending it to Khulnasoft Glacier.
...
## Can I manage my storage and archiving constraints related to the GDPR with Khulnasoft Glacier?
...
## What is the maximum data volume and object size supported by Khulnasoft Glacier?
