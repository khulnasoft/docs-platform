...
meta:
  ...
  description: Explore Khulnasoft Elastic Metal servers, pioneering 100% cloud bare metal solutions. Learn about billing, server ranges, installation guides, and more.
content:
  h1: Elastic Metal
...
To benefit from more advantageous monthly pricing, you will pay a one-time commitment fee equivalent to one month of subscription. In addition, if you rent an Elastic Metal server for more than 12 months, you will be rewarded with Khulnasoft credits equivalent to your commitment fee.
...
You are entitled to it if you have a monthly Elastic Metal server that is up and running for 12 consecutive months without termination. You have the right to one voucher per physical server, and it can be used for all Khulnasoft Cloud products, apart from Elastic Metal servers and their associated options.

The loyalty reward voucher is valid for six months and automatically added to your Khulnasoft account. [Learn more about how to get and use a loyalty reward](/bare-metal/elastic-metal/how-to/get-use-loyalty-reward/).
...
We provide a range of Linux and Windows Server distributions for automatic installation from the Khulnasoft console.
...
Each Elastic Metal dedicated server comes with an allocated bandwidth allowance. However, you can easily increase or decrease this bandwidth through your Khulnasoft console at any time.

Khulnasoft offers high-performance bandwidth options of up to 25Gbps per server, all of which include unlimited traffic.
For step-by-step instructions on how to modify the available bandwidth for your Elastic Metal server, refer to our documentation on [how to modify the available bandwidth for an Elastic Metal server](/bare-metal/elastic-metal/how-to/adjust-available-bandwidth/).
...
You can reinstall an Elastic Metal server with a few clicks at any time, [directly from your Khulnasoft console](/bare-metal/elastic-metal/how-to/install-server/).
...
Firmware updates are managed by Khulnasoft. If your Elastic Metal server requires a firmware upgrade, it will be installed automatically upon reboot of the server.
...
Licenses delivered with an Elastic Metal server are only valid on Khulnasoft's Elastic Metal servers, and cannot be recovered for external use.
