...
The `pgaudit` extension is available with Khulnasoft Managed Databases for PostgreSQL. The extension is a powerful monitoring and logging tool that allows you to keep track of the actions happening in your databases, and record them.
...
1. Click **PostgreSQL and MySQL** under **Managed Databases** on the side menu of the Khulnasoft console. A list of your Database Instances displays.
...
<Tabs id="pgauditlog">
 ...
 <TabsTab label="For a Database Instance">
   ...
   1. Go to the **Advanced settings** of your Database Instance in the Khulnasoft console.
...
3. Set the role you created as `pgaudit.role`
    ...
    ```
    <Message type="note">
      Alternatively, you can follow these steps to configure `pgaudit.role` from the Khulnasoft console:
...
- Audit logs are flushed and consume disk space. The [log ingestion rate](/observability/cockpit/reference-content/cockpit-limitations/#limits-for-loki) is **4 MB per second**.
...
2. Run the following command to deactivate the `pgaudit` extension in your Database Instance.
    ...
    ```
3. Go to the **Advanced settings** of your Database Instance in the Khulnasoft console.
4. Click <Icon name="edit" />.
5. Click **+ Add parameters**.
...
To set up session audit logging of all statements in all databases and roles,  set `pgaudit.log` to `ALL` in the advanced settings tab of your Database Instance dashboard in the Khulnasoft console.

### Session audit logging for only DML statements
To set up session audit logging only of Data Manipulation Language (DML) statements for all databases and roles, set `pgaudit.log` to `WRITE` in the advanced settings tab of your Database Instance dashboard in the Khulnasoft console.
...
1. Set `pgaudit.log` to `NONE` in the advanced settings tab of your Database Instance dashboard in the Khulnasoft console.
...
1. Set `pgaudit.log` to `FUNCTION,ROLE,DDL,MISC,MISC_SET`, and the `pgaudit.role` as `auditor` in the advanced settings tab of your Database Instance dashboard in the Khulnasoft console. You can replace `auditor` with the name of your principal audit role.
    <Message type="note">
