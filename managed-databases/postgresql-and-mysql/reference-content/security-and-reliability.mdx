...
| Standalone    | The Database Instance is provisioned on a single node. | 99.5%    |
  ...
  <Message type="note">
    The detailed SLAs measurements and guarantees can be found on the [Service Level Agreement for Managed Databases for PostgreSQL and MySQL](https://www.khulnasoft.com/en/database/sla/) page.
  </Message>
...
You can restore your backups and snapshots anytime. The restoration is triggered by you, and Khulnasoft then automatically executes the restore operation.
...
Creating, deleting, and promoting a Read Replica are actions triggered by you. Khulnasoft then automatically creates, deletes or promotes the Read Replica.
...
Khulnasoft creates your Database Instance with a default configuration fine-tuned to optimize database performance for standard workloads and monitor and solve issues caused by hardware, operating systems, or file system level failures.
...
Khulnasoft creates your Database Instance with a default configuration proven for standard use cases. Certain options are restricted to ensure reliability.
...
<Message type="note">
  Any intermediary Khulnasoft components, such as Load Balancers, do not decrypt Managed Databases' traffic.
</Message>
...
Most of these accesses are automatic actions, in cases where Khulnasoft needs to set up replication or upgrade minor versions.

Manual interventions might be required occasionally. All Khulnasoft accesses are authenticated and traced following industry security standards.
...
Moreover, these accesses are stored inside your Database Instance only and are independent of Khulnasoft IAM, which means you are responsible for ensuring they are properly backed up or removed when required.
