---
meta:
  title: Managed MongoDB® Databases Documentation
  description: Dive into Khulnasoft Managed MongoDB® Databases with our quickstart guides, how-tos, tutorials and more.
content:
  h1: Managed MongoDB® Databases Documentation
  paragraph: Dive into Khulnasoft Managed MongoDB® Databases with our quickstart guides, how-tos, tutorials and more.
---

<ProductHeader
   productName="Managed MongoDB® Databases"
   productLogo="documentDB"
   description="Managed MongoDB® Databases provide fully-managed databases with a MongoDB® compatible API, relying on MongoDB® and PostgreSQL as storage engines. Enjoy automatic backups, scaling, monitoring and much more."
   url="/managed-databases/mongodb/quickstart/"
   label="Managed MongoDB® Databases Quickstart"
/>

<Message type="important">
Managed MongoDB® is currently in private beta. Click [here](https://www.khulnasoft.com/en/managed-mongodb/) to join the waiting list.
</Message>

## Getting Started

<Grid>
    <SummaryCard
        title="Quickstart"
        icon="rocket"
        description="Learn how to create a Managed MongoDB® Database in a few steps."
        label="View Quickstart"
        url="/managed-databases/mongodb/quickstart/"
    />
    <SummaryCard
        title="Concepts"
        icon="info"
        description="Core concepts that give you a better understanding of Managed MongoDB® Databases."
        label="View Concepts"
        url="/managed-databases/mongodb/concepts/"
    />
    <SummaryCard
        title="How-Tos"
        icon="help-circle-outline"
        description="Learn how to use your Managed MongoDB® Database via the Khulnasoft console."
        label="View How-Tos"
        url="/managed-databases/mongodb/how-to/"
    />
    <SummaryCard
        title="Additional Content"
        icon="book-open-outline"
        description="Check out in-depth information about Managed MongoDB® Databases."
        label="View Additional Content"
        url="/managed-databases/mongodb/reference-content/"
    />
</Grid>


## API and CLI

<Grid>
    <DefaultCard
        title="Back up and restore MongoDB® Databases with MongoDB® tools"
        url="/managed-databases/mongodb/api-cli/backup-and-restore/"
        label="Read more"
    />
</Grid>

<ClickableBanner
    productLogo="cli"
    title="Managed MongoDB® Databases API"
    description="Manage your document Database Instances using the Khulnasoft API."
    url="https://www.khulnasoft.com/en/developers/api/document_db/"
    label="Go to Managed MongoDB® Databases API"
/>

## Changelog

<ChangelogList
    productName="mongodbs"
    numberOfChanges={3}
/>
