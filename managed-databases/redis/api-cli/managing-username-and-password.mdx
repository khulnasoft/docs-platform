...
- [Configured your environment variables](https://github.com/khulnasoft/khulnasoft-cli/blob/master/docs/commands/config.md) (optional)

## Specifying a name for a user

Follow the [create a Database Instance](https://www.khulnasoft.com/en/developers/api/managed-database-redis/#quickstart) procedure.

<Message type="tip">
You can also define a username when you [create a Redis™ Database Instance](/managed-databases/redis/how-to/create-a-database-for-redis/) via the Khulnasoft console.
</Message>
...
For a username to be valid, it must:
  ...
  - not start with `_rkv`
  - start with a letter
  - not be `khulnasoftadmin` or `metrics`
</Message>
...
<Tabs id="get database instance">
  <TabsTab label="API">
    ...
    'https://api.khulnasoft.com/redis/v1/zones/{zone}/clusters/{cluster_id}'
    ```
  </TabsTab>
