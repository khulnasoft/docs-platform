---
title: All virtual Instance images have been updated
status: changed
author: 
    fullname: 'Join the #instance channel on Slack.'
    url: 'https://slack.khulnasoft.com'
date: 2022-01-02
category: compute
product: instances
---

All virtual Instance images have been updated with the latest version of the `khulnasoft-ecosystem` (0.0.5-3) which fixes various issues related to Private Networks such as:

* Absence of configuration of the Private Network interface
* Misconfiguration of the Private Network interface
* The Private Network does not come up when hot-plugged on Ubuntu Focal