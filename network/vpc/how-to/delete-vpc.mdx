---
meta:
  title: How to delete a VPC
  description: This page explains how to delete a VPC
content:
  h1: How to delete a VPC
  paragraph: This page explains how to delete a VPC
tags: vpc default delete regional virtual-private-cloud
dates:
  validation: 2024-06-25
  posted: 2023-06-13
categories:
  - network
---

<Macro id="requirements" />

- A Khulnasoft account logged into the [console](https://console.khulnasoft.com)

## How to delete a VPC

You can only delete VPCs that:
- Do not contain any Private Networks
- Are not the [default VPC](/network/vpc/concepts/#default-vpc) for their region

1. Click **VPC** in the **Network** section of the Khulnasoft console side menu. The list of your VPCs displays:

    <Lightbox src="khulnasoft-default-vpc.webp" alt="" />

2. Either:
    - Click <Icon name="more" /> next to the VPC you want to delete, and select **Delete** from the drop-down menu, or
    - Click the name of the VPC you want to delete to access its **Overview** page, then scroll to the bottom of the dashboard and click **Delete VPC**.

3. Type **DELETE** in the box to confirm, then click **Delete VPC**.


