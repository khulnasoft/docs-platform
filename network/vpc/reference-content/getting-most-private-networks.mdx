...
meta:
  ...
  description: Optimize your Khulnasoft VPCs, Private Networks and resource connectivity with this guide which details tips and best practices.
content:
  h1: Getting the most from Private Networks
  paragraph: Optimize your Khulnasoft VPCs, Private Networks and resource connectivity with this guide which details tips and best practices.
...
This document sets out guidance, advice and best practices for building and optimizing your Khulnasoft VPCs and Private Networks.

## Private Networks and VPC definitions

VPC allows you to build your own **V**irtual **P**rivate **C**loud on top of Khulnasoft's shared public cloud. Within each VPC, you can create **Private Networks**. Resources attached to Private Networks can communicate securely between themselves, away from the public internet, reducing security risks by ensuring traffic is isolated from public routes.
...
VPCs and Private Networks are both **regional** resources. When you create a Private Network in a VPC, it is necessarily scoped to the same region as the VPC. Some Khulnasoft resources are also regional, others are zonal and scoped to a single Availability Zone (AZ). When attaching resources to a Private Network, you can attach resources from any AZ within the Private Network's region, allowing for example an Elastic Metal server in PAR-2 to communicate securely, away from the public internet, with an Instance in PAR-1 and a Managed Database in PAR-3.

<Lightbox src="khulnasoft-vpc-schema.webp" alt="A graphic shows how resources in three different AZs are connected and isolated from the internet by a Private Network in the Paris region." />
...
You can reserve private IP addresses from your Private Networks' CIDR blocks thanks to Khulnasoft's [**IP A**ddress **M**anagement solution](/network/ipam/), which helps you plan, track and manage the IP address space of your VPCs and their Private Networks. From the [IPAM space](https://console.khulnasoft.com/ipam/) in the Khulnasoft console, simply use the **Reserve private IP** feature to select the Private Network you want to reserve an IP address on, and choose to either reserve any available address, or a specific address not currently attached to any resource. The reserved address will then not risk being auto-assigned by Khulnasoft to other resources during network attachment, and can be kept until you are ready to use it to attach a specific resource.
...
Watch this space for Khulnasoft's upcoming solution to provide private, secure connectivity between resources in a Khulnasoft VPC and your external or on-premises architecture. In the meantime, you may consider installing a manual VPN on a Khulnasoft Instance to connect to other non-Khulnasoft infrastructure, and create a [custom route](/network/vpc/how-to/manage-routing/#how-to-create-a-custom-route) towards this VPN so traffic on your Private Network can securely communicate with resources at the other end of your VPN tunnel.

## Resource-specific information

Different types of Khulnasoft resources may have different requirements and possibilities in terms of Private Networks. See the comparative table below for more information, and to jump to the resource-specific documentation on Private Networks for each product.
...
| Additional information       | --       | [Paid-for <br />feature](https://www.khulnasoft.com/en/pricing/elastic-metal/#network) | PN cannot be changed <br />after cluster creation | Must have at least one of <br/> private and/or public endpoint |
