...
meta:
  ...
  description: Quickly set up Khulnasoft Edge Services for Object Storage buckets or Load Balancer origins with our guide. Learn to configure pipelines, customize endpoints, and manage caching for optimal performance.
content:
  h1: Edge Services - Quickstart
  paragraph: Quickly set up Khulnasoft Edge Services for Object Storage buckets or Load Balancer origins with our guide. Learn to configure pipelines, customize endpoints, and manage caching for optimal performance.
...
Edge Services is an additional feature for Khulnasoft Load Balancers and Object Storage buckets. It provides a [caching service](/network/edge-services/how-to/configure-cache/) to improve performance by reducing load on your [origin](#origin), and a customizable and secure [endpoint](#endpoint) for accessing content via Edge Services, which can be set to a subdomain of your choice.
...
<Lightbox src="khulnasoft-edge-services-pipeline.webp" alt="A diagram shows the elements and workflow of an Edge Services pipeline. The user connects to the customizable Edge Services endpoint (with its SSL/TLS certificate), which fetches content from the Edge Services cache, which itself fetches content to cache from an origin which is either an Object Storage bucket or Load Balancer" />
...
To use Edge Services, you must subscribe to a [pricing plan](https://www.khulnasoft.com/en/pricing/network/#edge-services). Within its monthly price, each pricing plan includes a fixed number of pipelines, and a certain amount of egress cache data. Any consumption that exceeds the limits of the plan is charged at an additional rate. 
...
1. Click **Edge Services** in the **Network** section of the [Khulnasoft console](https://console.khulnasoft.com/) side menu.

2. Click **Subscribe to Edge Services**. Information about available [pricing plans](https://www.khulnasoft.com/en/pricing/network/#edge-services) is displayed. 
...
3. Choose an **origin type**, the type of resource which Edge Services will connect to to fetch content to serve. This can either be **Load Balancer** or **Object Storage**. 
    ...
    <Tabs id="edge-services-quickstart">
      <TabsTab label="Load Balancer">
        ...
        <Lightbox src="khulnasoft-edge-create-pipeline-lb.webp" alt="A screenshot of the Khulnasoft console shows the Edge Services pipeline creation wizard. It prompts you to select an origin type (Load Balancer or Object Storage), and configure the other options described below on this page." />
      ...
      <Lightbox src="khulnasoft-edge-create-pipeline-os.webp" alt="A screenshot of the Khulnasoft console shows the Edge Services pipeline creation wizard. It prompts you to select an origin type (Load Balancer or Object Storage), origin bucket, and choose a name for the pipeline." />
...
1. In the Khulnasoft console, navigate to the Edge Services dashboard for the Object Storage bucket or Load Balancer pipeline whose domain you want to customize:

    <Lightbox src="khulnasoft-edge-services-dashboard.webp" alt="A screenshot of the Edge Services dashboard in the Khulnasoft console. This is a dashboard for Edge Services on an Object Storage bucket, showing links to the documentation, Khulnasoft Cockpit, the endpoint, cache settings and a button to disable the pipeline." />

2. In the **Endpoint** panel, click **Configure domain**. The following screen displays:

    <Lightbox src="khulnasoft-edge-configure-domain.webp" alt="A screenshot of the Configure Domain screen in the Edge Services part of the Khulnasoft console. Step one is to set a subdomain, step two is to provide an SSL/TLS certificate" />
...
4. This step depends on whether the domain used in the previous step is managed with [Khulnasoft Domains and DNS](/network/domains-and-dns/), or an external domain provider. Choose the appropriate tab below.

    <Tabs id="edge-cname">
    
        <TabsTab label="Domains and DNS">
        The domain you are using for Edge Services is considered to be managed with Khulnasoft Domains and DNS if:
        ...
        If either of the above is true, Khulnasoft will auto-detect that the domain is managed by Domains and DNS, and a message will display confirming that you do not need to create a CNAME record. We will auto-generate the appropriate CNAME record in your domain's [DNS records](/network/domains-and-dns/how-to/manage-dns-records/), to point your subdomain to the Edge Services endpoint. This record is generated when you click `Customize domain` in step 6.

        You should not attempt to modify or delete the CNAME record, which will be visible among your [DNS records](/network/domains-and-dns/how-to/manage-dns-records/) in the Khulnasoft console.
        </TabsTab>

        <TabsTab label="External provider">
        Khulnasoft cannot itself create the appropriate CNAME record to point your subdomain to Edge Services if your domain is managed by an external provider. You must create the CNAME record yourself.

        Log in to your domain provider, and locate the DNS settings for your domain. Create a new CNAME record pointing your subdomain to the Edge Services pipeline endpoint displayed in the Khulnasoft console. For help setting up CNAME records and troubleshooting any problems, [check out our dedicated documentation](/network/edge-services/reference-content/cname-record/).

        Back in the Khulnasoft console, click the `Verify CNAME` button to check whether your CNAME record has been correctly configured. Edge Services will carry out a check, and if it is successful the following message displays:

        <Lightbox src="khulnasoft-cname-success.webp" alt="Verification passed: Your DNS record has been correctly configured" />
...
5. Provide an SSL/TLS certificate for your subdomain so that Edge Services can serve traffic for it over HTTPS. You have three options for this:
    - Generate a free Let's Encrypt certificate, managed by Khulnasoft, including automatic renewals.
    - Select an existing certificate that you have stored in [Khulnasoft Secret Manager](/identity-and-access-management/secret-manager/quickstart/).
    - Manually import a certificate into Khulnasoft Secret Manager:
      ...
      Your certificate will be automatically stored in Secret Manager and [billed accordingly](https://www.khulnasoft.com/en/pricing/security-and-account/).
...
1. In the Khulnasoft console, navigate to the Edge Services dashboard for the Object Storage bucket or Load Balancer pipeline for which you want to enable caching:
...
2. In the **Cache** panel, use the <Icon name="toggle" /> icon to enable the cache.
    ...
    <Lightbox src="khulnasoft-edge-services-cache.webp" />
...
1. Click **Edge Services** in the **Network** section of the [Khulnasoft console](https://console.khulnasoft.com/) side menu. The **Pipelines** tab displays.
...
4. In the **Delete Edge Services pipeline** panel at the bottom of the screen, click **Delete Edge Services pipeline**.
    ...
    <Message type="tip">
    Remember to:
        - Delete any CNAME records created for this pipeline from your domain provider, unless your domain is managed with Khulnasoft Domains and DNS, in which case we take care of deletion for you.
        - Delete any SSL/TLS certificates you imported into Secret Manager for this pipeline (if no longer required elsewhere), so that you are no longer billed for them. If you generated a managed Let's Encrypt certificate however, Khulnasoft takes care of the deletion for you.
    </Message>
...
1. Click **Edge Services** in the **Network** section of the [Khulnasoft console](https://console.khulnasoft.com/) side menu. Your Edge Services dashboard displays.
