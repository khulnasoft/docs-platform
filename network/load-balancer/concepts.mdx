...
For more information about balancing rules, refer to our [blog post "What is a load balancer"](https://www.khulnasoft.com/en/blog/what-is-a-load-balancer/)
...
With the [customized error page feature](/network/load-balancer/how-to/set-up-s3-failover/), you can redirect users to a static website hosted on Khulnasoft Object Storage if none of your Load Balancer's backends are available to serve the requested content. This website could be a simple, single webpage or else something much more complex: you build it according to your own requirements.
...
Edge Services is an additional feature for Khulnasoft Load Balancers and Object Storage buckets.
...
Khulnasoft Load Balancers can use the **Host** header field to define [routes](#routes) from a frontend to a specified HTTP backend. HTTP headers can also be used in [ACLs](#acl) to **allow**, **deny** or **redirect** traffic based on the specified header field and value.

## HTTP/2 and HTTP/3

HTTP/2 and HTTP/3 are newer, improved versions of the "classic" HTTP/1 protocol. Khulnasoft Load Balancers support HTTP/2 connections to frontends, and HTTP/2 connections from backends to backend servers. In addition, Khulnasoft Load Balancers support HTTP/3 connections to frontends, but do not support HTTP/3 connections from backends to backend servers.

Read more about HTTP/2 and HTTP/3 and how they can be configured on Khulnasoft Load Balancers in our [dedicated documentation](/network/load-balancer/reference-content/http2-http3/).
...
When you create a `LoadBalancer` service within your [Khulnasoft Kubernetes cluster](/containers/kubernetes/quickstart/), the Khulnasoft Cloud Controller Manager will spin up an external Khulnasoft Load Balancer with the correct configuration to expose the specified service in your cluster.

The process for creating and managing Load Balancers for Kubernetes clusters is very specific - you **cannot** create or edit these via the Khulnasoft console or API. For full instructions on how to correctly create and configure a Khulnasoft Load Balancer for a Kubernetes cluster, see our [dedicated documentation](/containers/kubernetes/reference-content/kubernetes-load-balancer/).
...
Khulnasoft Load Balancers can use SNI to define [routes](#routes) from a frontend to a specified TCP backend.
