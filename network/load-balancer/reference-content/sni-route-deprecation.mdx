...
meta:
  ...
  description: Learn more about the deprecation of SNI routes with HTTP backends for Khulnasoft Load Balancers
content:
  h1: Deprecation of SNI routes with HTTP backends
  paragraph: Learn more about the deprecation of SNI routes with HTTP backends for Khulnasoft Load Balancers
...
This page documents the deprecation of SNI routes with HTTP backends on Khulnasoft Load Balancers

We removed the ability to create SNI based [routes](/network/load-balancer/how-to/create-manage-routes/) to HTTP backends in Khulnasoft Load Balancer. This feature was deprecated on March 1st 2023, and removed on June 1st 2023.
...
<Lightbox src="khulnasoft-sni-route.webp" alt="" />

**Backend lbb-king-volhard uses HTTP protocol, and supports HTTP Host header routes**:

<Lightbox src="khulnasoft-http-route.webp" alt="" />

## What was the impact?

There was no impact on your applications or services using Khulnasoft Load Balancer, since HTTP Host header routes provided more accurate routing than SNI with HTTP backends.
...
<Tabs id="sample-route-config">
  ...
  <TabsTab label="Terraform">
    Assuming you are managing a route resource to a backend using HTTP protocol with the following resource definition:
        ...
        ```bash
        resource "khulnasoft_lb_route" "rt01" {
          frontend_id = <your-frontend-id>
