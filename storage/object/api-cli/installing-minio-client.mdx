...
meta:
  ...
  description: Install and configure the MinIO client for Khulnasoft Object Storage.
content:
  h1: Installing and setting up MinIO Client
  paragraph: Install and configure the MinIO client for Khulnasoft Object Storage.
...
3. Follow the instructions in the [MinIO quickstart guide](https://min.io/docs/minio/linux/reference/minio-mc.html#create-an-alias-for-the-s3-compatible-service) to add a cloud storage service:
    ...
    ```
    For example, if your Khulnasoft Object Storage endpoint is in the `nl-ams` region, you can use the following command (replace the two fields in pointy brackets with your access key and secret key):
    ```
