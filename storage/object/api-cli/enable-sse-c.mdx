...
meta:
  ...
  description: Enable server-side encryption with customer-provided keys (SSE-C) in Khulnasoft.
content: 
  h1: Enabling server-side encryption (SSE-C)
  paragraph: Enable server-side encryption with customer-provided keys (SSE-C) in Khulnasoft.
...
**S**erver-**S**ide **E**ncryption with **C**ustomer-provided keys (SSE-C) is an encryption method provided by Khulnasoft Object Storage to protect your data at rest. It allows you to supply your own encryption keys to encrypt data when it is uploaded, and to decrypt data when accessed.

<Message type="note">
Currently, Khulnasoft does not support other types of server-side encryption methods, such as SSE-KMS, or SSE-S3.
</Message>
