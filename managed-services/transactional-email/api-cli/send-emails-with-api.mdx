...
This page shows you how to send a simple transactional email in `JSON` format to multiple recipients using the `additional_headers` parameter and the Khulnasoft Transactional Email API.
...
2. Run the following command to retrieve your domain's ID, as you will need it in the next step. The output should return your domain's information.
        ...
        "https://api.khulnasoft.com/transactional-email/v1alpha1/regions/fr-par/domains"
        ```
...
3. Run the following command to ensure that your domain is verified:
        ...
        ```
        curl -X GET "https://api.khulnasoft.com/transactional-email/v1alpha1/regions/$REGION/domains/<domain-id>" \
        -H "X-Auth-Token: $SCW_SECRET_KEY"
        ```
...
4. Copy the following template. Make sure that you replace the placeholder information with your own.
        ...
        cat > mail.json <<EOF
        {
            ...
            "project_id": "<Khulnasoft Project ID>", # Replace '<Khulnasoft Project ID>' with your Khulnasoft Project ID
        ...
        curl -X POST "https://api.khulnasoft.com/transactional-email/v1alpha1/regions/$REGION/emails" \
            -H "X-Auth-Token: $SCW_SECRET_KEY" \
            -d @mail.json
        ```
...
6. Run the following command to check that your email has been sent. Make sure that you replace `$EMAIL_ID` with the ID of the email you retrieved in the output of the previous step.
        ...
        ```
        curl --request GET \
        --url https://api.khulnasoft.com/transactional-email/v1alpha1/regions/fr-par/emails/$EMAIL_ID \
