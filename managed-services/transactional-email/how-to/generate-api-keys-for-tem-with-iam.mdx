...
<Message type="note">
 Khulnasoft supports the `PLAIN` and `LOGIN` authentication methods.
</Message>

The Transactional Email SMTP relay is available at `smtp.tem.khulnasoft.com` on ports `25`, `587`, `2587` (non TLS), `465` and `2465` (TLS). To send your emails using SMTP, a valid API secret key and your Project ID are required.
