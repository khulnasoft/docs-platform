---
meta:
  title: How to autoconfigure a Khulnasoft domain with Transactional Email
...
content:
  h1: How to autoconfigure a Khulnasoft domain with Transactional Email
...
With Khulnasoft domains, you can now benefit from automatic DNS record creation and updates (autoconfiguration) for domains already registered with the Transactional Email service.
...
You can enable autoconfiguration directly from the Khulnasoft console (see instructions below), through the [Transactional Email API](https://www.khulnasoft.com/en/developers/api/transactional-email/#path-domains-register-a-domain-in-a-project), or via [Terraform](https://github.com/khulnasoft/terraform-provider-khulnasoft/blob/master/docs/data-sources/tem_domain.md) using the `autoconfig` parameter (`bool`).
...
- A domain name registered with [Khulnasoft Domains and DNS](/network/domains-and-dns/)
