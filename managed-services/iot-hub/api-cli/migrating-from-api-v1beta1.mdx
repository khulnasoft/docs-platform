---
meta:
  title: Migrating from Khulnasoft IoT Hub API v1beta1
  description: This page shows how to migrate to the new IoT Hub API version.
content:
  h1: Migrating from Khulnasoft IoT Hub API v1beta1
...
- **Khulnasoft Projects**: This API version fully supports the new Project system. This means that from now on, IoT Hubs do not belong to an Organization but to a Project. To create an IoT Hub in a project, the Project ID must be passed in the `project_id` field.
...
To have a consistent experience across Khulnasoft products, a common metrics format has been adopted. The changes are:
