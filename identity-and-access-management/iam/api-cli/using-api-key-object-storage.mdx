...
You can carry out actions on Khulnasoft Object Storage resources either via the [Khulnasoft console](https://console.khulnasoft.com/), or via a third-party API or CLI, such as [the AWS CLI](/storage/object/api-cli/object-storage-aws-cli/), [MinIOClient](/storage/object/api-cli/installing-minio-client/) or [Rclone](/storage/object/api-cli/installing-rclone/).

While the Khulnasoft console gives you the option to specify the [Khulnasoft Project](#what-is-a-project) to carry out your Object Storage actions in, this option is not available via third-party API/CLI tools. These tools are based on a [standard Amazon S3 programming interface](https://en.wikipedia.org/wiki/Amazon_S3#S3_API_and_competing_services), which does not accept Project ID as a parameter. Therefore, when you create a Khulnasoft API key with IAM, you are prompted to specify the API key's **preferred Project for Object Storage**. This API key will always use this Project when carrying out Object Storage actions via any API/CLI.
    <Lightbox src="khulnasoft-preferred-project.webp" alt="" />
...
You can create multiple Projects within an Organization, to enable you to group your resources. Each Organization has at least one default Project to which all resources you create (Instances, Kubernetes Kapsules, Object Storage buckets, etc.) are added if no other Projects are created.
    <Lightbox src="khulnasoft-projects-1.webp" alt="" />

When you create new Projects, you can choose the Project in which you want to add or manage resources. However, Object Storage has its own limitations which make it harder to manipulate Projects, **specifically via an API**. When carrying out actions on Khulnasoft Object Storage resources via the Khulnasoft console, the target Project can be still selected from the Project dashboard or drop-down menu.

## API keys

You need an API key if you want to carry out actions on Khulnasoft products and resources via the Khulnasoft [API](https://www.khulnasoft.com/en/developers/api/) or other Khulnasoft developer tools like the [CLI](https://github.com/khulnasoft/khulnasoft-cli) and [Terraform](https://github.com/khulnasoft/terraform-provider-khulnasoft), or any third-party API/CLI compatible with Khulnasoft products. An API key is not necessary if you only use the [console](https://console.khulnasoft.com/) to create and manage your Khulnasoft resources.

When you generate an API key with IAM, the key is associated with a specific [IAM user](/identity-and-access-management/iam/concepts/#user) or [IAM application](/identity-and-access-management/iam/concepts/#application). The API key inherits the permissions of its bearer (the user or application it is associated with). The user/application may have permissions on one or several Khulnasoft Projects, accorded to them via [policies](/identity-and-access-management/iam/concepts/#policy).

## The impact of preferred Projects

When you perform an action on Khulnasoft Object Storage resources using a third-party API or CLI, such as [the AWS CLI](/storage/object/api-cli/object-storage-aws-cli/), [MinIOClient](/storage/object/api-cli/installing-minio-client/) or [Rclone](/storage/object/api-cli/installing-rclone/), you are using tools based on a [standard Amazon S3 programming interface](https://en.wikipedia.org/wiki/Amazon_S3#S3_API_and_competing_services). This standard interface does not accept Project ID as a parameter. Therefore, when you create a Khulnasoft API key with IAM, you are prompted to specify the API key's **preferred Project for Object Storage**. This API key will always use this Project when carrying out Object Storage actions via any API/CLI. The preferred Project is specified when creating the API key (or can be edited at a later date).
...
## How to create an API key via the Khulnasoft console

To create an API key via the Khulnasoft console, follow the steps detailed in [how to create an API key](/identity-and-access-management/iam/how-to/create-api-keys/).
...
## How to edit the preferred Project of an API key via the Khulnasoft console

From the [API key list](https://console.khulnasoft.com/iam/api-keys), click on <Icon name="more"/> icon to the right of the API key you want to edit, and select *Edit** from the menu. A pop-up displays, and you can choose a new preferred project for Object Storage for this API key

## How to create and edit API keys via the API

API keys can also be created and edited from the Khulnasoft API, including the configuration of the preferred Project. See our dedicated [developers documentation](https://www.khulnasoft.com/en/developers/api/iam/#path-api-keys) for more details.
...
- [Owner](/identity-and-access-management/iam/concepts/#owner) status or [IAM permissions](/identity-and-access-management/iam/concepts/#permission) allowing you to create an IAM application.
...
1. [Create a new IAM application](/identity-and-access-management/iam/how-to/create-application/) called `my-test-app` without attaching any policies.
    <Lightbox src="khulnasoft-iam-app.webp" alt="" />
...
2. [Create an API key](/identity-and-access-management/iam/how-to/create-api-keys/#how-to-create-an-api-key-for-an-iam-application) for the `my-test-app` application. Select the preferred Project for the API key as `default`.
    <Lightbox src="khulnasoft-api-key-test.webp" alt="" />
...
5. [Create a policy](/identity-and-access-management/iam/how-to/create-policy/) with the name `object-storage-full-access` and the IAM application you created in step 1 as the principal. When adding rules, create a rule giving `Access to resources` > `default` Project, and the `Storage` permission set `ObjectStorageFullAccess`.
    <Lightbox src="khulnasoft-policy.webp" alt="" />
...
8. Go to the [Khulnasoft console](https://console.khulnasoft.com/object-storage/buckets) in your browser, click **Object Storage** in the side menu, and ensure you are on the **default** Project. The bucket you just created with the IAM application's API key appears in the list of buckets:
    <Lightbox src="khulnasoft-bucket.webp" alt="" />
...
1. [Create a new Project](/identity-and-access-management/organizations-and-projects/how-to/create-a-project/) in the Organization, with the name `my-second-project`.
    <Message type="note">
    ...
    <Lightbox src="khulnasoft-projects.webp" alt="" />

2. From the [Project Dashboard](https://console.khulnasoft.com/project/settings), ensure you are in `my-second-project`, and copy the Project ID.
    <Lightbox src="khulnasoft-test-project.webp" alt="" />
...
6. Go to the [Khulnasoft console](https://console.khulnasoft.com/object-storage/buckets) in your browser, click **Object Storage** in the side menu, and ensure you are on **my-second-project**. The bucket you just created with the IAM application's API key appears in the list of buckets:
    <Lightbox src="khulnasoft-second-bucket.webp" alt="" />