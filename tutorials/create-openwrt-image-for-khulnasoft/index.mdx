---
meta:
  title: Creating an OpenWrt image for Khulnasoft Instances
  description: This page shows how to create an OpenWrt image for Khulnasoft Instances
content:
  h1: Creating an OpenWrt image for Khulnasoft Instances
  paragraph: This page shows how to create an OpenWrt image for Khulnasoft Instances
categories:
  - object-storage
  - instances
tags: openwrt qcow2
...
This tutorial shows how to build a custom image for Khulnasoft from scratch using the new export/import feature.
...
- Edit the image to fit Khulnasoft's ecosystem
- Upload the image to Khulnasoft Object Storage
...
In order to work on Khulnasoft Instances, the image must be using EFI (not just BIOS) to boot.
...
2. Convert the image and resize the target disk size (minimum disk size at Khulnasoft = 1 GB):
...
Network configuration and specific actions may need to be configured to access the image when run in Khulnasoft Instances.

For example:

* In Khulnasoft Instances, the first NIC (e.g. `eth0`) must be associated with the public network and need to be configured (e.g. with cloud-init, DHCP, or another method.)
...
2. Create the fetch script:
    ...
    start() {
        echo -e "\nFetching SSH keys from Khulnasoft Metadata" > /dev/console
...
1. Configure the network to be able to use the image on a Khulnasoft Instance:
    ...
    `eth0` is the first interface associated by Khulnasoft Instances to the public network (WAN) and needs to be available to our DHCP server for configuration.
...
You can use the Khulnasoft console or your favorite Amazon S3-compatible CLI tool to upload objects into a bucket.
...
2. Trigger the import of the image as a snapshot in one of the regions where the bucket is located, using [SCW CLI](https://github.com/khulnasoft/khulnasoft-cli):
...
<Message type="important">
Khulnasoft does not provide support for these kinds of images. Building and running of custom images is managed by the users themselves.
</Message>