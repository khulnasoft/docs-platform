...
- Automatically, using a bash script based on the Khulnasoft CLIv2. This could be a good option if you have multiple databases to migrate, and/or are concerned about errors during a manual restore.

## Manual migration via the Khulnasoft console and API

<Message type="important">
  This migration method is not suitable for Database Instances with extensions installed. In such cases, we recommend using the [automatic migration method with our script](#automatic-migration-via-a-bash-script-using-the-khulnasoft-cli-v2) or doing a manual dump/restore.
</Message>

There are three steps to completing a manual migration: creating a new PostgreSQL 13 instance, backing up your existing database using the Khulnasoft API, and finally restoring your backup on your new PostgreSQL 13 Instance. We walk you through these three steps below.
...
1. Open the [Khulnasoft console](https://console.khulnasoft.com) in a web browser and click **Managed Databases** in the **Storage** section of the side menu. The list of your databases displays. 
2. Click **+ Create a Database Instance**. The [Database Instance creation wizard](https://console.khulnasoft.com/rdb/instances/create) displays.
...
### Backing up your existing database using the Khulnasoft API

<Macro id="requirements" />

- Generated your [API key](/identity-and-access-management/iam/how-to/create-api-keys/)
- Installed `curl` on your local computer
...
1. Retrieve the database ID of your **old** Database Instance. You can find it on the **Database Instance Information** page of your Instance:
    <Lightbox src="khulnasoft-database-information.webp" alt="" />
2. Make a backup of your logical database(s) using the API:
    ```sh
    curl -X POST -H "Content-Type: application/json" \
    -H "X-Auth-Token: $SECRET_KEY" https://api.khulnasoft.com/rdb/v1/<region>/fr-par/backups -d '{
...
3. Replace the following information in the command above:
    * `$SECRET_KEY`: Your Khulnasoft API [secret key](/console/account/concepts/#secret-key)
...
2. Restore the backup of your logical database(s) using the API:
    ```sh
    curl -X POST -H "Content-Type: application/json" \
    -H "X-Auth-Token: $SECRET_KEY" https://api.khulnasoft.com/rdb/v1/regions/<region>/backups/<backup_id>/restore -d '{
    "database_name": "<my_database>",
...
3. The database appears in the **Managed Databases** tab of your new Database Instance, once the backup has been imported:
    <Lightbox src="khulnasoft-database-restored.webp" alt="" />
...
## Automatic migration via a bash script using the Khulnasoft CLI v2

This solution enables you to migrate your database(s) automatically, with a bash script using the Khulnasoft CLI v2. The script will walk you through the migration process.

<Macro id="requirements" />

- A PostgreSQL database running on version 9.6
- Installed the [Khulnasoft CLI v2](https://github.com/khulnasoft/khulnasoft-cli) on your machine and the relevant [PostgreSQL binaries](https://www.postgresql.org/download/) on your machine
...
1. Copy and paste the following script into a text file, and save it as `db_mig_script.sh`.
    ```bash
        ...
        # This script is designed to migrate a Khulnasoft RDB instance to a new PostgreSQL major version
        # It requires Khulnasoft CLI v2 as well as PostgreSQL binaries
        ...
        # Helper to get the Khulnasoft region to use
        get_region () {
            echo ""
            while true; do
                read -rp "Specify your Khulnasoft Zone. Default fr-par [fr-par/nl-ams/pl-waw] " region
                echo "$region"
                case $region in
        ...
        echo -e "Welcome to Khulnasoft RDB PostgreSQL Migration tool\nThis tool is designed to migrate your Databases from one major PostgreSQL version to another\n"
        echo -e "Please make sure, you got Khulnasoft V2 CLI (https://github.com/khulnasoft/khulnasoft-cli) as well as PostgreSQL binaries\n"

        scw version > /dev/null
        if [ $? -ne 0 ]; then
            log_error "The Khulnasoft CLI must be installed see https://github.com/khulnasoft/khulnasoft-cli"
            exit 1
        fi
