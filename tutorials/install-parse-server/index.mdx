...
meta:
  ...
  description: This page provides a step-by-step guide to installing a Parse server on a Khulnasoft Instance. Follow these detailed instructions to set up and configure your Parse server with ease.
content:
  h1: Installing Parse Server on Ubuntu 20.04
  paragraph: This page provides a step-by-step guide to installing a Parse server on a Khulnasoft Instance. Follow these detailed instructions to set up and configure your Parse server with ease. 
tags: Parse-Server Ubuntu MongoDB
categories:
  - instances
hero: assets/khulnasoft_parse.webp
dates:
...
1. Access the Parse server dashboard by visiting the URL `http://your-server-ip:4040`.
    <Lightbox src="khulnasoft-parse_dashboardlogin.webp" alt="" />
2. Add the credentials that you entered in the `parse-darshboard-config.json` file.
    <Lightbox src="khulnasoft-parse_dashboard.webp" alt="" />