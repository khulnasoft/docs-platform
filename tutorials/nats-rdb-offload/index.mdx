---
meta:
  title: Leverage NATS Messaging and Queuing pub/sub capabilities to offload your Managed Database
...
content:
  h1: Leverage NATS Messaging and Queuing pub/sub capabilities to offload your Managed Database
...
In this tutorial, we show how to set up a system that receives a message from a publisher and sends it to a database through a buffer subscriber. For this, we use Khulnasoft Serverless products and deploy two applications:

 - A publisher application, that sends a message to a NATS stream created with Khulnasoft Messaging and Queuing.
 - A subscriber application, that consumes messages published to the stream and then writes the data into a Khulnasoft Managed Database.

<Lightbox src="khulnasoft-nats-offload-archi.webp" alt="" />

We show how to provision all the required Khulnasoft resources using code examples and the Khulnasoft [console](https://console.khulnasoft.com/). The code for the functions is written in Python.
...
- [Created a Messaging and Queuing NATS account](/serverless/messaging/how-to/get-started/#how-to-create-a-nats-account)
...
When your NATS server is up and running, check you did not forget anything for the Khulnasoft [context](/serverless/messaging/api-cli/nats-cli/#define-a-context) by running the following code in a new window of your terminal:

    ```bash
    nats context info khulnasoft  
    ```
The output should mention "OK" for the credentials path, and confirm that your NATS server is ready.
If not, go back to the NATS CLI documentation to properly create a Khulnasoft context.
...
## Connect to your Khulnasoft Managed Database

1. Open a new terminal window.
2. Log in to the Khulnasoft console and access your Project.
...
2. Paste the following Python code into the file:
    ...
    Do not forget to replace the subscriber code with the correct configuration details for your Khulnasoft Managed Database information, and `NATS ACCOUNT URL` and `NATS CREDENTIALS PATH` with your own NATS account details.
...
While the volume of data processed in this example is quite small, thanks to the Messaging and Queuing NATS streams, your database will be able to process messages at an adapted pace.
