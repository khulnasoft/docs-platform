...
<Lightbox src="khulnasoft-schema-cockroach-db.webp" alt="" />
...
We configure `ntp` with the time servers of Khulnasoft and Ubuntu in this example:
...
3. Edit the file `/etc/ntp.conf` in your favorite text editor and remove any lines starting with `server` or `pool`. Then add the following content to the file:
    ```
    server ntp.int.khulnasoft.com iburst
