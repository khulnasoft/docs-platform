...
**Packer** is a tool for creating identical machine images for multiple platforms from a single source configuration file. It can build images for multiple cloud hosting platforms, including Khulnasoft.
...
To use Packer with Khulnasoft, you must [generate an API key](/identity-and-access-management/iam/how-to/create-api-keys/) from the management console.
...
- `access_key` (string) - The Khulnasoft API access key. It can also be specified via the environment variable `SCW_ACCESS_KEY`.

- `secret_key` (string) - The Khulnasoft API secret key. It can also be specified via the environment variable `SCW_SECRET_KEY`.

- `image` (string) - The UUID or marketplace image label of the base image to use. This will be used to boot the server on, before launching the additional tasks. A complete list of all base image UUIDs and image labels is available at [https://api-marketplace.khulnasoft.com/images](https://api-marketplace.khulnasoft.com/images).
...
See the rest of the parameters on the [Packer documentation](https://www.packer.io/docs/builders/khulnasoft).
...
3. Save the file and launch packer:
    ...
    ```
    packer build khulnasoft.json
    khulnasoft output will be in this color.

    ==> khulnasoft: Prevalidating image name: ubuntu-apache
    ==> khulnasoft: Prevalidating snapshot name: ubuntu-apache-snapshot
    ==> khulnasoft: Using existing SSH private key
    ==> khulnasoft: Creating server...
    ==> khulnasoft: Waiting for server to become active...
    ==> khulnasoft: Using ssh communicator to connect: 51.15.234.151
    ==> khulnasoft: Waiting for SSH to become available...
    ==> khulnasoft: Connected to SSH!
    ==> khulnasoft: Provisioning with shell script: /tmp/packer-shell591856108
    ==> khulnasoft: Shutting down server...
    ==> khulnasoft: Creating snapshot: ubuntu-apache-snapshot
    ==> khulnasoft: Creating image: ubuntu-apache
    ==> khulnasoft: Destroying server...
    Build 'khulnasoft' finished.

    ==> Builds finished. The artifacts of successful builds are:
    --> khulnasoft: An image was created: 'ubuntu-apache' (ID: e6e764cf-7f46-4c13-b251-a9e47367e284) in zone 'fr-par-1' based on snapshot 'ubuntu-apache-snapshot' (ID: 7abb7315-fae6-4317-9389-8ecf3c5e8a7b)
    ```

    The newly created image will appear in the image list in your account:

    <Lightbox src="khulnasoft-packer-image.webp" alt="" />
...
2. Set your credentials as an environment variable on your computer. For example in bash, you have to edit the file `~/.bashrc`, for zsh the procedure is the same, but the file to edit is `~/.zshrc`:
    ...
    export SCW_SECRET_KEY=<YOUR_SECRET_KEY>
    ```
3. Create a configuration file `khulnasoft.tf` with the following contents:
    ```hcl
    terraform {
      required_providers {
        khulnasoft = {
    ...
    resource "khulnasoft_instance_ip" "ip" {
    }

    data "khulnasoft_instance_image" "ubuntu-apache" {
      architecture = "x86_64"
      name         = "ubuntu-apache"
    }

    resource "khulnasoft_instance_server" "ubuntu-apache-server" {
      name  = "ubuntu-apache-server"
      ip_id = khulnasoft_instance_ip.ip.id
      image = data.khulnasoft_instance_image.ubuntu-apache.id
      type  = "DEV1-S"
      security_group_id = khulnasoft_instance_security_group.web.id
    }

    resource "khulnasoft_instance_security_group" "web" {
      name        = "http"
...
3. Initialize Terraform:
    ...
    Initializing provider plugins...
      ...
      - Installing khulnasoft/khulnasoft v1.17.0...
      - Installed khulnasoft/khulnasoft v1.17.0 (signed by a HashiCorp partner, key ID F5BF26CADF6F9614)
    ...
    in your configuration, with the constraint strings suggested below.
      ...
      * khulnasoft/khulnasoft: version = "~> 1.17.0"
...
5. Start your server from Terraform:
    ...
    khulnasoft_instance_ip.ip: Creating...
    khulnasoft_instance_volume.data: Creating...
    khulnasoft_instance_security_group.http: Creating...
    khulnasoft_instance_ip.ip: Creation complete after 1s [id=fr-par-1/278d1009-bd9f-4175-ab56-3ce6811ace72]
    khulnasoft_instance_server.ubuntu-apache-server: Creating...
    khulnasoft_instance_security_group.http: Creation complete after 4s [id=fr-par-1/065adee2-a74a-450f-a28c-bce3578c34a3]
    khulnasoft_instance_server.ubuntu-apache-server: Still creating... [10s elapsed]
    khulnasoft_instance_server.ubuntu-apache-server: Still creating... [20s elapsed]
    khulnasoft_instance_server.ubuntu-apache-server: Still creating... [30s elapsed]
    khulnasoft_instance_server.ubuntu-apache-server: Still creating... [40s elapsed]
    khulnasoft_instance_server.ubuntu-apache-server: Still creating... [50s elapsed]
    khulnasoft_instance_server.ubuntu-apache-server: Creation complete after 51s [id=fr-par-1/b3d9f0d3-a150-4d08-a505-9e5a4058958a]

    Apply complete! Resources: 4 added, 0 changed, 0 destroyed.
    ```

To find out more about all the resources that you can manage with Terraform, check out the [official documentation](https://registry.terraform.io/providers/khulnasoft/khulnasoft/latest/docs). For more information about Packer, refer to the official [Packer documentation](https://www.packer.io/docs/builders/khulnasoft.html).