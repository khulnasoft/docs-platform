...
content:
  h1: Automate periodic deletion of unused Container Registry images
  paragraph: Learn how to automate the deletion of unused images in your Container Registry to optimize storage efficiently.
categories:
  - container-registry
...
This tutorial will show you how to periodically remove older images with a specific tag using Serverless Functions, the Khulnasoft console, and Python's Urllib.
...
- Created a [Serverless Functions namespace](/containers/container-registry/how-to/create-namespace/)
...
8. Type the following code into the online code editor:
    ...
    def handle(event, context):
      ...
      while browse:
          url=f"https://api.khulnasoft.com/registry/v1/regions/{region}/images?page_size=100&order_by=created_at_asc&page="+str(i)
      ...
      for image in images_list:
          url=f"https://api.khulnasoft.com/registry/v1/regions/{region}/images/{image['id']}/tags?page_size=100&order_by=created_at_desc"
          req = urllib.request.Request(url, headers=headers)
          with urllib.request.urlopen(req) as response:
              res = response.read()
              tags=eval(res)["tags"]
      ...
      for tag in tags_to_delete:
          tag_id=tag['id']
          url=f"https://api.khulnasoft.com/registry/v1/regions/{region}/tags/{tag_id}"
...
11. Set your **scaling** preferences, or leave them at default values. The Khulnasoft platform autoscales the number of available Instances of your Function to match the incoming load, depending on the settings you define here.
