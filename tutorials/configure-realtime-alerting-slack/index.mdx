...
hero: assets/khulnasoft_iot_slack.webp
categories:
  - iot-hub
dates:
...
3. Once created, enable the application to send messages to your workspace's channels. Head to the `OAuth & Permissions` page, under `Scopes > Bot Token Scopes`, and add an OAuth scope named `chat:write`. It should look like the following.
    <Lightbox src="khulnasoft-iothub_slack_howto_scopes.webp" alt="" />
4. Finally, install the application in your workspace: scroll up the page and click the install button.
    <Lightbox src="khulnasoft-iothub_slack_howto_install.webp" alt="" />
5. Once authorized, the token used to publish messages from IoT Hub should appear. Write down the token, we will need it for the next step.
    <Lightbox src="khulnasoft-iothub_slack_howto_token.webp" alt="" />
...
Follow [this procedure](/managed-services/iot-hub/how-to/create-hub/) to create an IoT Hub. Select your name and plan of choice, and click the `Create Hub and Add a Device` button.
...
1. In the **Khulnasoft console**, head to the Routes tab of your IoT Hub.
2. Create a new route with the settings as follows:
    <Lightbox src="khulnasoft-iothub_slack_howto_route.webp" alt="" />
...
<Lightbox src="khulnasoft-iothub_slack_howto_message.webp" alt="" />
...
<Lightbox src="khulnasoft-iothub_slack_howto_goingfurther.webp" alt="" />
