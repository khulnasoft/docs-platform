...
meta:
  ...
  description: Discover how to send emails with Khulnasoft's Transactional Email and Serverless using the Serverless Framework
content:
  h1: Sending emails with Transactional Email and Serverless using the Serverless Framework
  paragraph: Discover how to send emails with Khulnasoft's Transactional Email and Serverless using the Serverless Framework
...
- [Configured your domain with Transactional Email and added SPF, DKIM, and MX records](/managed-services/transactional-email/quickstart/)
- [Installed the Khulnasoft CLI](https://github.com/khulnasoft/khulnasoft-cli) on your machine
...
2. Clone the GitHub repository containing the configuration files needed to deploy your function:
        ```
        git clone github.com:khulnasoft/serverless-examples.git
        ```
...
4. Paste the following command to install the Serverless plugin:
        ```
        serverless plugin install -n serverless-khulnasoft-functions
        ```
5. Copy and paste the following commands to export your environment variables. Make sure you add your **own variables**.
        ```
        export SCW_ACCESS_KEY=<Khulnasoft access key>
        export SCW_SECRET_KEY=<Khulnasoft secret key>
...
1. In the same terminal as the previous steps, paste the following command to trigger the deployment of your function:
        ...
        serverless deploy
        ```
2. Log in to your Khulnasoft account and click the Functions section in the left-side menu. Your `gomail` namespace displays.
