---
meta:
  title: Building a redundant load balancer and firewall infrastructure with pfSense and HAproxy on a Khulnasoft Dedibox
  description: This page explains how to install pfSense on a Khulnasoft Dedibox dedicated server
content:
  h1: Building a redundant load balancer and firewall infrastructure with pfSense and HAproxy on a Khulnasoft Dedibox
  paragraph: This page explains how to install pfSense on a Khulnasoft Dedibox dedicated server
categories:
  - elastic-metal
  - dedibox
  - load-balancer
...
This tutorial guides you through the installation of pfSense on a [Khulnasoft Dedibox](https://www.khulnasoft.com/en/dedibox/) server, the automatic configuration of failover IPs, and load balancing using HAProxy/Acme. It was written in collaboration with our community member [shannara](https://github.com/shannara).
...
- A [Khulnasoft Dedibox](https://www.khulnasoft.com/en/dedibox/) with KVM-over-IP access

## Installing pfSense

<Message type="important">
  The installation of pfSense is done using the [KVM-over-IP device](/dedibox/kvm-over-ip/quickstart/) of the Elastic Metal server or the Khulnasoft Dedibox.
</Message>

1. Connect to the [KVM-over-IP device](/dedibox/kvm-over-ip/quickstart/) of your Elastic Metal server or Khulnasoft Dedibox.
...
6. Choose the Keymap for your pfSense installation. By default, the `US` keymap is preselected. Select the desired keymap from the list and confirm by pressing **Enter**:
    <Lightbox src="khulnasoft-pfsense_install_03.webp" alt="" />
7. Choose the disk to install pfSense on. If your server has more than one disk, choose `ada0` as the installation destination. Press **Enter** to confirm.
    ...
    <Lightbox src="khulnasoft-pfsense_install_04.webp" alt="" />
8. Select the partition scheme for the volume. The default option is `MBR`. You can keep this value and confirm by pressing **Enter**:
    <Lightbox src="khulnasoft-pfsense_install_05.webp" alt="" />
9. The setup wizard starts copying the data from the virtual CD ROM drive to the server's hard disk. This may take a while depending on the connection speeds:
    <Lightbox src="khulnasoft-pfsense_install_06.webp" alt="" />
10. The automatic installation finishes. Choose **Yes** and confirm by pressing **Enter** to open a shell in the newly installed system to run a manual configuration:
    <Lightbox src="khulnasoft-pfsense_install_07.webp" alt="" />
...
12. In the configuration file, scroll down to the network configuration section:
    <Lightbox src="khulnasoft-pfsense_install_08.webp" alt="" />
...
<Lightbox src="khulnasoft-pfsense_welcome.webp" alt="" />
...
1. Open a web browser and point it to `https://<server_public_ip>`. The pfSense web interface displays. Log in using the user **admin** and the password **pfsense**:
    <Lightbox src="khulnasoft-pfsense_setup_01.webp" alt="" />
...
2. Once logged in, the pfSense setup wizard displays:
    <Lightbox src="khulnasoft-pfsense_setup_02.webp" alt="" />
...
4. Enter general information about the pfSense server. These include the **Hostname** and the **Domain** of the server. Leave **Primary DNS Server** and **Secondary DNS Server** empty and the option **Override DNS** checked. Click **Next** to continue:
    <Lightbox src="khulnasoft-pfsense_setup_04.webp" alt="" />
...
3. Go to **System** > **Advanced** and change the **TCP port** in the **WebConfigurator** settings to `31337` to match the previously configured rules.
    <Lightbox src="khulnasoft-pfsense_configure_tcp_port.webp" alt="" />
4. Save the settings by clicking on **Save** at the bottom of the page.

### Configuring the interfaces

Most [Khulnasoft Dedibox](https://www.khulnasoft.com/en/dedibox/) dedicated servers come with two physical network interfaces. In this configuration, the interface `igb0` is configured as a WAN (Internet) interface, and the interface `igb1` acts as a LAN (RPNv2) interface.
With [RPNv2](https://www.khulnasoft.com/en/dedibox/private-network-rpn/), it is possible to configure multiple private networks on the physical network interface. We suppose the [RPNv2 group](/dedibox-network/rpn/quickstart/#how-to-create-an-rpnv2-group) is already configured.
...
Two [failover IPs](/dedibox-console/account/how-to/outsource-failover-ip/) are being used as an endpoint for HAProxy (one by each Instance). To automatize the configuration in case of unavailability of one of the servers, the following workflow is needed:

- In case of a failure of `pf1`, `pf2` calls the Khulnasoft Dedibox API to switch the failover IP to `lb1`
- In case of a failure of `pf2`, `pf1` calls the Khulnasoft Dedibox API to switch the failover IP to `lb2`
...
With this, we delegate lifecycle management of the failover IP to CARP, by tweaking a bit of the pfSense CARP config to integrate the Khulnasoft Dedibox API.
...
10. Create the `carpcontrol.sh` script by clicking on **Diagnostics** > **Edit file** on the first firewall. **Enter** the path `/root/carpcontrol.sh in placeholder.sh` in the form, paste the content and click **Save**.
    ...
    > `API_TOKEN`: The Khulnasoft Dedibox API [private token](https://console.online.net/fr/api/access) > `PF1_IP`: Public IP of the first firewall
...
3. The configuration is replicated automatically on the second firewall.
    ...
    You have now configured a redundant load balancer and firewall infrastructure using pfSense and HAproxy on [Khulnasoft Dedibox dedicated servers](https://www.khulnasoft.com/en/dedibox/). To go further with the configuration of pfSense and HAproxy, refer to their official documentation:
