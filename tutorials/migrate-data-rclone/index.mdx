---
meta:
  title: Migrating Khulnasoft Object Storage data with Rclone
  description: This page shows you how to migrate Khulnasoft Object Storage data from one region to another with Rclone
content:
  h1: Migrating Khulnasoft Object Storage data with Rclone
  paragraph: This page shows you how to migrate Khulnasoft Object Storage data from one region to another with Rclone
categories:
  - instances
  - object-storage
...
- At least 2 [Object Storage buckets](/storage/object/how-to/create-a-bucket/)
...
3. Type `s3` and hit enter to confirm this storage type. The following output displays:
    ...
    13 / Any other S3 compatible provider
      \ "Other"
    ```
4. Type `Khulnasoft` and hit enter to confirm this S3 provider. The following output displays:
    ```
...
5. Type `false` and hit enter, to enter your credentials in the next step.
    ...
    access_key_id>
    ```
6. Enter your [API access key](/identity-and-access-management/organizations-and-projects/how-to/create-ssh-key/) and hit enter.
...
8. Enter your chosen region and hit enter. Here we choose `fr-par`.
    ...
    The following output displays:
    ```
    Endpoint for Khulnasoft Object Storage.
    Enter a string value. Press Enter for the default ("").
    Choose a number from below, or type in your own value
...
To transfer data to Khulnasoft Glacier class, add `--s3-storage-class=GLACIER` to your command, as such:

```
rclone copy --progress --s3-storage-class=GLACIER <SOURCE_BACKEND>:<SOURCE_PATH> <DEST_BACKEND>:<DEST_PATH>
```

You can verify the storage class of the transferred data by accessing your bucket on the Khulnasoft console.
...
```bash
# sync from the other Object Storage provider to Khulnasoft (nl-ams)
rclone --config rclone.conf sync other-provider://my-bucket/my-key-prefix scw-nl-ams://my-bucket/my-key-prefix

# sync from Khulnasoft (fr-par) to Khulnasoft (nl-ams)
