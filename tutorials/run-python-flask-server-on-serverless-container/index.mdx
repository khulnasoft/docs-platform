...
content:
  ...
  paragraph: Learn how to run a Python Flask Server on a Serverless Container with this comprehensive tutorial.
categories:
  - containers
...
4. Create a `server.py` text file, and add the following code to it. This creates a simple web app using Flask:
    ...
    DEFAULT_PORT = "8080"
    MESSAGE = "Hello, World from Khulnasoft Container!"
    ...
    if __name__ == "__main__":
        # Khulnasoft's system will inject a PORT environment variable on which your application should start the server.
...
3. [Deploy the image using the Khulnasoft console](/serverless/containers/how-to/deploy-container/).