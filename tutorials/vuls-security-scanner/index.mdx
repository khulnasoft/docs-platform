...
1. Connect to the server as `root` via [SSH](/compute/instances/how-to/connect-to-instance/).
...
4. Examine the results:
    ...
    <Lightbox src="khulnasoft-vuls-scan.webp" alt="" />
...
1. Connect to the remote server as `root` via [SSH](/compute/instances/how-to/create-an-instance/#-Logging-into-the-Instance).
...
6. Add the user to the sudoers file:
    ...
    adduser vuls sudo
    ```
7. Create a [SSH key](/identity-and-access-management/organizations-and-projects/how-to/create-ssh-key/) for on the Vuls master instance:
    ```
    ssh-keygen -o
    ```
...
2. In the APP parameters enable **Incoming Webhooks** and generate a new Webhook URL:
    <Lightbox src="khulnasoft-vuls_webhook.webp" alt="" />
...
4. Test the configuration by running the following command:
    ...
    <Lightbox src="khulnasoft-vuls_slack.webp" alt="" />
...
8. Open a web browser and point it to `http://YOUR_SERVER_IP:5111` to visualize the Vuls reports:
    <Lightbox src="khulnasoft-vuls_reports.webp" alt="" />
