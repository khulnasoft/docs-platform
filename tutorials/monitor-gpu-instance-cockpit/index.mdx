...
1. Create a [Cockpit token](/observability/cockpit/how-to/create-token/) from the [Khulnasoft console](https://console.khulnasoft.com/cockpit/tokens).
...
3. Copy and paste the following template inside `config.alloy`:
    ...
    ```
4. Replace the values of `cockpit.endpoint.url` (`https://example-afc6-4d02-a2fd-bc020bbaa7d0.metrics.cockpit.fr-par.scw.cloud/api/v1/push`) and `cockpit.endpoint.headers.X-TOKEN` (`example_bKNpXZZP6BSKiYzV8fiQL1yR_kP_VLB-h0tpYAkaNoVTHVm8q`) with the ones of your `gpu-instance-metrics`[Cockpit data source](https://console.khulnasoft.com/cockpit/dataSource).
...
1. Access the **Overview** tab of your [Cockpit](https://console.khulnasoft.com/cockpit/overview) and click **Open dashboards** to open your Cockpit dashboards in Grafana.
