...
2. Copy the following content in the file and set your access key and secret key. Then save the file and exit the text editor.
    ```
    [default]
    aws_access_key_id=KHULNASOFT_ACCESS_KEY
    aws_secret_access_key=KHULNASOFT_SECRET_KEY
    ```
...
2. Check the status of the backup using the `velero get backup` command:
    ...
    ```
    When you go to your Object Storage bucket in the [Khulnasoft console](https://console.khulnasoft.com/), you notice the `backup` folder created by Velero:
    <Lightbox src="khulnasoft-velero-backup-console.webp" alt="" />
    <Message typ="tip">
