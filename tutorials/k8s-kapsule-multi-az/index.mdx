...
Multi-AZ Kubernetes Kapsule is a configuration where a managed Kubernetes cluster is distributed across multiple isolated Availability Zones (AZs) within a Khulnasoft region, enhancing the resilience and availability of applications.
...
- A domain managed on the Khulnasoft [Domains and DNS service](/network/domains-and-dns/quickstart/)
- Installed [Terraform](/tutorials/terraform-quickstart/) on your local computer

<Message type="tip">
  You can download the complete [Terraform configuration files to deploy a multi-AZ cluster](https://github.com/khulnasoft/kapsule-terraform-multi-az-tutorial/) used in this tutorial from GitHub.
</Message>
...
<Lightbox src="khulnasoft-k8s-kapsule-multi-az-1-v2.webp" alt="" size="large" />
...
1. Create a `main.tf` file that will contain the Terraform part of this project, and set the required providers and their versions.
    <Message type="tip">
      You can download the complete [`main.tf` configuration file](https://github.com/khulnasoft/kapsule-terraform-multi-az-tutorial/blob/main/main.tf) used in this tutorial from GitHub.
    </Message>
    ```
    terraform {
        required_providers {
    ...
    ```
2. Add the Khulnasoft provider and its configuration. Refer to the Khulnasoft Terraform provider documentation for more information on how to [configure the provider](https://registry.terraform.io/providers/khulnasoft/khulnasoft/latest/docs).
    ```
    provider "khulnasoft" {
        region  = "fr-par"
    }
...
3. Create a dedicated VPC and a Private Network where we will deploy the Kapsule cluster. VPC and Private Networks are region-wide, so you can use them to host a cluster spread across multiple Availability Zones.
    ```
    resource "khulnasoft_vpc" "vpc_multi_az" {
        name = "vpc-multi-az"
        tags = ["multi-az"]
    }

    resource "khulnasoft_vpc_private_network" "pn_multi_az" {
        name   = "pn-multi-az"
        vpc_id = khulnasoft_vpc.vpc_multi_az.id
        tags   = ["multi-az"]
    }
    ```
...
4. Add the Kapsule cluster and the three pools, each in a different Availability Zone but in the same Private Network, to effectively make the cluster multi-AZ.
    Refer to the Khulnasoft Terraform provider documentation for available [Kapsule parameters](https://registry.terraform.io/providers/khulnasoft/khulnasoft/latest/docs/resources/k8s_cluster).
    - `khulnasoft_k8s_cluster` requires no specific configuration for the multi-AZ mode.
    - The [pools configuration](https://registry.terraform.io/providers/khulnasoft/khulnasoft/latest/docs/resources/k8s_pool) `khulnasoft_k8s_pool` will get a `zone` parameter that allows placing each one in a different Availability Zone.
    ```
    resource "khulnasoft_k8s_cluster" "kapsule_multi_az" {
        ...
        private_network_id = khulnasoft_vpc_private_network.pn_multi_az.id
    }

    output "kapsule" {
        description = "Kapsule cluster id"
        value = khulnasoft_k8s_cluster.kapsule_multi_az.id
    }

    resource "khulnasoft_k8s_pool" "pool-multi-az" {
        for_each = {
            "fr-par-1" = "PRO2-XXS",
            "fr-par-2" = "PRO2-XXS",
            "fr-par-3" = "PRO2-XXS"
        ...
        zone                   = each.key
        tags                   = ["multi-az"]
        cluster_id             = khulnasoft_k8s_cluster.kapsule_multi_az.id
...
* Healthy cluster
<Lightbox src="khulnasoft-k8s-kapsule-multi-az-2-v2.webp" alt="" size="large" />

* Loss of one AZ
<Lightbox src="khulnasoft-k8s-kapsule-multi-az-3-v2.webp" alt="" size="large" />

<Message type="note">
    For this section you need, as a prerequisite, a domain managed on the Khulnasoft "Domains and DNS" service.
</Message>

The domain used for this tutorial can be of type `internal` when bought with your Khulnasoft account, or of the `external` type if you want to re-use a domain bought with an external DNS registrar. If you choose the `external` method, you can follow the [How to add an external domain to DNS](/network/domains-and-dns/how-to/add-external-domain/) documentation. If you don't want to delegate all your domains to your Khulnasoft account, import your domain, but do not change the name servers in your registrar. Then you can create a "DNS zone" in Khulnasoft on your external domain, and declare two NS records in your registrar matching this zone. For example, with a domain "your-domain.tld" imported in Khulnasoft, you can create a DNS zone "scw", and delegate this zone to your registrar by adding those two records:
    ```
    scw 600 IN NS ns0.dom.scw.cloud.
    scw 600 IN NS ns1.dom.scw.cloud.
    ```
The zone "scw.your-domain.tld" is now managed in your Khulnasoft account.

1. Create a new file `ingress-nginx.tf` to store the deployment configuration of your ingress controller. The first step is to configure `kubectl` and `helm` providers to use the host and credentials generated by the `khulnasoft_k8s_cluster` resource when we created the Kapsule cluster.
    <Message type="tip">
      You can download the complete [`ingress-nginx.tf` configuration file](https://github.com/khulnasoft/kapsule-terraform-multi-az-tutorial/blob/main/main.tf) used in this tutorial from GitHub.
    </Message>
    ```
    provider "kubernetes" {
        host                   = khulnasoft_k8s_cluster.kapsule_multi_az.kubeconfig[0].host
        token                  = khulnasoft_k8s_cluster.kapsule_multi_az.kubeconfig[0].token
        cluster_ca_certificate = base64decode(khulnasoft_k8s_cluster.kapsule_multi_az.kubeconfig[0].cluster_ca_certificate)
    }
...
2. Use the `kubectl` command below to get the label list of a node. We will use label sets on nodes to configure the distribution of the pods on the cluster. You can see that multiple labels, set by the `khulnasoft-cloud-controller-manager`, contain information about the `region` and `zone` where the node is deployed. The label `topology.kubernetes.io/zone`, at the end of the list, is particularly interesting in your case as it will allow us to know on which availability zone a pod will be deployed.
    ```
    ...
    k8s.khulnasoft.com/kapsule=6ee0a129-ee3b-4a89-ad4b-a74754441746
    k8s.khulnasoft.com/managed=true
    k8s.khulnasoft.com/multi-az=
    k8s.khulnasoft.com/node=51c43255-4c42-449a-9e63-d734c357cd65
    k8s.khulnasoft.com/pool=2d669c67-bb46-4e4f-93a0-a59834f6c6ba
    k8s.khulnasoft.com/pool-name=pool-fr-par-1
    k8s.khulnasoft.com/runtime=containerd
    kubernetes.io/arch=amd64
    kubernetes.io/hostname=scw-kapsule-multi-az-pool-fr-par-1-51c432554c4
    kubernetes.io/os=linux
    node.kubernetes.io/instance-type=PRO2-XXS
    topology.csi.khulnasoft.com/zone=fr-par-1
...
Then, disable the default Kubernetes service creation by the Helm chart as it will be created through Terraform later on to have multiple `LoadBalancers`.
    ```
...
4. Add Kubernetes services of type `LoadBalancer` to expose your `ingress-nginx` pods. The important part here is that we will use the `service.beta.kubernetes.io/scw-loadbalancer-zone` annotation to create one load balancer per AZ. This annotation is used by the `khulnasoft-cloud-controller-manager` to create a load balancer in a specific AZ. In the `fr-par` region, load balancers are available in two AZs, so we will have two load balancers. Each load balancer dispatches traffic to the healthy nodes of all the AZs. This means, in case of the loss of one AZ where one of the two load balancers is located, the other load balancer will still be available and will dispatch traffic to the healthy nodes, and their healthy `ingress-nginx` pods, on the other AZs.
The selector matches the labels set by the Helm chart on the ingress-nginx pods, and the service exposes both HTTP and HTTPS ports. At the end of the resource, we tell Terraform to ignore some labels and annotations that the `khulnasoft-cloud-controller-manager` sets automatically, and that we do not want to override on the next Terraform runs.
    ```
    resource "kubernetes_service" "nginx" {
        ...
        lifecycle {
            ignore_changes = [
                ...
                metadata[0].labels["k8s.khulnasoft.com/cluster"],
                metadata[0].labels["k8s.khulnasoft.com/kapsule"],
                metadata[0].labels["k8s.khulnasoft.com/managed-by-khulnasoft-cloud-controller-manager"],
            ]
        }
    }
    ```

5. Add the creation of a DNS record to direct the external traffic to the available load balancers. We assume that you have an internal or external domain configured in your account with a `scw` zone, as described in the prerequisites part at the beginning of this section. In the code below, we use `your-domain.tld` as a placeholder, so you need to replace it with your domain name. If you want to use a bare domain, remove the subdomain parameter from the `khulnasoft_domain_zone` resource.
...
<Message type="important">
  The host used in the "url" must not match any ingress host defined in the cluster, otherwise, the "/healthz" endpoint will not work.
    ...
    ```
    data "khulnasoft_domain_zone" "multi_az" {
        domain    = "your-domain.tld"
        subdomain = "scw"
    }

    resource "khulnasoft_domain_record" "multi_az" {
        dns_zone = data.khulnasoft_domain_zone.multi_az.id
        name     = "ingress"
        type     = "A"
...
In this last section, you will deploy a multi-AZ Elasticsearch and a Kibana instance to query it. It will allow you to work with the stateful constraints imposed by the use of persistent volumes in a multi-AZ architecture. As a prerequisite, you need to have a default `scw-bssd` storage class with a `volumeBindingMode` set to `WaitForFirstConsumer`. You can check your cluster with the `kubectl` command below. Previous clusters were provisioned with the binding mode set to `Immediate`, so if you are in this case you need to update your cluster to the latest patch version (kubernetes >=1.24.17, >=1.25.13, >=1.26.8, >=1.27.5 or >=1.28.1). The storage class will be automatically updated to the new binding mode during the update.
    ...
    scw-bssd (default)   csi.khulnasoft.com   Delete          WaitForFirstConsumer   true                   131m
    scw-bssd-retain      csi.khulnasoft.com   Retain          WaitForFirstConsumer   true                   131m
    ```
...
1. Create a new Terraform file `elasticsearch.tf` for this section. Deploying a clustered application commonly implies a more complex deployment logic. For Elasticsearch and Kibana, this logic is managed by ECK, the Elasticsearch operator for Kubernetes. It will be in charge of creating Elasticsearch nodes and configuring them to make a cluster. It will also automatically configure Kibana to connect to the Elasticsearch cluster. So the first thing to do is to deploy the ECK operator using its chart with the Helm provider.
    <Message type="tip">
      You can download the complete [`elasticsearch.tf` configuration file](https://github.com/khulnasoft/kapsule-terraform-multi-az-tutorial/blob/main/main.tf) used in this tutorial from GitHub.
    </Message>
    ```
...
6. Create a DNS CNAME record for `kibana.scw.your-domain.tld`, pointing to the A record of your ingress-nginx controller deployed in the second section. Since this A record, `ingress.scw.your-domain.tld`, is configured with health checks, it will automatically resolve to the IPs of the available load balancers, so in case of the loss of one AZ, the DNS record will automatically resolve to the IP of the healthy load balancer and the traffic will be dispatched to the healthy nodes of the other AZs, maintaining the availability of Kibana.
    ```
    resource "khulnasoft_domain_record" "kibana" {
      dns_zone        = data.khulnasoft_domain_zone.multi_az.id
      name            = "kibana"
      type            = "CNAME"
