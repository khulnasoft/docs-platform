---
meta:
  title: Deploy a Next.js application using Khulnasoft Serverless Containers and Serverless SQL Databases
...
content:
  h1: Deploy a Next.js application using Khulnasoft Serverless Containers and Serverless SQL Databases
  paragraph: This page explains how to deploy your serverless Next.js application using Serverless Containers and Serverless SQL Database
categories:
  - sql-databases
tags: nextjs sql
dates:
...
You can either deploy your application:
  - [step by step using Khulnasoft CLI](#deploy-next.js-application-frontend-using-serverless-containers) to understand each detailed action performed, and the resources and subresources required
  - [using a Terraform template](#deploy-a-next.js-application-using-terraform-templates) to deploy your application faster and have a ready-to-use Infrastructure as Code.
...
- Installed and initialized the [Khulnasoft CLI](/developer-tools/khulnasoft-cli/quickstart/)
...
### Pushing the image to Khulnasoft Container Registry
...
### Deploying the container with Khulnasoft Serverless Containers
...
<Message type="tip">
If you want to avoid doing these modifications manually, you can also retrieve the final code from this tutorial from our [serverless-examples](https://github.com/khulnasoft/serverless-examples/tree/main/projects/tutorial-sdb-nextjs-terraform) repository with the command below:
```
git clone git@github.com:khulnasoft/serverless-examples.git
```
...
However, your application would then connect with your [user credentials](/identity-and-access-management/iam/concepts/#user), which is not a recommended security practice.
To secure your deployment, we will now add a dedicated [IAM application](/identity-and-access-management/iam/concepts/#application), give it the minimum required permissions, and provide its credentials to your application.
...
- Inspect your newly created resources in the Khulnasoft console:

  - You can display your **Registry namespace** and **container image** in the [Container Registry section](https://console.khulnasoft.com/registry/namespaces).

  - You can display your **Serverless Containers namespace** and **container deployment** in the [Serverless Containers section](https://console.khulnasoft.com/containers/namespaces).

  - You can display your **Serverless SQL Database** in the [Serverless SQL Databases section](https://console.khulnasoft.com/serverless-db/databases).
...
<Message type="note">
  You must have [Terraform](https://developer.hashicorp.com/terraform/tutorials/aws-get-started/install-cli) installed and configured with [Khulnasoft Terraform Provider](https://registry.terraform.io/providers/khulnasoft/khulnasoft/latest/docs) for this part.
</Message>
...
3. Run the following command to create a folder to store the template source code, and access it:
    ...
    && cd my-nextjs-blog
    ```
4. Copy the files from the [serverless-examples](https://github.com/khulnasoft/serverless-examples/tree/main/projects/tutorial-sdb-nextjs-terraform) repository to the folder.
      <Message type="note">
...
3. Add the following code to your `main.tf` file:
    ...
    resource khulnasoft_container main {
      ...
      environment_variables = {
          "PGUSER" = khulnasoft_iam_application.app.id,
          "PGHOST" = trimsuffix(trimprefix(regex(":\\/\\/.*:",khulnasoft_sdb_sql_database.database.endpoint), "://"),":")
          "PGDATABASE" = khulnasoft_sdb_sql_database.database.name,
          "PGPORT" = trimprefix(regex(":[0-9]{1,5}",khulnasoft_sdb_sql_database.database.endpoint), ":")
      }
      secret_environment_variables = {
        "PGPASSWORD" = khulnasoft_iam_api_key.api_key.secret_key,
      }
    }
      
    resource khulnasoft_iam_application "app" {
    name = "tutorial-nextjs-app-tf"
    }
      
    resource khulnasoft_iam_policy "db_access" {
    name = "tutorial-nextjs-policy-tf"
    description = "Gives tutorial Nextjs app access to Serverless SQL Database"
    application_id = khulnasoft_iam_application.app.id
    rule {
    ...
    resource khulnasoft_iam_api_key "api_key" {
    application_id = khulnasoft_iam_application.app.id
    }
      
    resource khulnasoft_sdb_sql_database "database" {
    ...
    value = format("postgres://%s:%s@%s",
      khulnasoft_iam_application.app.id,
      khulnasoft_iam_api_key.api_key.secret_key,
      trimprefix(khulnasoft_sdb_sql_database.database.endpoint, "postgres://"),
    )
    sensitive = true
    }
    ...
    // Output as an example, you can give this string to your application
    value = khulnasoft_container.main.domain_name
    sensitive = true
    }
    ```
...
- An [IAM API key](/identity-and-access-management/iam/concepts/#api-key) used as credentials to authenticate your application to the database
...
- **Change the security configuration** for your container. It is currently public and anyone with the link can access it, but you can make it [private to require authentication](https://www.khulnasoft.com/en/developers/api/serverless-containers/#authentication).
 
## Troubleshooting

If you happen to encounter any issues, first check that you meet all the requirements.

- You have installed and configured the [Khulnasoft CLI](/developer-tools/khulnasoft-cli/quickstart/). Running the `scw account project get` command should return the following output:
    ```
    ...
    ```
  <Message type="tip">  
  You can also find and compare your Project and organization ID in the [Khulnasoft console settings](https://console.khulnasoft.com/project/settings).
  </Message>
