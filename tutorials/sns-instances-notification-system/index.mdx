---
meta:
  title: Creating a simulated CPU monitor notification system with Terraform using Khulnasoft Instances and Messaging and Queuing SNS
  description: Learn how to set up a simulated CPU monitor notification system with Terraform using Khulnasoft Instances and Messaging and Queuing SNS. Trigger alerts based on CPU usage with ease. 
content:
  h1: Creating a simulated CPU monitor notification system with Terraform using Khulnasoft Instances and Messaging and Queuing SNS
  paragraph: Learn how to set up a simulated CPU monitor notification system with Terraform using Khulnasoft Instances and Messaging and Queuing SNS. Trigger alerts based on CPU usage with ease.
categories:
  - messaging
  - instances
...
This tutorial demonstrates how to set up a notification system, where a simulated CPU usage threshold triggers alerts which are sent to an HTTP server using Khulnasoft's serverless products.

It involves two Khulnasoft [Instances](https://www.khulnasoft.com/en/virtual-instances/):
...
You can find the code for the publisher and the subscriber servers on [GitHub](https://github.com/khulnasoft/serverless-examples/tree/main/mnq/sns-instances-notification-system).
...
<Macro id="requirements" />
  ...
  - [Set up the Khulnasoft CLI](/developer-tools/khulnasoft-cli/quickstart/)
...
4. Create the `main.tf` file still inside the `terraform` directory, and add the code below to set up the [Khulnasoft Terraform provider](https://registry.terraform.io/providers/khulnasoft/khulnasoft/latest/docs):
    ```
    terraform {
      required_providers {
        khulnasoft = {
...
1. Create the project `sns_tutorial` in your Organization dashboard:
    ```
    resource "khulnasoft_account_project" "sns_tutorial" {
      name = "sns-tutorial"
    }
    ```
...
2. Create the SSH key that will be used in the `sns_tutorial` Project to access the Instances:
    ...
    ```
    resource "khulnasoft_iam_ssh_key" "main" {
      name       = "sns-tutorial-public-ssh-key"
      project_id = khulnasoft_account_project.sns_tutorial.id
...
1. Activate SNS in your Project:
    ...
    ```
    resource "khulnasoft_mnq_sns" "main" {
      project_id = khulnasoft_account_project.sns_tutorial.id
      region     = "fr-par"
    }
    ```
...
2. Create the credentials for SNS:
    ```
    resource "khulnasoft_mnq_sns_credentials" "main" {
      project_id = khulnasoft_account_project.sns_tutorial.id
      permissions {
      ...
      depends_on = [
        khulnasoft_mnq_sns.main
      ]
    }
    ```
3. Finalize the SNS setup by creating the topic:
    ```
    resource "khulnasoft_mnq_sns_topic" "topic" {
      project_id = khulnasoft_account_project.sns_tutorial.id
      name       = "sns-tutorial-topic"
      access_key = khulnasoft_mnq_sns_credentials.main.access_key
      secret_key = khulnasoft_mnq_sns_credentials.main.secret_key
    }
    ```
...
1. Create the [security group](/compute/instances/concepts/#security-group) that will serve both Instances:
    ```
    resource "khulnasoft_instance_security_group" "sns_www" {
      project_id              = khulnasoft_account_project.sns_tutorial.id
...
2. Create the subscriber Instance's IP address:
    ```
    resource "khulnasoft_instance_ip" "subscriber_public_ip" {
      project_id = khulnasoft_account_project.sns_tutorial.id
...
3. Create the subscriber Instance itself, with all the tools it will need when it starts up:
    ```
    resource "khulnasoft_instance_server" "subscriber_sns_tuto_instance" {
      project_id        = khulnasoft_account_project.sns_tutorial.id
      name              = "suscriber-server"
      type              = "PLAY2-PICO"
      image             = "debian_bookworm"
      ip_id             = khulnasoft_instance_ip.subscriber_public_ip.id
      security_group_id = khulnasoft_instance_security_group.sns_www.id
...
4. Create a `terraform_data` resource to wait for port 8081 to be opened on the subscriber Instance before subscribing to the topic we previously created:
    ...
    resource "terraform_data" "bootstrap" {
      triggers_replace = [
        khulnasoft_instance_server.subscriber_sns_tuto_instance.id,
      ]
      ...
      provisioner "local-exec" {
        ...
        while [ $(($(date +%s) - $START_TIME)) -lt $TIMEOUT ]; do
          nc -z ${khulnasoft_instance_server.subscriber_sns_tuto_instance.public_ip} 8081 && exit 0
          sleep 1
        done
...
5. Create the SNS topic subscription with the subscriber Instance's endpoint:
    ```
    resource "khulnasoft_mnq_sns_topic_subscription" "main" {
      project_id = khulnasoft_account_project.sns_tutorial.id
      access_key = khulnasoft_mnq_sns_credentials.main.access_key
      secret_key = khulnasoft_mnq_sns_credentials.main.secret_key
      topic_id   = khulnasoft_mnq_sns_topic.topic.id
      protocol   = "http"
      endpoint   = "http://${khulnasoft_instance_ip.subscriber_public_ip.address}:8081/notifications"
    }
    ```

6. Create the publisher Instance's IP address:
    ```
    resource "khulnasoft_instance_ip" "publisher_public_ip" {
      project_id = khulnasoft_account_project.sns_tutorial.id
      zone       = "fr-par-1"
    }
    ```
...
7. Create the publisher Instance itself, with all the environment variables it will need to publish the notifications to the topic:
    ```
    resource "khulnasoft_instance_server" "publisher_sns_tuto_instance" {
      project_id        = khulnasoft_account_project.sns_tutorial.id
      name              = "publisher-server"
      type              = "PLAY2-PICO"
      image             = "debian_bookworm"
      ip_id             = khulnasoft_instance_ip.publisher_public_ip.id
      security_group_id = khulnasoft_instance_security_group.sns_www.id
      ...
      user_data = {
        ...
        write_files:
          - content: |
              #!/bin/bash
              export TOPIC_ARN="${khulnasoft_mnq_sns_topic.topic.arn}"
              export AWS_ACCESS_KEY="${khulnasoft_mnq_sns_credentials.main.access_key}"
              export AWS_SECRET_KEY="${khulnasoft_mnq_sns_credentials.main.secret_key}"
            owner: root:root
...
8. Check that your `main.tf` file looks like the fully completed example shown on the [GitHub repository](https://github.com/khulnasoft/serverless-examples/tree/main/mnq/sns-instances-notification-system).
...
2. From the [Instances section](https://console.khulnasoft.com/instance/servers) of the Khulnasoft console, access and copy the public IP address of the Instance we created via Terraform (identifiable from its name: `subscriber_server`).
...
6. Back in the Khulnasoft console, retrieve and copy the IP address of the Instance called `publisher_server`.
...
We have demonstrated a working infrastructure to test the sending and receiving of notifications relating to the publisher Instance's CPU usage. You can also create this setup by creating the necessary resources through the Khulnasoft console, API, or other developer tools.