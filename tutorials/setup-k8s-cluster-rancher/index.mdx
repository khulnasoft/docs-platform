...
- [Configured a domain name](/network/domains-and-dns/quickstart/) (i.e. `rancher.example.com`) pointing to the first Instance
...
3. To deploy Instances with [Docker](https://www.docker.com/) preinstalled, click **+ Create Instance**:
    <Lightbox src="khulnasoft-rancher_instances.webp" alt="" />
4. Click the **InstantApps** tab, and choose the **Docker** image:
    <Lightbox src="khulnasoft-rancher_instant_apps.webp" alt="" />
...
1. Once Rancher is installed, open a web browser and point it to your Rancher domain (i.e. `https://rancher.example.com`). The following page displays:
    <Lightbox src="khulnasoft-rancher_password.webp" alt="" />
...
3. The (empty) Rancher dashboard displays:
    <Lightbox src="khulnasoft-rancher_dashboard_empty.webp" alt="" />

### Creating a cluster

1. Click **Add cluster** to configure a new Kubernetes cluster.
2. The cluster creation page displays. Click **Custom** to deploy the cluster on the already launched Khulnasoft Instances.
    <Lightbox src="khulnasoft-rancher_add_cluster.webp" alt="" />
3. Enter a name for the cluster, choose the desired Kubernetes version and network provider, and select **None** as cloud provider.
    <Lightbox src="khulnasoft-rancher_cluster_options.webp" alt="" />
...
1. Head over **Global** in the header bar, select the cluster and click **Default** from the drop-down menu:
    <Lightbox src="khulnasoft-rancher_menu.webp" alt="" />
...
3. Enter the details of the workload:
    ...
    <Lightbox src="khulnasoft-deploy_workload.webp" alt="" />
...
4. Once deployed, open a web browser and point it to `http://<rancher.example.com>:30000/`. The Nginx demo application displays:
    <Lightbox src="khulnasoft-nginx_demo_app.webp" alt="" />
...
1. From the cluster dashboard, click **…**. Then, click **Edit** in the pop-up menu:
    <Lightbox src="khulnasoft-workload_edit.webp" alt="" />
2. Edit the Workload type and set the number of scalable deployments to **3**:
    <Lightbox src="khulnasoft-upgrade_service.webp" alt="" />
3. Click **Save**. Rancher will now send instructions to Kubernetes to update the workload configuration and to deploy 3 pods running the Nginx demo application in parallel.
    <Lightbox src="khulnasoft-workload_updated.webp" alt="" />
