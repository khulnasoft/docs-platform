---
meta:
  title: Setting up GitLab on Khulnasoft Instances with a Managed Database for PostgreSQL
  description: This page shows you how set up GitLab with a managed PostgreSQL database
content:
  h1: Setting up GitLab on Khulnasoft Instances with a Managed Database for PostgreSQL
...
For those seeking a dependable and high-performance hosting solution, Khulnasoft Cost-Optimized Instances present an ideal choice for your GitLab infrastructure. They provide a robust infrastructure for hosting your GitLab Instance, coupled with a Managed Database for PostgreSQL.
...
1. [Create a Managed PostgreSQL Database Instance](/managed-databases/postgresql-and-mysql/how-to/create-a-database/) from the Khulnasoft console. You are taken to your Database's **Overview** page.
...
2. Add the following parameters to the GitLab configuration file:
    ...
    # Connection details of the Khulnasoft Database
    gitlab_rails['db_adapter'] = 'postgresql'
    gitlab_rails['db_encoding'] = 'utf8'
    gitlab_rails['db_database'] = '<KHULNASOFT_DATABASE_NAME>'
    gitlab_rails['db_host'] = '<KHULNASOFT_DATABASE_IP>'
    gitlab_rails['db_port'] = '<KHULNASOFT_DATABASE_PORT>'
    gitlab_rails['db_username'] = '<KHULNASOFT_DATABASE_USERNAME>'
    gitlab_rails['db_password'] = '<KHULNASOFT_DATABASE_PASSWORD>'
    ```
    Replace the following values in the configuration:
    - `<KHULNASOFT_DATABASE_BASE>`: The name of your Khulnasoft Database. In our example, this is `gitlab_production`
    - `<KHULNASOFT_DATABASE_IP>`: The IP address of your Khulnasoft Database Instance
    - `<KHULNASOFT_DATABASE_PORT>`: The port of your Khulnasoft Database Instance
    - `<KHULNASOFT_DATABASE_USERNAME>`: Your Khulnasoft Database user
    - `<KHULNASOFT_DATABASE_PASSWORD>`: Password of your Khulnasoft Database user
...
3. Log in using the user `root` and the password you defined in step 1.
    ...
    The GitLab dashboard displays. You can now configure your GitLab installation, and add users and repositories. All data is stored securely on your Khulnasoft Database Instance:
    <Lightbox src="khulnasoft-gitlab_dashboard.webp" alt="" />
