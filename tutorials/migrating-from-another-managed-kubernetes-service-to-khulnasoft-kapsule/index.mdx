---
meta:
  title: Migrating from another managed Kubernetes service to Khulnasoft Kapsule
  description: Step-by-step guide to migrate your Kubernetes clusters from services like GKE, EKS, or AKS to Khulnasoft Kapsule with minimal downtime.
content:
  h1: Migrating from another managed Kubernetes service to Khulnasoft Kapsule
  paragraph: Step-by-step guide to migrate your Kubernetes clusters from services like GKE, EKS, or AKS to Khulnasoft Kapsule with minimal downtime.
...
This guide provides a step-by-step process to help you migrate your existing Kubernetes clusters from other managed services like GKE (Google Kubernetes Engine), EKS (Amazon Elastic Kubernetes Service), or AKS (Azure Kubernetes Service) to **[Khulnasoft Kapsule](/containers/kubernetes/quickstart/)**. The migration aims to minimize downtime and ensure a smooth transition of your workloads.
...
- Installed the following tools: `kubectl`, `helm` (if used), and `khulnasoft CLI` (optional but recommended).
- Access to your container images stored in a registry (Khulnasoft Container Registry, Docker Hub, ECR, GCR, etc.).

## Overview of migration steps

- [Step 1: Assess and document current cluster resources](#step-1-assess-and-document-current-cluster-resources)
- [Step 2: Set up Khulnasoft environment](#step-2-set-up-khulnasoft-environment)
- [Step 3: Migrate container images to Khulnasoft Container Registry](#step-3-migrate-container-images-to-khulnasoft-container-registry)
...
## Step 2: Set Up Khulnasoft environment

### 2.1 Create a Khulnasoft account

If you do not already have one, [sign up for a Khulnasoft account](https://console.khulnasoft.com/register/).

### 2.2 Install Khulnasoft CLI (Optional)

[Installing the Khulnasoft CLI](https://github.com/khulnasoft/khulnasoft-cli) can simplify some tasks.

Run the following command in a terminal to install the Khulnasoft CLI:
```sh
curl -s <https://raw.githubusercontent.com/khulnasoft/khulnasoft-cli/master/scripts/get.sh> | sh
scw init
```

Follow the prompts to configure your access keys.

## Step 3: Migrate container images to Khulnasoft Container Registry

Your new cluster will need access to your container images.

### 3.1 Set Up Khulnasoft Container Registry

#### Create a namespace

1. Log in to the [Khulnasoft Console](https://console.khulnasoft.com/).
...
<Message type="note">
  Refer to the dedicated documentation [How to create a namespace](/containers/container-registry/how-to/create-namespace/) for detailed information how to create a Khulnasoft Container Registry namespace.
</Message>

### 3.2 Authenticate Docker with Khulnasoft Registry

Use the following command to login to your Khulnasoft Registry using Docker:
...
<Message type="tip">
  Use your **Khulnasoft credentials** or generate a dedicated token.
</Message>

### 3.3 Pull images from existing registry and push to Khulnasoft
...
### 4.1 Using the Khulnasoft console
...
3. On the cluster configuration page, provide the following details:
    ...
    * Select **Kubernetes Kapsule** as the cluster type, which uses exclusively Khulnasoft Instances.
...
1. In the Khulnasoft console, go to your cluster's **Overview** page.
...
Your existing manifests may contain cloud-provider-specific settings that need adjustment to align with Khulnasoft Kapsule's configuration.

### 6.1 Review and modify manifests

#### Storage classes

- Update storage classes to match Khulnasoft's offerings.
- List available storage classes:
    ```sh
    kubectl get storageclass
    ```
- Common storage classes in Khulnasoft:
  - `scw-bssd` (Block Storage)
  - `scw-sbs` (Faster Block Storage)

#### Load Balancers

- Modify Service definitions of type `LoadBalancer` to use [Khulnasoft's Load Balancers](/network/load-balancer/quickstart/). Refer to our [annotations documentation](/containers/kubernetes/reference-content/using-load-balancer-annotations/).
...
- Ensure they align with Khulnasoft's network architecture.
...
- If you use external DNS, configure it to work with Khulnasoft's DNS or your DNS provider.
...
- Monitor CPU, memory, and storage usage in [Khulnasoft Cockpit](/observability/cockpit/quickstart/).
...
- **Resource Quotas**: Check for any limitations in Khulnasoft that may affect deployments.
...
- **Image Pull Errors**: Confirm that images are correctly tagged and accessible in [Khulnasoft Container Registry](/containers/container-registry/quickstart/).

## Additional considerations

### Leveraging Khulnasoft features
...
- **Transparent pricing**: Familiarize yourself with [Khulnasoft's pricing models](https://www.khulnasoft.com/en/pricing/containers/#kubernetes-kapsule).
- **Cost estimation tools**: Use Khulnasoft's tools or third-party services to estimate and monitor costs.
...
- **Khulnasoft Documentation**:
  ...
  - [Khulnasoft Container Registry](/containers/container-registry/)
...
- **Community and Support**:
  - [Khulnasoft Support](https://console.khulnasoft.com/support/tickets)
  - [Khulnasoft Community](https://slack.khulnasoft.com/)
...
- **Contact support**: Use the [Khulnasoft support portal](https://console.khulnasoft.com/support/tickets) for technical assistance.
- **Community Slack**: Engage with other users and experts in the [Khulnasoft Community](https://slack.khulnasoft.com).
