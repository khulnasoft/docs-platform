---
meta:
    title: Use Khulnasoft Managed Redis™ for MySQL caching with Entity Framework
    description: In this tutorial, you will learn to use Khulnasoft Redis™ as a record cache for Khulnasoft MySQL
content:
    h1: Use Khulnasoft Managed Redis™ for MySQL caching with Entity Framework
    paragraph: In this tutorial, you will learn to use Khulnasoft Redis™ as a record cache for Khulnasoft MySQL.
...
This tutorial demonstrates the integration of a [Khulnasoft Redis™](https://www.khulnasoft.com/en/managed-database-for-redistm/) Instance as a record cache (commonly referred to as a second-level cache) for an application using a [Khulnasoft MySQL](https://www.khulnasoft.com/fr/database/) Instance through [Entity Framework](https://learn.microsoft.com/en-us/ef/).
...
1. Create an empty project:
    ```
    dotnet new web -n KhulnasoftTutorial
    ```
...
1. Get the public IP and port of your Database Instance. You can locate this information on the **Database Instance Information** page of your Instance in the [Khulnasoft console](https://console.khulnasoft.com/rdb/instances), under **Public endpoint**.
    <Lightbox src="khulnasoft-mysql-db-information.webp" size="small" alt="Database Instance Information page in the Khulnasoft Console" />
...
2. Create a new user for your database, granting administrative rights from the **Users page**. Alternatively, use the default user.
    <Lightbox src="khulnasoft-mysql-user-information.webp" size="small" alt="User page in the Khulnasoft Console" />
...
4. During the application startup, establish the connection between Entity Framework and your Khulnasoft MySQL Instance. Begin by clearing the content of `Program.cs`, then insert the following snippet:
    ```csharp
...
1. Get the public IP, port, and username for your Redis™ Instance. Locate this information on the **Database Instance Information** page of your Instance in the [Khulnasoft console](https://console.khulnasoft.com/redis/clusters), under **Public endpoints**.
<Lightbox src="khulnasoft-redis-db-information.webp" size="small" alt="Redis Overview page in the Khulnasoft Console" />
...
4. During application startup, configure the EasyCaching library to connect to your Redis™ Database Instance, and configure it as the second-level cache for Entity Framework. Insert this snippet at the end of `Program.cs`:
    ...
    builder.Services.AddEasyCaching(options =>
    {
        ...
        options.UseRedis(config =>
        {
            ...
            var khulnasoftCert = new X509Certificate2("<your TLS certificate filename>", "");
            config.DBConfig.ConfigurationOptions.CertificateValidation +=
                (_, _, chain, _) => chain.ChainElements.Any(x => x.Certificate.Thumbprint == khulnasoftCert.Thumbprint);
        }, "khulnasoft-redis");
    });

    builder.Services.AddEFSecondLevelCache(options => options.UseEasyCachingCoreProvider("khulnasoft-redis"));
    ```
...
Your application is now fully configured to use the Redis™ Database Instance as a second-level cache for the Khulnasoft MySQL Instance. To test the setup, create a couple of API endpoints.
