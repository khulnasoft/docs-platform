---
meta:
  title: Backing up your dedicated server on Khulnasoft Object Storage with Duplicity
  description: Automatize your backups with Duplicity and Khulnasoft Object Storage
content:
  h1: Backing up your dedicated server on Khulnasoft Object Storage with Duplicity
  paragraph: Automatize your backups with Duplicity and Khulnasoft Object Storage
...
In this article, we will learn how to back up data from a Khulnasoft Instance, using [Khulnasoft's Object Storage](/storage/object/quickstart/) service and Duplicity. [Duplicity](https://duplicity.gitlab.io/) is a free and open-source tool that backs up folders to a remote server, in this case, an Object Storage bucket. Using libsync and GPG, Duplicity can make space-efficient encrypted backups.
...
- Create a bucket in Khulnasoft's Object Storage
- Install [Duplicity](https://duplicity.gitlab.io/)
- Generate a GPG key
- Combine Duplicity and Khulnasoft Object Storage

Although this tutorial focuses on backing up an Instance, you can also back up a dedicated server with Duplicity. When you are using a [Khulnasoft Dedibox](https://www.khulnasoft.com/fr/dedibox/) or a [Khulnasoft Elastic Metal server](https://www.khulnasoft.com/en/elastic-metal/), the data transfer is free of charge within the same region.
...
- An [Instance](/compute/instances/how-to/create-an-instance/) running Ubuntu or Debian 
...
1. Generate the GPG key with the following command.
    ...
    Real name: backups
    Email address: me@khulnasoft.com
    Comment: Khulnasoft Object Storage backups
    You selected this USER-ID:
      backups (Khulnasoft Object Storage backups) <me@khulnasoft.com>
    ...
    uid                      backups (Khulnasoft Object Storage backups) <me@khulnasoft.com>
...
2. Copy the GPG key fingerprint (its location in the output is shown above), which could be an 8, 16 or 40 char long hash. You can also find the fingerprint of your key with the following command:
    ...
    ------------------------------
      ...
      pub rsa3072 2020-03-26 [SC]
        XXXXXXXXXXXXX-FINGERPRINT-XXXXXXXXXXXXXX
      uid 		[ultimate] backups (Khulnasoft Object Storage backups) <me@khulnasoft.com>
      sub rsa3072 2020-03-26 [E]
      ```
...
## Combining Duplicity and Khulnasoft
...
1. Create our initial, empty script files and log files and make them executable.
    ...
    ```
2. Add the following lines to `.scw-configrc`. Make sure you replace the necessary values with the details of your Khulnasoft API key, Object Storage bucket, and GPG key. You also need to enter a path to the desired backup folder:
    ```bash
    # Khulnasoft credentials keys
    export AWS_ACCESS_KEY_ID="<KHULNASOFT ACCESS KEY>"
    export AWS_SECRET_ACCESS_KEY="<KHULNASOFT SECRET ACCESS KEY>"
...
1. Copy the following script to `scw-backups.sh`:
    ```bash
        ...
        if [ $currently_backuping -eq 0 ]; then
          ...
          # duplicity >= 0.8.23
          # determine S3_ENDPOINT_URL for khulnasoft
          S3_ENDPOINT_URL="https://s3.${S3_REGION_NAME}.scw.cloud"

          log ">>> creating and uploading backup to Khulnasoft Glacier"
          duplicity \
...
2. Run the script `./scw-backups.sh` to make sure the configuration is correctly set. Check the Object Storage bucket on the [Khulnasoft console](https://console.khulnasoft.com) to see the backup files, or examine the logs with the following command:
    ```bash
...
We created an automatic backup using Duplicity, hosting the encrypted data on Khulnasoft’s Object Storage. We are able to recover specific files or the entire backup itself at a given date.
