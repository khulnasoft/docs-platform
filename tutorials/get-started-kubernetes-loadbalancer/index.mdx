...
In our previous tutorials, we created a [Kubernetes Kapsule](https://www.khulnasoft.com/en/kubernetes-kapsule/) and deployed the containerized [whoami](https://github.com/jwilder/whoami) application to it. This is what our cluster looked like at the end of the last tutorial:
...
<Lightbox src="khulnasoft-nodeport.webp" alt="" />
...
<Lightbox src="khulnasoft-loadbalancer-service.webp" alt="" />
...
Then, the cluster’s Cloud Controller Manager (a component of the Kapsule’s control plane, managed by Khulnasoft) takes care of creating the external Khulnasoft Load Balancer and is responsible for all of its configuration and management. We can check the console though, to see that the Load Balancer has indeed been created.

We use `kubectl` to check the IP address of our Load Balancer, and then we are ready to test.

<Message type="note">

It is not possible to create the external Khulnasoft Load Balancer yourself via the console/API and then connect it to your cluster afterward. You must create the LoadBalancer Service in the cluster first, so that the Khulnasoft Cloud Controller Manager in your Kapsule’s control plane can handle the creation of the Khulnasoft Load Balancer, with the right configuration to direct its traffic to the correct pods of your cluster.
...
1. Open a browser and go to [console.khulnasoft.com](http://console.khulnasoft.com).
2. Select **Network** > **Load Balancers** from the side menu.
  A list of your Khulnasoft Load Balancers displays. You should see one with the tags `kapsule` and `cluster=xxxx`. This is the Load Balancer created by the Cloud Controller Manager of your Kubernetes Kapsule cluster when you created your LoadBalancer Service in the previous step.
3. Click the LoadBalancer to view its details.
  <Lightbox src="khulnasoft-load-balancer.webp" alt="" />
