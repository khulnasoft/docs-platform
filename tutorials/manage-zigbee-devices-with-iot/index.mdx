---
meta:
  title: Manage Zigbee connected devices with Khulnasoft IoT Hub
  description: In this tutorial you will learn how to set up and configure Zigbee2mqtt with Khulnasoft IoT Hub.
content:
  h1: Manage Zigbee devices with IoT Hub
  paragraph: In this tutorial you will learn how to set up and configure Zigbee2mqtt with Khulnasoft IoT Hub.
categories:
  - iot-hub
...
2. Add a new device to the Hub, with the option "Deny insecure connection" selected. You need to download <Icon name="download" /> the device certificate and device private key, as well as <Icon name="copy" /> the **Device ID**.
    <Lightbox src={["khulnasoft-configure-device.webp","khulnasoft-retrieve-device-parameters.webp"]} size={"medium"}  alt={["",""]} />
3. Download <Icon name="download" /> the certificate used to secure the MQTT endpoint in the **Networks** tabs.
    <Lightbox src="khulnasoft-download-hub-certificate.webp" size="medium" alt="" />
...
As soon as the configuration is done, you will see some metrics on the Khulnasoft console.
  <Lightbox src="khulnasoft-validate-configuration.webp" size="medium" alt="" />
Thanks to Khulnasoft built-in integration, similar metrics to monitor the performance of your IoT Hub are also available on your [Khulnasoft Observability](/observability/) dashboard.
  <Lightbox src="khulnasoft-metrics-observability.webp" size="medium" alt="" />