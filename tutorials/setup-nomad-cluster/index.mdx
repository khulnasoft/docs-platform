...
meta:
  ...
  description: This tutorial shows you how to deploy a Nomad cluster with access control lists (ACLs) on Khulnasoft.
content:
  h1: Setting up a Nomad cluster with ACLs
  paragraph: This tutorial shows you how to deploy a Nomad cluster with access control lists (ACLs) on Khulnasoft.
...
- Installed [Terraform 1.2.0 or a later version](https://developer.hashicorp.com/terraform/tutorials/aws-get-started/install-cli)
- Installed [Nomad 1.3.3 or a later version](https://developer.hashicorp.com/nomad/tutorials/get-started/gs-install)

## Clone the GitHub repository

We have created a [GitHub repository](https://github.com/khulnasoft/learn-nomad-cluster-setup.git) that contains all the code you need to deploy a Nomad cluster on Khulnasoft.

Clone the repository to your local machine using the following command:

```shell
git clone https://github.com/khulnasoft/learn-nomad-cluster-setup.git
```

Run the following command to access the `learn-nomad-cluster-setup/khulnasoft` directory:

```shell
cd learn-nomad-cluster-setup/khulnasoft
```

### Files and directories

In the `khulnasoft` directory, you will find the following files:
...
1. Open the `variables.hcl` file in your text editor of choice and update the following variables:
    ...
    - `project_id`: the ID of your [Project of choice](https://console.khulnasoft.com/project/settings).
...
2. Run the following command to build the Nomad Instance image:
    ...
    The following output should display once the build finishes:
      ...
      ```shell
      Build 'khulnasoft.hashistack' finished after 2 minutes 54 seconds.

      ==> Wait completed after 2 minutes 54 seconds

      ==> Builds finished. The artifacts of successful builds are:
      --> khulnasoft.hashistack: An image was created: 'hashistack-[random-number]' ...
      ```
...
# default project ID configured with the Khulnasoft CLI
...
# variable
# retry_join                    = "provider=khulnasoft tag_name=consul-auto-join"
