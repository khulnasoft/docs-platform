...
Chef is a configuration management tool that is written in Ruby and Erlang. It is capable of managing both your on-premise machines and your Khulnasoft resources with ease.
...
1. Connect to your remote machine via SSH. Follow the instructions depending on your remote machine type: [Instance](/compute/instances/how-to/connect-to-instance/), [Elastic Metal](/bare-metal/elastic-metal/how-to/connect-to-server/), or [Dedibox](/dedibox/dedicated-servers/how-to/connect-to-server/).
    <Message type="note">
...
4. Edit the configuration file to add a domain name or IP address where the server can be reached, following the example below:
    ...
    <Message type = "tip">
      You can find the IP address of your Instance or Elastic Metal server in its **Overview** tab in the [Khulnasoft console](https://console.khulnasoft.com/). For an Instance with host alias `scw-flamboyant-boyd`, IP address `192.158.1.38`, and no fully qualified domain name, the configuration file once edited might look like this:
...
2. Launch the `chef-server-ctl` with the `org-create` subcommand:
    ...
    For our example, we will create an organization with the following information:
      - **Short Name** (refers to the organization within Chef): khulnasoft
      - **Long Name** (name of the organization): Khulnasoft, Inc.
      - **Association User** (username that has access to administer the organization): testuser
      - **Filename:** khulnasoft-file.pem
    ...
    ```
    chef-server-ctl org-create khulnasoft "Khulnasoft, Inc." --association_user testuser -f khulnasoft-file.pem
    ```
...
5. Copy the `testuser.pem` and `khulnasoft-file.pem` files from the Chef server to the folder you created in your Chef workstation with the following commands:
    ```
    scp root@chef_server_domain_or_IP:/root/testuser.pem ~/chef_repo/.chef
    scp root@chef_server_domain_or_IP:/root/khulnasoft-file.pem ~/chef_repo/.chef
    ```
    ...
    scp username@chef_server_domain_or_IP:/home/username/khulnasoft-file.pem ~/chef_repo/.chef
    ```

#### Option 2: Downloading keys when connecting to a Chef server with passwords

<Message type="note">
  By default, you cannot connect to a Khulnasoft Instance or Elastic Metal server with a password. For more information, refer to the dedicated [Ubuntu Forum](https://askubuntu.com/questions/101670/how-can-i-allow-ssh-password-authentication-from-only-certain-ip-addresses)
</Message>
...
3. After adding a space, indicate the directory on the local computer where you wish the downloaded files to be placed (`~/chef_repo/.chef` in our case).
    ...
    scp username@chef_server_domain_or_IP:/home/username/khulnasoft-file.pem ~/chef_repo/
    ```
...
2. Paste the following information replacing the information with your infrastructure information:
    ...
    validation_client_name   "khulnasoft-validator"
    validation_key           "#{current_dir}/khulnasoft-file.pem"
    chef_server_url          "https://chef_server_domainname_or_IP/organizations/khulnasoft"
    syntax_check_cache_path  "#{ENV['HOME']}/.chef/syntaxcache"
    cookbook_path            ["#{current_dir}/../cookbooks"]
    ```
...
4. Test the configuration file. We need to be in our `~/chef-repo` directory for our configuration file to be read correctly:
    ...
    Original Exception: OpenSSL::SSL::SSLError: SSL Error connecting to https://51.15.240.56/organizations/khulnasoft/clients - SSL_connect returned=1 errno=0 state=error: certificate verify failed (self signed certificate)
    ```
