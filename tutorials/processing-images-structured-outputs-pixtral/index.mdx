...
- An API key from Khulnasoft [Identity and Access Management](/identity-and-access-management/iam/)
- Access to a Khulnasoft [Managed Inference](/ai-data/managed-inference/reference-content/pixtral-12b-2409/) endpoint with Pixtral deployed or to Khulnasoft [Generative APIs](/ai-data/generative-apis/quickstart/) service
...
BASE_URL = os.environ.get("KHULNASOFT_INFERENCE_ENDPOINT_URL")
# use https://api.khulnasoft.ai/v1 for Khulnasoft Generative APIs
...
Make sure to set the `KHULNASOFT_API_KEY` and `KHULNASOFT_INFERENCE_ENDPOINT_URL` environment variables with your actual API key from Khulnasoft IAM, and the appropriate endpoint URL for Khulnasoft Managed Inference or Generative APIs service.
...
1. Set the environment variables for your Khulnasoft API key and inference endpoint URL:
   ```
   export KHULNASOFT_API_KEY="your_api_key_here"
   export KHULNASOFT_INFERENCE_ENDPOINT_URL="your_endpoint_url_here"
   ```
