...
meta:
  ...
  description: This page shows how to create and manage Serverless Jobs on Khulnasoft using Dagster.
content:
  h1: Managing Serverless Jobs using Dagster
  paragraph: This page shows how to create and manage Serverless Jobs on Khulnasoft using Dagster.
...
First, we will initialize the Dagster project locally and add the scraper's source code to it. Then, we will set up the environment, build the image and push it to the Khulnasoft registry. Finally, we will execute Dagster locally to scrape the Hacker News top stories through the Dagster UI.
...
6. Copy the scraper code below and paste it into the `my-dagster-project/assets.py` file:
    ...
    client = khulnasoft.Client.from_config_file_and_env()
...
1. Retrieve the following elements:
    ...
    - the project ID of the [Khulnasoft project](/identity-and-access-management/organizations-and-projects/how-to/create-a-project/) you want to use
...
2. Export the environment variables below:
    ...
    export SCW_DEFAULT_PROJECT_ID=<your_khulnasoft_project_id>
    export SCW_ACCESS_KEY=<your_khulnasoft_access_key>
    export SCW_SECRET_KEY=<your_khulnasoft_secret_key>
    export S3_BUCKET_NAME=<your_bucket_name>
    ```

3. Build and push the image to your container registry namespace:
    ```bash
    docker build -t rg.fr-par.scw.cloud/<your-namespace>/dagster-khulnasoft-example:latest . &&
    docker push rg.fr-par.scw.cloud/<your-namespace>/dagster-khulnasoft-example:latest
    ```
    <Message type="note">
    You can find the name and endpoint of the container registry namespace you created in the [Khulnasoft console](https://console.khulnasoft.com/registry/namespaces)
    </Message>
4. Create a `dagster.yaml` file at the root of the Dagster project to store the configuration, and replace the container image with the one you pushed:
  ...
  ```yml
  run_launcher:
    module: dagster_khulnasoft
    class: KhulnasoftServerlessJobRunLauncher
    config:
      docker_image: rg.fr-par.scw.cloud/<your-namespace>/dagster-khulnasoft-example:latest
      env_vars:
      - PG_CONN_STRING
  ...
  storage:
    postgres:
      postgres_url:
        # Can be obtained from the Khulnasoft Console
        env: PG_CONN_STRING
  ```
...
1. Run the command below to launch the Dagster user interface locally:
    ...
    <Lightbox src="khulnasoft-dagster-ui.webp" size="medium" alt="" />
    <Message type="note">
...
2. In the Dagster UI, click **Materialize all** on the right to create the [Dagster assets](https://docs.dagster.io/concepts#software-defined-assets) and start your Serverless Job.
    ...
    <Lightbox src="khulnasoft-dagster-materialize.webp" alt="" />

3. Click **Runs** in the top menu, to display the list of past and current job runs.

    <Lightbox src="khulnasoft-dagster-run.webp" alt="" />
...
5. Once the job run is done, click **Show Markdown** in the **INFO** column to display a preview of the title, author, and URL of the top Hacker News articles.
    ...
    <Lightbox src="khulnasoft-dagster-preview.webp" alt="" />
...
- The source code is available in the [dagster-khulnasoft](https://github.com/khulnasoft/dagster-khulnasoft) repository.

- Refer to the [official Dagster documentation](https://docs.dagster.io/getting-started/what-why-dagster) for more information.

- You can automate your Dagster deployment using Terraform. Refer to our [Terraform Quickstart](/developer-tools/terraform/quickstart/) and to the [Khulnasoft Provider registry](https://registry.terraform.io/providers/khulnasoft/khulnasoft/latest/docs) for more information.
