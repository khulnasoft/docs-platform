---
meta:
   title: Implementing Retrieval-Augmented Generation (RAG) with LangChain and Khulnasoft Generative APIs
   description: Step by step Retrieval-Augmented Generation (RAG) with LangChain and Khulnasoft Generative APIs
content:
    h1: Implementing Retrieval-Augmented Generation (RAG) with LangChain and Khulnasoft Generative APIs
...
In this tutorial, you will learn how to implement RAG using LangChain, a leading framework for developing powerful language model applications. We will integrate LangChain with **Khulnasoft’s Generative APIs**, **Khulnasoft’s PostgreSQL Managed Database** (utilizing `pgvector` for vector storage), and **Khulnasoft’s Object Storage** to ensure seamless data management and efficient integration.

## What you will learn
- How to embed text using ***Khulnasoft Generative APIs***
...
Create a .env file and add the following variables. These will store your API keys, database connection details, and other configuration values.
   ...
   # Khulnasoft API credentials https://console.khulnasoft.com/iam/api-keys
   ## Will be used to authenticate to Khulnasoft Object Storage and Khulnasoft Generative APIs
   SCW_ACCESS_KEY=your_khulnasoft_access_key_id
   SCW_API_KEY=your_khulnasoft_secret_key
   ...
   # Khulnasoft Generative APIs endpoint
   ## LLM and Embedding model are served through this base URL
   SCW_GENERATIVE_APIs_ENDPOINT="https://api.khulnasoft.ai/v1"
   ```

## Setting Up Khulnasoft Managed Database
...
#### Key parameters:
- `openai_api_key`: This is your API key for accessing the OpenAI-powered embeddings service, in this case, hosted by Khulnasoft’s Generative APIs.
- `openai_api_base`: This is the base URL that points Khulnasoft Generative APIs where the embedding model is hosted. This URL serves as the entry point to make API calls for generating embeddings.
...
At this stage, you need to have proprietary data (e.g., PDF, CSV) stored in your Khulnasoft Object storage bucket.
...
- Create an Amazon S3 client: we establish an Amazon client to interact with the Khulnasoft Object Storage service.
