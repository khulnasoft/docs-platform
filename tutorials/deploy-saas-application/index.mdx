---
meta:
  title: Deploying a Software-as-a-Service application with Khulnasoft
...
content:
  h1: Deploying a Software-as-a-Service application with Khulnasoft
...
In this tutorial, you will learn how to deploy a Software-as-a-Service (SaaS) application, based on a SaaS application developed using Django Template. You use the following Khulnasoft products:

* [**Object Storage**](https://www.khulnasoft.com/en/object-storage/), to store your static files
* [**Database as-a-Service**](https://www.khulnasoft.com/en/database/), for your data storage
* [**Container Registry**](https://www.khulnasoft.com/en/container-registry/), to store your Docker images
* [**Kubernetes Kapsule**](https://www.khulnasoft.com/en/kubernetes-kapsule/), for your production environment

You will learn how to store environment variables with Kubernetes secrets and use ImagePullSecrets to pull your private images. We also give you some tips for going further with your deployed SaaS application, including how to expose it with a Khulnasoft Load Balancer.
...
- A [Khulnasoft Database](/managed-databases/postgresql-and-mysql/how-to/create-a-database/)
...
5. Now we can take a look at the variables we added in step 2. Since there are so many variable imbrications it may seem a little overwhelming. However, once the setup is complete you won't need to worry about them anymore:
    - `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY` are the [access key and secret key for your Khulnasoft account](/identity-and-access-management/iam/how-to/create-api-keys/)
    ...
    <Message type="note">
      Remember that Amazon S3 is a standard protocol. Even though the `boto3` library asks us to prefix variables with `AWS`, it nonetheless works perfectly with Khulnasoft Object Storage.
    </Message> 
...
As noted in the requirements for this tutorial, you should already have [created a private Khulnasoft Container Registry namespace](/containers/container-registry/how-to/create-namespace/). For this tutorial, we use a namespace called `my_registry_namespace`.
...
- Use Khulnasoft Load Balancer to expose your application in production.