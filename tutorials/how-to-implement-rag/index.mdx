---
meta:
   title: Implementing Retrieval-Augmented Generation (RAG) with LangChain and Khulnasoft Managed Inference
   description: Master Retrieval-Augmented Generation (RAG) with LangChain and Khulnasoft Managed Inference
content:
    h1: Implementing Retrieval-Augmented Generation (RAG) with LangChain and Khulnasoft Managed Inference
...
In this comprehensive guide, you will learn how to implement RAG using LangChain, one of the leading frameworks for developing robust language model applications. We will combine LangChain with ***Khulnasoft’s Managed Inference***, ***Khulnasoft’s PostgreSQL Managed Database*** (featuring pgvector for vector storage), and ***Khulnasoft’s Object Storage*** for seamless integration and efficient data management.

## Why LangChain?
LangChain simplifies the process of enhancing language models with retrieval capabilities, allowing developers to build scalable, intelligent applications that access external datasets effortlessly. By leveraging LangChain’s modular design and Khulnasoft’s cloud services, you can unlock the full potential of Retrieval-Augmented Generation.

## What you will learn
- How to embed text using a sentence transformer with ***Khulnasoft Manage Inference***
- How to store and query embeddings using ***Khulnasoft’s Managed PostgreSQL Database*** with pgvector
- How to manage large datasets efficiently with ***Khulnasoft Object Storage***
...
Create a .env file and add the following variables. These will store your API keys, database connection details, and other configuration values.
   ...
   # Khulnasoft API credentials
   SCW_ACCESS_KEY=your_khulnasoft_access_key
   SCW_API_KEY=your_khulnasoft_secret_ke
   SCW_REGION=your_khulnasoft_region

   # Khulnasoft managed database (PostgreSQL) credentials
   SCW_DB_NAME=your_khulnasoft_managed_db_name
   SCW_DB_USER=your_khulnasoft_managed_db_username
   SCW_DB_PASSWORD=your_khulnasoft_managed_db_password
   SCW_DB_HOST=your_khulnasoft_managed_db_host  # The IP address of your database instance
   SCW_DB_PORT=your_khulnasoft_managed_db_port  # The port number for your database instance

   # Khulnasoft Object Storage bucket configuration
   SCW_BUCKET_NAME=your_khulnasoft_bucket_name
   SCW_BUCKET_ENDPOINT="https://s3.{{SCW_REGION}}.scw.cloud" # Object Storage endpoint, e.g., https://s3.fr-par.scw.cloud

   # Khulnasoft Inference API configuration (Embeddings)
   SCW_INFERENCE_EMBEDDINGS_ENDPOINT="https://{{SCW_INFERENCE_EMBEDDINGS_DEPLOYMENT_ID}}.ifr.fr-par.khulnasoft.com/v1"  # Endpoint for sentence-transformers/sentence-t5-xxl deployment

   # Khulnasoft Inference API configuration (LLM deployment)
   SCW_INFERENCE_DEPLOYMENT_ENDPOINT="https://{{SCW_INFERENCE_DEPLOYMENT_ID}}.ifr.fr-par.khulnasoft.com/v1"  # Endpoint for your LLM deployment
   ```
...
#### Key parameters:
- `openai_api_key`: This is your API key for accessing the OpenAI-powered embeddings service, in this case, deployed via Khulnasoft’s Managed Inference.
- `openai_api_base`: This is the base URL that points to your deployment of the sentence-transformers/sentence-t5-xxl model on Khulnasoft's Managed Inference. This URL serves as the entry point to make API calls for generating embeddings.
...
In the context of using Khulnasoft’s Managed Inference and the `sentence-t5-xxl` model, TikToken tokenization is not necessary because the model you are using (sentence-transformers) works with raw text and handles its own tokenization internally.
Moreover, leaving `tiktoken_enabled` as `True` causes issues when sending data to Khulnasoft’s API because it results in tokenized vectors being sent instead of raw text. Since Khulnasoft's endpoint expects text and not pre-tokenized data, this mismatch can lead to errors or incorrect behavior.
By setting `tiktoken_enabled=False`, you ensure that raw text is sent to Khulnasoft's Managed Inference endpoint, which is what the sentence-transformers model expects to process. This guarantees that the embedding generation process works smoothly with Khulnasoft's infrastructure.
...
- Set up a Boto3 session: We initialize a Boto3 session, which is the AWS SDK for Python, fully compatible with Khulnasoft Object Storage. This session manages configuration, including credentials and settings, that Boto3 uses for API requests.
- Create an Amazon S3 client: We establish an Amazon S3 client to interact with the Khulnasoft Object Storage service.
- Set up pagination for listing objects: We prepare pagination to handle potentially large lists of objects efficiently.
- Iterate through the bucket: This initiates the pagination process, allowing us to list all objects within the specified Khulnasoft Object bucket seamlessly.
...
- S3FileLoader: The S3FileLoader loads each file individually from your ***Khulnasoft Object Storage bucket*** using the file's object_key (extracted from the file's metadata). It ensures that only the specific file is loaded from the bucket, minimizing the amount of data being retrieved at any given time.
