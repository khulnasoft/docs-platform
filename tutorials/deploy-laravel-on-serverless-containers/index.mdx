...
meta:
  ...
  description: This tutorial provides a step-by-step guide for deploying a containerized Laravel application on the Khulnasoft cloud platform.
content:
  h1: Deploying Laravel 10.x on Serverless Containers
  paragraph: This tutorial provides a step-by-step guide for deploying a containerized Laravel application on the Khulnasoft cloud platform.
tags: laravel php docker nginx fpm
hero: assets/khulnasoft-umami.webp
categories:
  - containers
  - container-registry
dates:
...
This tutorial provides a step-by-step guide for deploying a containerized Laravel application on the Khulnasoft cloud platform. It covers the entire process, from setting up the required infrastructure to building and deploying the application using Docker and Khulnasoft services. The tutorial aims to help developers easily deploy their Laravel applications on Khulnasoft by providing clear instructions and best practices.
...
## Why Khulnasoft is suitable for deploying containerized applications

Khulnasoft provides a seamless environment for running and managing containers, offering features such as [Container Registry](https://www.khulnasoft.com/en/container-registry/), [Serverless Containers](https://www.khulnasoft.com/en/serverless-containers/), [Messaging & Queuing](https://www.khulnasoft.com/en/messaging-and-queuing/) and [Observability](https://www.khulnasoft.com/en/cockpit/). By combining the benefits of containerization with Khulnasoft's platform, developers should be able to easily deploy, scale, and manage their applications efficiently.

<Message type="tip">
[Serverless Containers](https://www.khulnasoft.com/en/serverless-containers/) offers significant advantages when deploying Laravel applications over [Serverless Functions](https://www.khulnasoft.com/en/serverless-functions/), as it provides the flexibility to customize the runtime environment according to your specific needs.
</Message>

## Creating a SQS/SNS queue

Laravel applications make use of [queues](https://laravel.com/docs/10.x/queues) to process long-running jobs in the background. As this feature of the Laravel framework is nearly always used, we will configure it and leverage the [Messaging & Queuing](https://www.khulnasoft.com/en/messaging-and-queuing/) product from Khulnasoft. The [Khulnasoft documentation](/serverless/messaging/quickstart/) provides clear information on how this managed service works and can be configured.

1. Create a SQS/SNS namespace: we will create a `SQS/SNS` namespace in the `PAR` region.

    <Lightbox src="khulnasoft-serverless-messaging.webp" alt="Khulnasoft Console Messaging and Queuing Namespace" />
...
2. Create a queue. In this example, we create a `Standard` queue (At-least-once delivery, the order of messages is not preserved) with the default parameters. This queue will be the default queue used by our application.
    ...
    <Lightbox src="khulnasoft-serverless-messaging-queue.webp" alt="Khulnasoft Console Messaging and Queuing Queue" />

3. Generate credentials. In this example, we generate the credentials with `read` and `write` access.

    <Lightbox src="khulnasoft-serverless-messaging-credential.webp" alt="Khulnasoft Console Messaging and Queuing Credential" />
...
1. [Create a Khulnasoft Container Registry namespace](/containers/container-registry/how-to/create-namespace/) in the `PAR` region. Set the visibility to `Private` to avoid having your container retrieved without proper authentication and authorization.

    <Lightbox src="khulnasoft-serverless-containers-namespace.webp" alt="Khulnasoft Console Container Registry Namespace" />
...
2. Run the following command in your local terminal to log in to the newly created Container Registry.
    ...
    <Message type="important">
      To run this example, create an API key in the API keys tab of your Identity and Access Management dashboard. Then, replace the expression `$SCW_SECRET_KEY` with the secret key element of the API key. [Create an API key](https://console.khulnasoft.com/iam/api-keys)
    </Message>
...
The Khulnasoft documentation website provides a Quickstart on how to [create and manage a Serverless Container Namespace](/serverless/containers/quickstart/).
...
2. Deploy the application. Click **+ Deploy a Container** once the namespace is created, and follow the instructions of the creation wizard. Select the registry namespace and the previously uploaded Docker image and configure the listening port (the Nginx web server is listening on port 80). For the CPU and memory, define at least 560mVPCU and 256 MB respectively. To reduce the limitations due to [cold start](/serverless/containers/concepts/#cold-start), we will run at least 1 instance.
    ...
    <Lightbox src="khulnasoft-serverless-container.webp" alt="Khulnasoft Console Deploy Container" />
...
3. Enter the name of your endpoint. You can use any domain or subdomain that you already own. Click **Add Endpoint** to validate.
    ...
    <Lightbox src="khulnasoft-serverless-endpoint.webp" alt="Khulnasoft Console Container Endpoint" />


A new endpoint displays in the list of endpoints. You can now access the application using your custom endpoint name.

<Lightbox src="khulnasoft-serverless-laravel.webp" alt="Browsing to Laravel default home page" />

## Enabling monitoring and load testing (optionally)

By default, some metrics will be available in the Khulnasoft console. However, to better understand how your application behaves under load, you can use [Khulnasoft Cockpit](https://www.khulnasoft.com/en/cockpit/).
...
<Lightbox src="khulnasoft-cockpit-loadtesting.webp" alt="Khulnasoft Cockpit Serverless Container dashboard" />
...
<Lightbox src="khulnasoft-cockpit-queue.webp" alt="Khulnasoft Cockpit SQS dashboard" />