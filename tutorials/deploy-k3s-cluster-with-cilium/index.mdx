---
meta:
  title: Creating a Kubernetes cluster on Khulnasoft with K3s and Cilium
  description: This tutorial offers a detailed, step-by-step guide for setting up a Kubernetes cluster with K3s and Cilium on the Khulnasoft cloud platform.
content:
  h1: Creating a Kubernetes cluster on Khulnasoft with K3s and Cilium
  paragraph: This tutorial offers a detailed, step-by-step guide for setting up a Kubernetes cluster with K3s and Cilium on the Khulnasoft cloud platform.
...
- Installed and configured the [Khulnasoft CLI (v2)](https://github.com/khulnasoft/khulnasoft-cli)
...
## Setting up Khulnasoft infrastructure
...
To begin with, you need a Khulnasoft Instance running a Linux-based operating system. We recommend using Ubuntu 22.04.3 LTS (Jammy Jellyfish) for its compatibility with K3s and Cilium.
...
Follow these steps to create your Instance on Khulnasoft:
...
In this section, we'll guide you through the process of installing K3s on your Khulnasoft Instance and setting up a Kubernetes cluster.
...
3. Connect to the Khulnasoft Instance with `ssh root@[$main]`.
...
1. On the Khulnasoft `node1` Instance, create a `my-application.yaml` Kubernetes manifest file to specify the resources for deploying `traefik/whoami`, a Tiny Go webserver that prints OS information and HTTP request to output:
...
<Lightbox src="khulnasoft-validate-configuration.webp" size="medium" alt="" />

4. Optionally, you can use Cloudflare services to proxy incoming traffic, enabling IPv4 connectivity to your infrastructure. To achieve this, create an AAAA record pointing to the public IPv6 addresses of all your Khulnasoft Instances.

## Adding k3s node (optional)

1. Reiterate steps 3 to 7 of the **Setting Up Khulnasoft Infrastructure** section to create an additional Khulnasoft Instance named `agent-1`.
...
4. Run the installation script with the `K3S_URL` and `K3S_TOKEN` environment variables. Here is an example showing how to join an agent:
    ...
    The K3s agent will register with the K3s server listening at the supplied URL. The value to use for `K3S_TOKEN` is stored at `/var/lib/rancher/k3s/server/node-token` on your Khulnasoft `main` Instance.
...
7. Thanks to the K3s ServiceLB LoadBalancer controller, your application is also accessible via the public IP address of the new Khulnasoft Instance.

## Conclusion

In this tutorial, we created a highly efficient and cost-effective Kubernetes cluster on the Khulnasoft cloud platform. We started by laying the foundation, setting up an IPv6-only infrastructure, replacing the traditional kube-proxy with Cilium for enhanced security and performance.
...
If you prefer not to handle the management and configuration of a Kubernetes cluster yourself, you may want to explore [Khulnasoft Kubernetes Kapsule](https://www.khulnasoft.com/en/kubernetes-kapsule/), a fully managed Kubernetes solution.
