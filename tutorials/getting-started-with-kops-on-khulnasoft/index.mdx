---
meta:
  title: Getting started with kOps on Khulnasoft
  description: This page shows you how to get started with kOps on Khulnasoft.
content:
  h1: Getting started with kOps on Khulnasoft
  paragraph: This page shows you how to get started with kOps on Khulnasoft.
...
<Message type="important">
    Khulnasoft support on kOps is currently in **alpha**, which means that it is in the early stages of development and subject to change. Use it with caution.
</Message>
...
- An [Object Storage bucket](/storage/object/quickstart/) and its credentials

## Configuring environment variables

Before working on the tutorial, it is important to set the following [environment variables](https://github.com/khulnasoft/khulnasoft-sdk-go/blob/master/scw/README.md) on your local computer.
...
# Khulnasoft Object Storage is Amazon S3-compatible so we just override some configurations to talk to our bucket
export S3_REGION=fr-par                     # or another khulnasoft region providing Object Storage
...
# this is required since Khulnasoft support is currently in alpha so it is feature gated
export KOPS_FEATURE_FLAGS="Khulnasoft"
```
...
* Create a cluster in the `fr-par-1` Availability Zone, by running the following command:
    ```
    kops create cluster --cloud=khulnasoft --name=mycluster.k8s.local --zones=fr-par-1 --yes
    ```
    <Message type="note">
