...
- A Khulnasoft [Project](/identity-and-access-management/organizations-and-projects/quickstart/#project) dedicated to this tutorial
...
4. Copy and paste the following code in the editor:
    ...
    /* This is used to test locally and will not be executed on Khulnasoft Functions */
    if (process.env.NODE_ENV === 'test') {
      import("@khulnasoft/serverless-functions").then(scw_fnc_node => {
...
7. Run the following command to download the required dependencies and packages:
    ...
    ```bash
    npm i @aws-sdk/client-s3 @aws-sdk/client-sqs @khulnasoft/serverless-functions
    ```
...
1. Open the **Triggers** tab of your function.
2. Click **+ Create trigger**.
3. Select **SQS (Khulnasoft)** as a type.
...
<Message type="tip">
    Check out our [serverless-examples repository](https://github.com/khulnasoft/serverless-examples) for more examples and real-life use cases using Khulnasoft's Serverless products.
</Message>