---
meta:
  title: Set up a serverless architecture for handling large messages using Khulnasoft's NATS, Serverless Functions, and Object Storage.
  description: Learn how to build a serverless architecture for handling large messages with Khulnasoft's NATS, Serverless Functions, and Object Storage. Follow our step-by-step Terraform-based tutorial for asynchronous file conversion using messaging, functions, and triggers.
content:
  h1: Create a serverless architecture for handling large messages using Khulnasoft's NATS, Serverless Functions, and Object Storage.
  paragraph: Learn how to build a serverless architecture for handling large messages with Khulnasoft's NATS, Serverless Functions, and Object Storage. Follow our step-by-step Terraform-based tutorial for asynchronous file conversion using messaging, functions, and triggers.
categories:
  - messaging
  - functions
  - object-storage
...
In this tutorial, we will build a simple architecture to store and automatically convert images to PDF. The focus is on transferring large messages using a messaging service, specifically the [Khulnasoft Messaging and Queuing](/serverless/messaging/quickstart/) NATS server. Our setup involves using the [Object Storage](/storage/object/quickstart/) service for image storage and the [Serverless Functions](/serverless/functions/quickstart/) service for conversion.

We show how to provision all the required Khulnasoft resources via Terraform, but you can also use the [console](https://console.khulnasoft.com/login), the [API](https://www.khulnasoft.com/en/developers/api/) or one of our other supported [developer tools](https://www.khulnasoft.com/en/developers/).
...
- [AWS CLI configured with Khulnasoft credentials](/storage/object/api-cli/object-storage-aws-cli/#how-to-install-the-aws-cli)

## Architecture

<Lightbox src="khulnasoft-large-messages-architecture.webp" alt="" />
...
1. Create a `main.tf` file, which we will use to provision all our Khulnasoft resources and infrastructure. Paste the following code to import the necessary providers.
    <Message type="tip">
    ...
    ```
    The Khulnasoft provider is needed, but also three providers from HashiCorp that we will use later in the tutorial.

2. Include two variables to enable the secure passage of your Khulnasoft credentials. Then initialize the Khulnasoft provider in the `fr-par-1` region.
    ```hcl
...
3. Create a `terraform.tfvars` file and add your Khulnasoft access key and access key ID to it.
    ```terraform
...
4. Continuing in the `main.tf` file, add the following Terraform code to create an Object Storage bucket that will be used for storing your images.
    ...
    resource "khulnasoft_object_bucket" "large_messages" {
      name = "large-messages-${random_id.bucket.hex}"
    }

    resource "khulnasoft_object_bucket_acl" "large_messages" {
      bucket = khulnasoft_object_bucket.large_messages.id
      acl    = "private"
    }

    output "bucket_name" {
      value       = khulnasoft_object_bucket.large_messages.name
      description = "Bucket name to use with the producer script"
    }
    ```
    In this code, the resource `random_id.bucket` generates a random ID, which is then passed to the object bucket to ensure its uniqueness. Additionally, a `khulnasoft_object_bucket_acl` ACL is applied to the bucket, setting it to private and outputting the bucket name for use in your producer.

5. Add these resources to create a NATS account and your NATS credentials file:
    ```hcl
    resource "khulnasoft_mnq_nats_account" "large_messages" {
      name = "nats-acc-large-messages"
    }

    resource "khulnasoft_mnq_nats_credentials" "large_messages" {
      name       = "nats-large-messages-creds"
      account_id = khulnasoft_mnq_nats_account.large_messages.id
    }

    resource "local_file" "nats_credential" {
      content         = khulnasoft_mnq_nats_credentials.large_messages.file
      filename        = "large-messages.creds"
      file_permission = 644
    }

    output "nats_url" {
      value       = khulnasoft_mnq_nats_account.large_messages.endpoint
      description = "NATS url to use with the producer script"
    }
    ```
    We also output the NATS server URL.

6. Run `terraform init` and `terraform apply` to create the resources in your Khulnasoft account.
...
2. Configure the NATS CLI with the NATS URL and the credentials file.
    ...
    To upload the file, we will use the AWS CLI configured with the Khulnasoft endpoint and credentials because Khulnasoft Object storage is fully compliant with S3.
...
We continue using the Khulnasoft ecosystem and deploy the consumer using a Serverless Function in Python.
...
5. Before proceeding with the function's logic, improve the Terraform code by adding the following code to your `main.tf` file:
    ...
    ```
    The `null_resource` is used to download and package the correct versions of the libraries that we use with the function. Learn more about this in the [Khulnasoft documentation.](/serverless/functions/how-to/package-function-dependencies-in-zip/#specific-libraries-(with-needs-for-specific-c-compiled-code))

6. Create the function namespace.
    ```hcl
    resource "khulnasoft_function_namespace" "large_messages" {
...
7. Add the resource to set up the function.
    ```hcl
    resource "khulnasoft_function" "large_messages" {
      namespace_id = khulnasoft_function_namespace.large_messages.id
      runtime      = "python311"
      ...
      environment_variables = {
        ENDPOINT_URL  = khulnasoft_object_bucket.large_messages.api_endpoint
        BUCKET_REGION = khulnasoft_object_bucket.large_messages.region
        BUCKET_NAME   = khulnasoft_object_bucket.large_messages.name
      }
...
8. Create the function trigger to "wake up" the function when a NATS message comes in.
    ```hcl
    resource "khulnasoft_function_trigger" "large_messages" {
      function_id = khulnasoft_function.large_messages.id
...
10. Get the input file name from the body, define the PDF file name from this, and set up the Amazon S3 client to upload the file with Khulnasoft credentials.
    ```python
...
In this introductory tutorial, we have demonstrated the usage of the NATS server for Messaging and Queuing, along with other services from the Khulnasoft ecosystem, to facilitate the transfer of large messages surpassing the typical size constraints. There are possibilities to expand upon this tutorial for various use cases, such as:
