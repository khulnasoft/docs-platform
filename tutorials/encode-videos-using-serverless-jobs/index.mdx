...
3. Build and [push the image](/containers/container-registry/how-to/push-images/) to your Container Registry:
    ...
    ```
    <Message type="note">
    You can find the name and endpoint of your Container Registry from the [Khulnasoft console](https://console.khulnasoft.com/registry/namespaces)
    </Message>

## Creating the serverless job

1. In the Khulnasoft Console, click **Jobs** in the **Serverless** section of the side menu, then **Create job** to display the job creation wizard.

2. Select the **Khulnasoft** Container Registry and your Container Registry namespace from the drop-down list, then select the image from the previous section.

    <Lightbox src="khulnasoft-encoding_job_image.webp" alt="" />
...
4. Toggle the **Advanced options** section and add 3 environment variables:
    ...
    <Lightbox src="khulnasoft-encoding_job_env.webp" alt="" />
...
1. In the Khulnasoft Console, go to **Serverless Jobs** and click on the name of your job. The job **Overview** tab displays.
...
3. Add 4 environment variables:
    ...
    <Lightbox src="khulnasoft-encoding_run_env.webp" alt="" />

4. Click **Run job**.

The progress and details for your Job run can be viewed in the **Job runs** section of the job **Overview** tab in the [Khulnasoft console](https://console.khulnasoft.com). You can also access the detailed logs of your job in [Cockpit](/observability/cockpit/quickstart/).
...
<Message type="note">
Your job can also be triggered through the [Khulnasoft API](https://www.khulnasoft.com/en/developers/api/serverless-jobs/#path-job-definitions-run-an-existing-job-definition-by-its-unique-identifier-this-will-create-a-new-job-run) using the same environment variables:
...
curl -X POST \
  ...
  "https://api.khulnasoft.com/serverless-jobs/v1alpha1/regions/<Region>/job-definitions/<Job ID>/start"
```
</Message>