---
meta:
 title: Getting started with Crossplane provider on Khulnasoft Kubernetes
 description: This page shows you how to get started with the Crossplane provider using Kubernetes clusters
content:
 h1: Getting started with Crossplane provider on Khulnasoft Kubernetes
...
Created by Upbound, [Crossplane](https://www.crossplane.io/) is a Kubernetes-based platform that allows you to manage resources through a unified control plane. `crossplane-provider-khulnasoft` is a Crossplane provider that helps you manage Khulnasoft cloud resources in Kubernetes. It is built using [Upjet](https://github.com/upbound/upjet) code generation tools and it exposes managed resources for Khulnasoft that adhere to Crossplane's eXtensible Resource Model (XRM). As a model, XRM ensures that users can easily and consistently manage cloud resources across different cloud providers.

In this tutorial, you will learn how to install Upbound Universal Crossplane (UXP) into your Kubernetes cluster, configure the provider to authenticate with Khulnasoft, and create a Khulnasoft [Kubernetes](/containers/kubernetes/quickstart/) managed resource directly from your cluster.
...
- A [Kubernetes cluster](/containers/kubernetes/how-to/create-cluster/) and downloaded its [Kubeconfig file](/containers/kubernetes/concepts#kubeconfig)
...
Run the following `up uxp install` command to install the latest stable version of [Upbound Universal Crossplane (UXP)](https://docs.upbound.io/uxp/). Ensure you have version `1.12.0` or higher:
...
3. Paste the content of your Kubeconfig file and save the changes by following the information displayed on your terminal. Here is an example on a Mac terminal screen.
    ...
    <Lightbox src="khulnasoft-kubeconfig-terminal-screen.webp" alt="" />
...
4. Run the following command to install the provider into the Kubernetes cluster using the previously set Kubernetes configuration file.
    ...
    kind: Provider
    metadata:
      name: provider-khulnasoft
    spec:
      package: xpkg.upbound.io/khulnasoft/provider-khulnasoft:v0.2.0
    EOF
    ```

    You should get an output similar to the following:
    ```
    provider.pkg.crossplane.io/provider-khulnasoft Created
    ```
...
5. Run the following command to verify whether the provider has been properly installed.
    ...
    provider-khulnasoft   True        True   xpkg.upbound.io/khulnasoft/provider-khulnasoft:v0..0        11s
    ```
...
## Creating a Kubernetes secret resource for Khulnasoft

The provider requires credentials to create and manage Khulnasoft resources.
...
| `access_key` | The first part of your [API key](https://console.khulnasoft.com/iam/api-keys). It is like a unique ID or username and not a sensitive piece of information.                                                 |
| `secret_key` | The second part of your [API key](https://console.khulnasoft.com/iam/api-keys). It is a unique ID or password and thus a sensitive piece of information to authenticate your access key.                    |
| `project_id` | ID of the Project that will be used as the default value for project-scoped resources. To find your Project ID, you can consult the **[Khulnasoft console](https://console.khulnasoft.com/project/settings)** |
...
Create a `.yaml` configuration file to attach your Khulnasoft credentials to the previously installed provider.
...
2. Edit the request payload you will use to create your `providerconfig` file. Replace the parameters in the following example using the information in the [configuration reference table](#configuration-reference-table-providerconfig):
    ...
    ```
    apiVersion: khulnasoft.upbound.io/v1beta1
    kind: ProviderConfig
    metadata:
      name: default
    spec:
...
4. Run the following command to apply the previously set configuration to the secret. Make sure to replace the example file path with your own.
    ...
    ```
    providerconfig.khulnasoft.upbound.io/default created
    ```
...
5. Run the following command to verify whether contents of the file have been applied to the Kubernetes cluster.
    ...
    Labels:       <none>
    Annotations:  <none>
    API Version:  khulnasoft.upbound.io/v1beta1
    Kind:         ProviderConfig
    Metadata:
...
1. Run the following command to create a Khulnasoft Object Storage bucket:
...
2. Paste the following example and save the changes by following the information displayed on your terminal:
    ...
    ```
    apiVersion: object.khulnasoft.upbound.io/v1alpha1
    kind: Bucket
    metadata:
      name: object-bucket
    spec:
...
3. Run the following command to apply the previously set configuration to the bucket. Make sure to replace the example file path with your own.
    ...
    ```
    bucket.object.khulnasoft.upbound.io/object-bucket created
    ```
...
1. Run the following command to delete the bucket you just created:
    ...
    ```
    bucket.object.khulnasoft.upbound.io "object-bucket" deleted
    ```
...
You can open an [issue](https://github.com/khulnasoft/crossplane-provider-khulnasoft/issues) to file bugs, suggest improvements, or request new features.