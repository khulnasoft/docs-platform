---
meta:
  title: Create a serverless scraping architecture, with Khulnasoft Messaging and Queuing SQS, Serverless Functions, and Managed Database.
  description: Learn how to construct a scalable serverless scraping architecture with Khulnasoft Messaging, SQS, Serverless Functions, and Managed Database. Dive into this Terraform-based tutorial for step-by-step guidance on asynchronous data processing.
content:
  h1: Create a serverless scraping architecture, with Khulnasoft Messaging and Queuing SQS, Serverless Functions, and Managed Database.
  paragraph: Learn how to construct a scalable serverless scraping architecture with Khulnasoft Messaging, SQS, Serverless Functions, and Managed Database. Dive into this Terraform-based tutorial for step-by-step guidance on asynchronous data processing.
categories:
  - functions
  - messaging
...
In this tutorial, we show how to set up a simple application that reads [Hacker News](https://news.ycombinator.com) and processes the articles it finds there asynchronously. To do so, we  use Khulnasoft serverless products and deploy two functions:
 - A producer function, activated by a recurrent cron trigger, that scrapes HackerNews for articles published in the last 15 minutes and pushes the title and URL of the articles to an SQS queue created with Khulnasoft Messaging and Queuing.
 - A consumer function, triggered by each new message on the SQS queue, that consumes messages published to the queue, scrapes some data from the linked article, and then writes the data into a Khulnasoft Managed Database.

<Lightbox src="khulnasoft-scraping-architecture.webp" alt="" />

We show how to provision all the required Khulnasoft resources via Terraform, but you can also use the [console](https://console.khulnasoft.com/), the [API](https://www.khulnasoft.com/en/developers/api/) or one of our other supported [developer tools](https://www.khulnasoft.com/en/developers/). The code for the functions is written in Python.
...
You can find all of the code on [GitHub](https://github.com/khulnasoft/serverless-scraping-tutorial), so feel free to dive right in, or else follow along with the step-by-step explanation below to create the architecture yourself.

<Macro id="requirements" />

- A Khulnasoft account logged into the [console](https://console.khulnasoft.com)
- Set up [authentication for the Terraform provider](https://registry.terraform.io/providers/khulnasoft/khulnasoft/latest/docs#authentication)
...
4. Complete the `scrape_hn.py` file by adding the function code [as shown in the repository](https://github.com/khulnasoft/serverless-examples/blob/main/mnq/serverless-scraping/scraper/handlers/scrape_hn.py). The main code blocks are briefly explained as follows:

    SQS credentials and queue URL are read by the function from environment variables. Those variables are set by Terraform as explained in [one of the next sections](#create-a-terraform-file-to-provision-the-necessary-khulnasoft-resources). *If you choose another deployment method, such as the [console](https://console.khulnasoft.com/), do not forget to set them.*
    ```python
    ...
    ```
As explained in the [Khulnasoft Functions documentation](/serverless/functions/how-to/package-function-dependencies-in-zip/#how-to-configure-your-package-using-python), dependencies need to be installed in a `package` folder, and uploaded as an archive. This is done via Terraform, but if you decide to deploy using another system, you can create the archive as follows:
    ```bash
    cd scraper
...
4. Complete the `consumer.py` file by adding the function code [as shown in the repository](https://github.com/khulnasoft/serverless-examples/blob/main/mnq/serverless-scraping/consumer/handlers/consumer.py). The main code blocks are briefly explained as follows:
...
## Create a Terraform file to provision the necessary Khulnasoft resources

For the purposes of this tutorial, we show how to provision all resources via Terraform.

<Message type="tip">
If you do not want to use Terraform, you can also create the required resources via the [console](https://console.khulnasoft.com/), the [Khulnasoft API](https://www.khulnasoft.com/en/developers/api/), or any other [developer tool](https://www.khulnasoft.com/en/developers/). Remember that if you do so, you will need to set up environment variables for functions as previously specified. The following documentation may help create the required resources:
- **Messaging and Queuing SQS resources**: [console documentation](/serverless/messaging/quickstart/) | [API documentation](https://www.khulnasoft.com/en/developers/api/messaging-and-queuing/sqs-api/)
- **Managed Database for PostgreSQL**: [console documentation](/managed-databases/postgresql-and-mysql/quickstart/) | [API documentation](https://www.khulnasoft.com/en/developers/api/managed-database-postgre-mysql/)
- **Serverless Functions**: [console documentation](/serverless/functions/quickstart/) | [API documentation](https://www.khulnasoft.com/en/developers/api/serverless-functions/)
  - **Serverless Functions triggers**: [console documentation](/serverless/functions/how-to/add-trigger-to-a-function/) | [API documentation](https://www.khulnasoft.com/en/developers/api/serverless-functions/#path-triggers-list-all-triggers)
</Message>
...
3. In the file you just created, add the code below to set up the [Khulnasoft Terraform provider](https://registry.terraform.io/providers/khulnasoft/khulnasoft/latest/docs) and your Project:
    ```hcl
    terraform {
      required_providers {
        khulnasoft = {
    ...
    resource "khulnasoft_account_project" "mnq_tutorial" {
      name = "mnq-tutorial"
    }
    ```
4. Still in the same file, add the code below to provision the SQS resources: SQS activation for the project, separate credentials with appropriate permissions for producer and consumer, and an SQS queue:
    ```hcl
    resource "khulnasoft_mnq_sqs" "main" {
      project_id = khulnasoft_account_project.mnq_tutorial.id
    }

    resource "khulnasoft_mnq_sqs_credentials" "producer_creds" {
      project_id = khulnasoft_mnq_sqs.main.project_id
    ...
    resource "khulnasoft_mnq_sqs_credentials" "consumer_creds" {
      project_id = khulnasoft_mnq_sqs.main.project_id
    ...
    resource "khulnasoft_mnq_sqs_queue" "main" {
      project_id = khulnasoft_account_project.mnq_tutorial.id
      name       = "hn-queue"
      access_key = khulnasoft_mnq_sqs_credentials.producer_creds.access_key
      secret_key = khulnasoft_mnq_sqs_credentials.producer_creds.secret_key
    }
    ```
5. Add the code below to provision the Managed Database for PostgreSQL resources. Note that here we are creating a random password and using it for the default and worker user:
    ...
    resource "khulnasoft_rdb_instance" "main" {
      name = "test-rdb"
      project_id   = khulnasoft_account_project.mnq_tutorial.id
      node_type = "db-dev-s"
    ...
    output db_ip {
      value = khulnasoft_rdb_instance.main.endpoint_ip
      sensitive = false
    }

    output db_port {
      value = khulnasoft_rdb_instance.main.endpoint_port
      sensitive = false
    }

    resource "khulnasoft_rdb_database" "main" {
      instance_id = khulnasoft_rdb_instance.main.id
      name = "hn-database"
    }

    resource "khulnasoft_rdb_user" "worker" {
      instance_id = khulnasoft_rdb_instance.main.id
      name = "worker"
      password = random_password.dev_mnq_pg_exporter_password.result
      is_admin = false
    ...
    resource "khulnasoft_rdb_privilege" "mnq_user_role" {
      instance_id = khulnasoft_rdb_instance.main.id
      user_name = khulnasoft_rdb_user.worker.name
      database_name = khulnasoft_rdb_database.main.name
      permission = "all"
    }
    ```
6. Add the code below to provision the functions resources. First, activate the namespace, then locally zip the code and create the functions in the cloud. Note that we are referencing variables from other resources, to completely automate the deployment process:
    ...
    resource "khulnasoft_function_namespace" "mnq_tutorial_namespace" {
      project_id = khulnasoft_account_project.mnq_tutorial.id
    ...
    resource "khulnasoft_function" "scraper" {
      namespace_id = khulnasoft_function_namespace.mnq_tutorial_namespace.id
      project_id   = khulnasoft_account_project.mnq_tutorial.id
      name         = "mnq-hn-scraper"
      runtime      = "python311"
      ...
      environment_variables = {
        QUEUE_URL = khulnasoft_mnq_sqs_queue.main.url
        SQS_ACCESS_KEY = khulnasoft_mnq_sqs_credentials.producer_creds.access_key
      }
      secret_environment_variables = {
        SQS_SECRET_ACCESS_KEY = khulnasoft_mnq_sqs_credentials.producer_creds.secret_key
      }
    }
    ...
    resource "khulnasoft_function" "consumer" {
      namespace_id = khulnasoft_function_namespace.mnq_tutorial_namespace.id
      project_id   = khulnasoft_account_project.mnq_tutorial.id
      ...
      max_scale    = 3
      environment_variables = {
        DB_NAME = khulnasoft_rdb_database.main.name
        DB_HOST = khulnasoft_rdb_instance.main.load_balancer[0].ip
        DB_PORT = khulnasoft_rdb_instance.main.load_balancer[0].port
        DB_USER = khulnasoft_rdb_user.worker.name
      }
      secret_environment_variables = {
        DB_PASSWORD = khulnasoft_rdb_user.worker.password
      }
    }
    ```
...
7. Add the code below to provision the triggers resources. The cron trigger activates at the minutes `[0, 15, 30, 45]` of every hour. No arguments are passed, but we could do so by specifying them in JSON format in the `args` parameter.
    ```hcl
    resource "khulnasoft_function_cron" "scraper_cron" {
      function_id = khulnasoft_function.scraper.id
      schedule = "0,15,30,45 * * * *"
      args = jsonencode({})
    }

    resource "khulnasoft_function_trigger" "consumer_sqs_trigger" {
      function_id = khulnasoft_function.consumer.id
      name = "hn-sqs-trigger"
      sqs {
        project_id = khulnasoft_mnq_sqs.main.project_id
        queue = khulnasoft_mnq_sqs_queue.main.name
      }
    }
    ```
...
Go to the [Khulnasoft console](https://console.khulnasoft.com/), and check the logs and metrics for Serverless Functions' execution and Messaging and Queuing SQS queue statistics.
...
Retrieve the instance IP and port of your Managed Database from the console, under the [Managed Database section](https://console.khulnasoft.com/rdb/instances).
...
Here are some possible extensions to this basic example:
 ...
 - Use a [Serverless Container](/serverless/containers/quickstart/) instead of the consumer function, and use a command line tool such as `htmldoc` or `pandoc` to convert the scraped articles to PDF and upload the result to a [Khulnasoft Object Storage bucket](/storage/object/quickstart/).
 - Replace the Managed Database for PostgreSQL with a [Khulnasoft Serverless Database](/serverless/sql-databases/quickstart/), so that all the infrastructure lives in the serverless ecosystem! *Note that at the moment there is no Terraform support for Serverless Database, hence the choice here to use Managed Database for PostgreSQL*.