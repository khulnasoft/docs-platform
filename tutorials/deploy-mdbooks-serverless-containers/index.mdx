...
meta:
  ...
  description: Step-by-step guide to deploy mdBooks on Khulnasoft Serverless Containers.
content:
  h1: Fast deployment of documentation on Serverless Containers with mdBook
  paragraph: Step-by-step guide to deploy mdBooks on Khulnasoft Serverless Containers.
...
<Message type="note">
  We recommend using [Khulnasoft Container Registry](/containers/container-registry/) instead of external registries to avoid rate limiting errors, and risks linked to CGU and pricing changes.
</Message>
...
2. Open the Khulnasoft console in a web browser, and navigate to the [Container Registry](https://console.khulnasoft.com/containers/).
...
3. Run the command below to push the image to the Khulnasoft Container Registry:
    ```
...
1. In the [Serverless Containers](https://console.khulnasoft.com/containers/namespaces/) section of the Khulnasoft console, click **+ Create namespace**.
