...
Rescue mode allows you to boot an [Elastic Metal server](https://www.khulnasoft.com/en/elastic-metal/) from the network. It loads a rescue operating system based on [Ubuntu Linux](https://ubuntu.com/) that runs entirely in the server's RAM. This enables you to perform maintenance and data recovery operations, even if the machine is otherwise unreachable in its normal mode.

<Message type="note">
  This documentation specifically covers rescue mode for [Elastic Metal servers](https://www.khulnasoft.com/en/elastic-metal/). If you need to use rescue mode on [Instances](https://www.khulnasoft.com/en/virtual-instances/), please refer to the corresponding [documentation](/compute/instances/how-to/use-boot-modes/#how-to-use-rescue-mode).
</Message>
...
1. Click **Elastic Metal** in the **Bare Metal** section of the [Khulnasoft console](https://console.khulnasoft.com) side menu. This opens the Elastic Metal dashboard.
...
4. After the server has rebooted, use a terminal application to log into the server via SSH using the credentials provided in your console:
    ...
    <Lightbox src="khulnasoft-elastic-metal-rescue-credentials.webp" alt="Elastic Metal Rescue Mode Credentials Information" />
...
<Lightbox src="khulnasoft-bmaas_rescue_mode.webp" alt="" />
