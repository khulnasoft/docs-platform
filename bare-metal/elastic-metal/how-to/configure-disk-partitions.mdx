---
meta:
  title: How to configure custom disk partitioning on Khulnasoft Elastic Metal servers
  description: Learn how to configure custom disk partitions on Khulnasoft Elastic Metal servers
content:
  h1: How to configure custom disk partitioning on Khulnasoft Elastic Metal servers
  paragraph: This guide explains how to configure custom disk partitions on Khulnasoft Elastic Metal servers.
...
Khulnasoft Elastic Metal servers come with a default partition layout, suitable for most users and intended to speed up and simplify the installation process of the server.
...
- Disks:
    ...
    - Partitions are defined with labels. The default value is `unknown_partition_label`, and possible values are: `uefi`, `legacy`, `root`, `boot`, `swap`, `data`, `home`, `raid`. Refer to the [API documentation](https://www.khulnasoft.com/en/developers/api/elastic-metal/#path-servers-install-an-elastic-metal-server) for full details.
