...
This can now be fixed by installing a new version of the OpenVPN scripts, which has been packaged in Khulnasoft's stable PPA and contains the fixed script.
...
1. Add Khulnasoft's stable PPA

```sh
add-apt-repository ppa:khulnasoft/stable
```
2. Add the khulnasoft-ovpn-scripts package containing the new scripts

```sh
apt -y install khulnasoft-ovpn-scripts
```
