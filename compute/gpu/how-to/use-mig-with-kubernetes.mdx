...
<Message type="note">
  * Khulnasoft offers MIG-compatible GPU Instances such as H100 PCIe GPU Instances
  * NVIDIA uses the term *GPU instance* to designate a MIG partition of a GPU (MIG= Multi-Instance GPU).
  * To avoid confusion, we use the term GPU Instance in this document to refer to the Khulnasoft GPU Instance, and *MIG partition* in the context of the MIG feature.
</Message>
...
- A [Kubernetes cluster](/containers/kubernetes/quickstart/#how-to-create-a-kubernetes-cluster) with a [GPU Instance](https://www.khulnasoft.com/en/gpu-instances/) as node

<Message type="tip">
    MIG is fully supported on [Khulnasoft managed Kubernetes](/containers/kubernetes/quickstart/) clusters (Kapsule and Kosmos).
</Message>
...
<Message type="tip">
    To enable autoscaling with [Kubernetes Kapsule](/containers/kubernetes/quickstart/), it is necessary to designate a tag for the node pool. This tag will be automatically inherited as a label by all nodes within that pool.
