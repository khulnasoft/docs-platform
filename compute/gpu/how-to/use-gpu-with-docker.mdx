---
meta:
  title: How to access the GPU with Docker on Khulnasoft GPU Instances
  description: Learn how to efficiently access and use GPUs with Docker on Khulnasoft GPU Instances.
content:
  h1: How to access the GPU with Docker on Khulnasoft GPU Instances
  paragraph: Learn how to efficiently access and use GPUs with Docker on Khulnasoft GPU Instances.
tags: gpu docker
dates:
...
2. Choose a Docker image from the containers shipped with your GPU Instance. See our [additional content](/compute/gpu/reference-content/docker-images/) for more information about the available Docker images, including the specific commands to run each of them.
...
We recommend that you map volumes from your GPU Instance to your Docker containers, so that your work is not lost when you exit the Docker container. For increased data redundancy, we also recommend storing the directories you map to your Docker containers on [Block Storage](/storage/block/quickstart/).
...
<Message type="important">
  When you connect to your GPU Instance, you are probably connecting as the `root` user. Once you then run and enter the Khulnasoft Docker container, the user `jovyan` is used to run Jupyter Lab. You should therefore adjust the user access rights of your local folder on your GPU Instance OS so that files can be read and written from and to the container as required. This can be achieved by setting the ownership of the directory to map on your GPU Instance OS to the user/group `1000:100`, as used by the user `jovyan` in Jupyter Lab.
...
## How to push and list Docker images stored on your Khulnasoft Registry

<Message type="tip">
  GPU Instances are compatible with [Khulnasoft Container Registry](/containers/container-registry/quickstart/). You can create a Container Registry to store and pull your own Docker images.
</Message>
