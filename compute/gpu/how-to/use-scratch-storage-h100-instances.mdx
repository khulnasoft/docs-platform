...
Khulnasoft H100 and L40S GPU Instances are equipped with additional scratch storage. This form of temporary Local Storage operates differently from our regular Local Storage.
...
Khulnasoft's H100 and L40S GPU Instances use NVMe disks for their scratch storage, which are fine-tuned for high-speed data access. This design ensures fast read and write speeds, which is crucial for applications demanding extensive data processing capabilities.
...
## How can I add scratch storage to my GPU Instance using the Khulnasoft CLI or console?
...
## How can I add scratch storage to my GPU Instance using Terraform?
    ```
    resource “khulnasoft_instance_volume” “scratch_volume” {
    size_in_gb = 3000
    type = “scratch”
    }
    resource “khulnasoft_instance-server” “myserver” {
    type = “H100-1-80G”
    image = “ubuntu_jammy_gpu_os_12”
    additional_volume_ids = [khulnasoft_instance_volume.scratch_volume.id]
    }
    ```