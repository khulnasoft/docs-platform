...
meta:
  ...
  description: This page explains how to use the preinstalled environment on a Khulnasoft Docker image for a GPU Instance
content:
...
GPU Instances have different types of preinstalled environments, depending on the OS image you chose during [creation of the Instance](/compute/gpu/how-to/create-manage-gpu-instance/):
...
2. Use the [official conda documentation](https://docs.conda.io/projects/conda/en/latest/commands.html) if you need any help managing your conda environment.
    ...
    As Docker is also preinstalled, you could choose to [launch one of Khulnasoft's ready-made Docker images](/compute/gpu/how-to/use-gpu-with-docker/) to access our latest working environments, if you wish.
...
1. [Connect to your Instance via SSH](/compute/instances/how-to/connect-to-instance/).
    ...
    You are now connected to your Instance, and see your OS. A minimum of packages, including Docker, are installed. Pipenv is **not** preinstalled here. You must launch a Khulnasoft AI Docker container to access the preinstalled Pipenv environment.
...
1. Launch an application. Here, we launch Jupyter Lab:
    ...
    <Message type="tip">
      Jupyter Lab is launched automatically when you run any Khulnasoft container image. You will see a message upon start-up telling you how to access the notebook in your browser. To override Jupyter Lab being launched automatically in this way, add `/bin/bash` to the end of your `docker run` command, e.g. `docker run --runtime=nvidia -it --rm -p 8888:8888 -p 6006:6006 rg.fr-par.scw.cloud/scw-ai/pytorch:latest /bin/bash`. This preempts the launch of Jupyter Lab at container startup, and replaces it with the specified command, in this case, a bash shell.
    </Message>
2. Open a browser window on your local computer, and enter the following URL. Replace `<ip-address>` with the IP address of your Khulnasoft GPU Instance, and `<my-token>` with the token shown in the last lines of terminal output after the `jupyter-lab` command.
    ```
    <ip-address>:8888/lab?token=<my-token>
    ```

    <Message type="tip">
      You can find the IP address of your Instance in the Khulnasoft console. In the side menu, click **Instances** to see a list of your Instances. The IP address of each of them is shown in the list that displays.
    </Message>

    Jupyter Lab now displays in your browser. You can use the Notebook, Console, or other features as required:

    <Lightbox src="khulnasoft-jupyter-lab.webp" alt="" />
