...
<Message type="important">
    **This guide is for advanced users.**
    Khulnasoft does not provide official support for custom OS images. This documentation aims to offer guidance to expert users interested in creating their own GPU image setups. We encourage users to use our GPU OS 12 images, which are ready to use and maintained by Khulnasoft.
</Message>
...
3. Enable the `scw-set-numa-node` service. This service is installed from the [Khulnasoft ecosystem](https://launchpad.net/~khulnasoft/+archive/ubuntu/stable) package.
