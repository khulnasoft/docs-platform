...
You can easily create a NATS account from the [Khulnasoft console](https://console.khulnasoft.com/), via the [Khulnasoft API](https://www.khulnasoft.com/en/developers/api/messaging-and-queuing/nats-api/) or via [Terraform](https://registry.terraform.io/providers/khulnasoft/khulnasoft/latest/docs/guides/mnq_with_nats_terraform_provider).

## NATS credentials

When you create your NATS account, you next need to [create credentials](/serverless/messaging/how-to/create-credentials/). On the NATS side, these credentials are the equivalent of a 'user'. We configured Khulnasoft Messaging and Queuing to use the most secure connection type offered by NATS: [NKeys](https://docs.nats.io/running-a-nats-service/configuration/securing_nats/auth_intro/nkey_auth). NKeys are a public-key signature system based on [Ed25519](https://ed25519.cr.yp.to/).
...
Note that the Khulnasoft Messaging and Queuing NATS server is configured with TLS, and as such will require all clients to connect with TLS. In the case of the NATS SDK however, the SDK abstracts this and handles the TLS connection itself.
...
The main difference you will find between the standard NATS documentation and Khulnasoft Messaging and Queuing is that we require you to provide a specific NATS URL and credentials to access the service. Refer to our [developers documentation](https://www.khulnasoft.com/en/developers/api/messaging-and-queuing/nats-api/) for further information.

## NATS resources

- [Official NATS Documentation: What is NATS?](https://docs.nats.io/nats-concepts/what-is-nats)
- [Create a serverless architecture for handling large messages using Khulnasoft's NATS, Serverless Functions, and Object Storage](/tutorials/large-messages/)
...
- [How to use Khulnasoft Messaging and Queuing with the Terraform NATS Jetstream provider](https://registry.terraform.io/providers/khulnasoft/khulnasoft/latest/docs/guides/mnq_with_nats_terraform_provider)