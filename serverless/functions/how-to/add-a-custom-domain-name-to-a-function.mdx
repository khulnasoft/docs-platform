...
meta:
  ...
  description: Guide to adding a custom domain name to your Khulnasoft serverless function.
content:
  h1: How to add a custom domain to a function
  paragraph: Guide to adding a custom domain name to your Khulnasoft serverless function.
...
1. On your domain provider's platform (for example, [Khulnasoft's domains](/network/domains-and-dns/how-to/manage-dns-records/)), add a CNAME record pointing to your function default URL in the domain's DNS zone file.
...
When adding a custom domain, the following tasks will be performed on Khulnasoft's side:
...
- your web application is hosted on a Khulnasoft Instance, with the IP `51.15.x.x`
