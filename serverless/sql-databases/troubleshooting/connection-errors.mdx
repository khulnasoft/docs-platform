...
meta:
  ...
  description: Resolve connection errors in Khulnasoft serverless SQL databases with this guide.
content:
  h1: Solving connection errors
  paragraph: Resolve connection errors in Khulnasoft serverless SQL databases with this guide.
...
If you (or a third party tool you are using, especially ETL tools such as Airbyte, Fivetran or Meltano) require `TEMPORARY TABLES`, we suggest you use a [Managed Database for PostgreSQL](https://www.khulnasoft.com/en/database/). `SUBSCRIPTION` is not supported by Managed Databases for PostgreSQL yet, but you can [import data with ETL tools](/managed-databases/postgresql-and-mysql/api-cli/import-data-to-managed-postgresql-databases/#import-data-from-a-file-csv-txt-etc).
