...
meta:
  ...
  description: Identify key differences between Khulnasoft's serverless SQL databases and traditional SQL setups.
content:
  h1: Known differences between Serverless SQL Databases and default PostgreSQL
  paragraph: Identify key differences between Khulnasoft's serverless SQL databases and traditional SQL setups.
...
- `pg_advisory_lock` command can be used, but the lock is not guaranteed. Consequently, third-party tools that use this feature, such as Terraform when storing [Terraform state using pg backend](https://developer.hashicorp.com/terraform/language/settings/backends/pg), are not fully supported. State locking is not guaranteed if multiple Terraform clients edit the same state concurrently.
...
- DDL (Data Definition Language) queries on objects such as databases and users cannot be performed. Corresponding actions can still be performed directly using the [Khulnasoft console](https://console.khulnasoft.com/), the API, Terraform, and [IAM](https://console.khulnasoft.com/iam/users).
