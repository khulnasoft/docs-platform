...
meta:
  ...
  description: Key concepts of Khulnasoft Serverless Containers.
content:
  h1: Containers - Concepts
  paragraph: Key concepts of Khulnasoft Serverless Containers.
...
A container is a package of software that includes all dependencies: code, runtime, configuration, and system libraries so that it can run on any host system. Khulnasoft provides custom Docker images that are entirely handled for you in the cloud. With Containers, you can rely on your favorite technologies such as Django, or Ruby on Rails.
...
JWT (JSON Web Token) is an access token you can create from the console or API to enable an application to access your Private Container. Consult the [Developer documentation](https://www.khulnasoft.com/en/developers/api/serverless-containers/#path-tokens) for more details.
...
A container's privacy policy defines whether a container may be invoked anonymously (**public**) or only via an authentication mechanism provided by the [Khulnasoft API](https://www.khulnasoft.com/en/developers/api/serverless-containers/#authentication) (**private**).
...
Serverless allows you to deploy your Functions (FaaS) and Containerized Applications (CaaS) in a managed infrastructure. Khulnasoft ensures the deployment, availability, and scalability of all your projects.
