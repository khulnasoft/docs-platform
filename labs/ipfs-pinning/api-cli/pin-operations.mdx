---
meta:
  title: Pinning operations with Khulnasoft CLI
  description: This page explains how to use the Khulnasoft IPFS Pinning API.
content:
  h1: Pinning operations with Khulnasoft CLI
  paragraph: This page explains how to use the Khulnasoft IPFS Pinning API.
...
In this page we show you how you can manage your pin operations via both the [Khulnasoft Command Line Interface](https://github.com/khulnasoft/khulnasoft-cli) and [Khulnasoft API](https://www.khulnasoft.com/en/developers/api/).

## Khulnasoft Command Line Interface overview

The [Khulnasoft Command Line Interface (CLI)](https://github.com/khulnasoft/khulnasoft-cli) allows you to create and interact with your volumes & pins. You will be able to create, add/delete content and manage volumes.
...
- [Added data to your own local node](/labs/ipfs-pinning/reference-content/install-ipfs-desktop/) and retrieved its [CID](/labs/ipfs-pinning/concepts/#content-identifier-(cid))
...
Enter the following CLI command to pin content to a volume using its CID. Just make sure to replace the example values with your own, using the reference table below.
  ...
  | :------------ | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
  | `volume-id`   | ID of the volume you wish to pin your content to. You can find it using either the [Khulnasoft console](https://console.khulnasoft.com/ipfs/volumes) or the [Khulnasoft API/CLI](/labs/ipfs-pinning/api-cli/volumes-operations/)                                      |
  ...
  curl -X POST \
    ...
    "https://api.khulnasoft.com/ipfs/v1alpha1/regions/$REGION_ID/pins/create-by-cid"
  ```
...
```
  curl -X POST \
    ...
    "https://api.khulnasoft.com/ipfs/v1alpha1/regions/$REGION_ID/pins/create-by-url"
```
...
```
  curl -X DELETE \
    -H "Content-Type: application/json" \
    -H "X-Auth-Token: $SECRET_KEY" \
    "https://api.khulnasoft.com/ipfs/v1alpha1/regions/$REGION_ID/pins/<pin_id>?volumes/<volume-id>"
```
...
```
  curl -X GET \
    -H "Content-Type: application/json" \
    -H "X-Auth-Token: $SCW_SECRET_KEY" \
    "https://api.khulnasoft.com/ipfs/v1alpha1/regions/$REGION_ID/pins?order_by=created_at_asc&volume_id=11111111-1111-1111-1111-111111111111"
```
